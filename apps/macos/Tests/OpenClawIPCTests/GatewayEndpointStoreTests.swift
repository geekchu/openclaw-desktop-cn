...
@Suite struct GatewayEndpointStoreTests {
    ...
    @Test func dashboardURLUsesLocalBasePathInLocalMode() throws {
        let config: GatewayConnection.Config = try (
            url: #require(URL(string: "ws://127.0.0.1:28789")),
            token: nil,
            password: nil)
        ...
        #expect(url.absoluteString == "http://127.0.0.1:28789/control/")
    }

    @Test func dashboardURLSkipsLocalBasePathInRemoteMode() throws {
        let config: GatewayConnection.Config = try (
            url: #require(URL(string: "ws://gateway.example:28789")),
            token: nil,
            password: nil)
        ...
        #expect(url.absoluteString == "http://gateway.example:28789/")
    }
    ...
    @Test func normalizeGatewayUrlAddsDefaultPortForLoopbackWs() {
        ...
        #expect(url?.port == 28789)
        #expect(url?.absoluteString == "ws://127.0.0.1:28789")
    }

    @Test func normalizeGatewayUrlRejectsNonLoopbackWs() {
        let url = GatewayRemoteConfig.normalizeGatewayUrl("ws://gateway.example:28789")
        #expect(url == nil)
    }
