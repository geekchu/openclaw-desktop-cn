...
- Restart: stop old gateway and run:
  `pkill -9 -f openclaw-gateway || true; nohup openclaw gateway run --bind loopback --port 28789 --force > /tmp/openclaw-gateway.log 2>&1 &`
- Verify: `openclaw channels status --probe`, `ss -ltnp | rg 28789`, `tail -n 120 /tmp/openclaw-gateway.log`.
...
- Mac packaging (dev): `scripts/package-mac-app.sh` defaults to current arch. Release guide: `docs/release-process.md`.
...
- Web provider stores creds at `~/.openclawcn/credentials/`; rerun `openclaw login` if logged out.
- Pi sessions live under `~/.openclawcn/sessions/` by default; the base directory is not configurable.
- Environment variables: see `~/.profile`.
- Never commit or publish real phone numbers, videos, or live configuration values. Use obviously fake placeholders in docs, tests, and examples.
- Release flow: always read `docs/release-process.md` before any desktop release work; do not ask routine questions once it answers them.
...
- When asked to open a “session” file, open the Pi session logs under `~/.openclawcn/agents/<agentId>/sessions/*.jsonl` (use the `agent=<id>` value in the Runtime line of the system prompt; newest unless a specific ID is given), not the default `sessions.json`. If logs are needed from another machine, SSH via Tailscale and read the same path there.
...
- Release only already-on-npm plugins. Verify the extension package name in its `package.json`, then confirm the published version with `npm view <name> version --userconfig "$(mktemp)"` before publishing.
