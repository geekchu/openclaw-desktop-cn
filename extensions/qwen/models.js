...
export const QWEN_STANDARD_GLOBAL_BASE_URL = "https://dashscope-intl.aliyuncs.com/compatible-mode/v1";
...
export const QWEN_MODEL_CATALOG = [
    {
        id: "qwen3.5-plus",
        name: "qwen3.5-plus",
        reasoning: false,
...
export function isQwenCodingPlanBaseUrl(baseUrl) {
    if (!baseUrl?.trim()) {
        return false;
    }
    try {
...
export function isQwen36PlusSupportedBaseUrl(baseUrl) {
    return !isQwenCodingPlanBaseUrl(baseUrl);
}
export function buildQwenModelCatalogForBaseUrl(baseUrl) {
    return isQwen36PlusSupportedBaseUrl(baseUrl)
        ? QWEN_MODEL_CATALOG
        : QWEN_MODEL_CATALOG.filter((model) => model.id !== QWEN_36_PLUS_MODEL_ID);
}
export function isNativeQwenBaseUrl(baseUrl) {
    return supportsNativeStreamingUsageCompat({
        providerId: "qwen",
        baseUrl,
    });
}
export function applyQwenNativeStreamingUsageCompat(provider) {
...
export function buildQwenDefaultModelDefinition() {
