...
describe("utils", () => {
  ...
  describe("formatDingTalkErrorPayload", () => {
    it("formats code and message with serialized payload", () => {
      const text = formatDingTalkErrorPayload({
        code: "invalidParameter",
        message: "robotCode required",
      });
  ...
  describe("retryWithBackoff", () => {
    ...
    it("logs payload details with unified prefix before retry decision", async () => {
      const fn = vi.fn<() => Promise<string>>().mockRejectedValue({
        response: { status: 400, data: { code: "invalidParameter", message: "bad payload" } },
      });
      const log = { debug: vi.fn() };

      await expect(
        retryWithBackoff(fn, { maxRetries: 3, baseDelayMs: 10, log: log as any }),
      ).rejects.toBeDefined();
