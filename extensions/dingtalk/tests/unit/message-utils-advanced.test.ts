...
import { extractMessageContent } from "../../src/message-utils.js";

describe("message-utils advanced extraction", () => {
  it("extracts picture/audio/video/file payloads", () => {
    expect(
      extractMessageContent({ msgtype: "picture", content: { downloadCode: "d1" } } as any),
    ).toEqual(
    ...
    expect(
      extractMessageContent({
        msgtype: "audio",
        content: { recognition: "语音识别", downloadCode: "d2" },
      } as any),
    ).toEqual(expect.objectContaining({ text: "语音识别", mediaPath: "d2", mediaType: "audio" }));

    expect(
      extractMessageContent({ msgtype: "video", content: { downloadCode: "d3" } } as any),
    ).toEqual(
    ...
    expect(
      extractMessageContent({
        msgtype: "file",
        content: { downloadCode: "d4", fileName: "a.pdf" },
      } as any),
    ).toEqual(
