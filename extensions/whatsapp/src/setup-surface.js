...
import { DEFAULT_ACCOUNT_ID, normalizeAllowFromEntries, normalizeE164, pathExists, splitSetupEntries, setSetupChannelEnabled, } from "openclaw/plugin-sdk/setup";
...
async function promptWhatsAppOwnerAllowFrom(params) {
    ...
    const allowFrom = normalizeAllowFromEntries([...existingAllowFrom.filter((item) => item !== "*"), normalized], normalizeE164);
...
export const whatsappSetupWizard = {
    channel,
    status: {
    ...
    finalize: async ({ cfg, accountId, forceAllowFrom, prompter, runtime }) => {
        let next = accountId === DEFAULT_ACCOUNT_ID
            ? cfg
