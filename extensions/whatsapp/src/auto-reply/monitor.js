...
import { defaultRuntime, formatDurationPrecise, } from "openclaw/plugin-sdk/runtime-env";
...
export async function monitorWebChannel(verbose, listenerFactory = monitorWebInbox, keepAlive = true, replyResolver = getReplyFromConfig, runtime = defaultRuntime, abortSignal, tuning = {}) {
    ...
    while (true) {
        ...
        active.unregisterUnhandled = registerUnhandledRejectionHandler((reason) => {
            ...
            reconnectLogger.warn({ connectionId: active.connectionId, error: errorStr }, "web reconnect: unhandled rejection from WhatsApp socket; forcing reconnect");
