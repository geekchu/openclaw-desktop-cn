...
function extractParticipantAddresses(chat) {
    const raw = (Array.isArray(chat.participants) ? chat.participants : null) ??
    ...
    for (const entry of raw) {
        ...
        if (entry && typeof entry === "object") {
            const record = entry;
            const candidate = (typeof record.address === "string" && record.address) ||
...
export async function createChatForHandle(params) {
    ...
    if (!res.ok) {
        ...
        if (res.status === 400 ||
            ...
            throw new Error(`BlueBubbles send failed: Cannot create new chat - Private API must be enabled. Original error: ${errorText || res.status}`);
        }
...
export async function sendMessageBlueBubbles(to, text, opts = {}) {
    ...
    if (privateApiDecision.throwEffectDisabledError) {
        throw new Error("BlueBubbles send failed: reply/effect requires Private API, but it is disabled on the BlueBubbles server.");
    }
