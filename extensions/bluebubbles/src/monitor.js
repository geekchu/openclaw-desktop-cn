...
import { DEFAULT_WEBHOOK_PATH, normalizeWebhookPath, resolveWebhookPathFromConfig, } from "./monitor-shared.js";
import { fetchBlueBubblesServerInfo } from "./probe.js";
import { WEBHOOK_RATE_LIMIT_DEFAULTS, createFixedWindowRateLimiter, createWebhookInFlightLimiter, registerWebhookTargetWithPluginRoute, readWebhookBodyOrReject, resolveRequestClientIp, resolveWebhookTargetWithAuthOrRejectSync, withResolvedWebhookRequestPipeline, } from "./runtime-api.js";
...
export function clearBlueBubblesWebhookSecurityStateForTest() {
...
function resolveWebhookClientIp(req, trustedProxies, allowRealIpFallback) {
    ...
    return (resolveRequestClientIp(req, [...trustedProxies], allowRealIpFallback) ??
...
export async function handleBlueBubblesWebhookRequest(req, res) {
    ...
    return await withResolvedWebhookRequestPipeline({
        ...
        handle: async ({ path, targets }) => {
            ...
            const headerToken = req.headers["x-guid"] ??
            ...
            if (firstTarget) {
                logVerbose(firstTarget.core, firstTarget.runtime, `webhook received path=${path} keys=${Object.keys(payload).join(",") || "none"}`);
            }
            ...
            if (reaction) {
                if (firstTarget) {
                    logVerbose(firstTarget.core, firstTarget.runtime, `webhook accepted reaction sender=${reaction.senderId} msg=${reaction.messageId} action=${reaction.action}`);
                }
            }
            else if (message) {
                if (firstTarget) {
                    logVerbose(firstTarget.core, firstTarget.runtime, `webhook accepted sender=${message.senderId} group=${message.isGroup} chatGuid=${message.chatGuid ?? ""} chatId=${message.chatId ?? ""}`);
                }
            }
            return true;
        },
