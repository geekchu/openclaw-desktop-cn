...
import { buildMessagePlaceholder, formatGroupAllowlistEntry, formatGroupMembers, formatReplyTag, normalizeParticipantList, parseTapbackText, resolveGroupFlagFromChatGuid, resolveTapbackContext, } from "./monitor-normalize.js";
import { getShortIdForUuid, rememberBlueBubblesReplyCache, resolveBlueBubblesMessageId, resolveReplyContextFromCache, } from "./monitor-reply-cache.js";
import { hasBlueBubblesSelfChatCopy, rememberBlueBubblesSelfChatCopy, } from "./monitor-self-chat-cache.js";
import { enrichBlueBubblesParticipantsWithContactNames } from "./participant-contact-names.js";
import { isBlueBubblesPrivateApiEnabled } from "./probe.js";
import { normalizeBlueBubblesReactionInput, sendBlueBubblesReaction } from "./reactions.js";
import { DM_GROUP_ACCESS_REASON, createChannelPairingController, createChannelReplyPipeline, evictOldHistoryKeys, logAckFailure, logInboundDrop, logTypingFailure, mapAllowFromEntries, readStoreAllowFromForDmPolicy, recordPendingHistoryEntryIfEnabled, resolveAckReaction, resolveDmGroupAccessWithLists, resolveControlCommandGate, stripMarkdown, } from "./runtime-api.js";
import { normalizeSecretInputString } from "./secret-input.js";
import { resolveChatGuidForTarget, sendMessageBlueBubbles } from "./send.js";
import { extractHandleFromChatGuid, formatBlueBubblesChatTarget, isAllowedBlueBubblesSender, normalizeBlueBubblesHandle, } from "./targets.js";
...
async function fetchBlueBubblesParticipantsForInboundMessage(params) {
    ...
    for (let offset = 0; offset < 5000; offset += limit) {
        ...
        for (const chat of chats) {
            ...
            const matches = (params.chatGuid && chatGuid === params.chatGuid) ||
...
function isBlueBubblesSelfChatMessage(message, isGroup) {
    ...
    const chatHandle = (message.chatGuid ? extractHandleFromChatGuid(message.chatGuid) : null) ??
...
export function logVerbose(core, runtime, message) {
...
function logGroupAllowlistHint(params) {
    ...
    const accountHint = params.accountId
        ...
        : "";
    if (params.entry) {
        log(`[bluebubbles] group message blocked (${params.reason}). Allow this group by adding ` +
    ...
    log(`[bluebubbles] group message blocked (${params.reason}). Allow groups by setting ` +
...
export async function processMessage(message, target) {
    ...
    if (message.fromMe) {
        ...
        cacheInboundMessage();
        const confirmedAssistantOutbound = confirmedOutboundCacheEntry?.senderLabel === "me" &&
    ...
    if (isGroup &&
        ...
        message.participants = await enrichBlueBubblesParticipantsWithContactNames(message.participants);
    }
    ...
    cacheInboundMessage();
    const maxBytes = account.config.mediaMaxMb && account.config.mediaMaxMb > 0
    ...
    if (!chatGuidForActions && baseUrl && password) {
        const resolveTarget = isGroup && (chatId || chatIdentifier)
            ? chatId
                ? { kind: "chat_id", chatId }
    ...
    const shouldAckReaction = () => Boolean(ackReactionValue &&
    ...
    const ackReactionPromise = shouldAckReaction() && ackMessageId && chatGuidForActions && ackReactionValue
        ...
        await core.channel.reply.dispatchReplyWithBufferedBlockDispatcher({
            ...
            dispatcherOptions: {
                ...
                deliver: async (payload, info) => {
                    const rawReplyToId = privateApiEnabled && typeof payload.replyToId === "string"
                    ...
                    const textLimit = account.config.textChunkLimit && account.config.textChunkLimit > 0
                    ...
                    const text = sanitizeReplyDirectiveText(core.channel.text.convertMarkdownTables(payload.text ?? "", tableMode));
                    const chunks = chunkMode === "newline"
                        ? resolveTextChunksWithFallback(text, core.channel.text.chunkTextWithMode(text, textLimit, chunkMode))
                        : resolveTextChunksWithFallback(text, core.channel.text.chunkMarkdownText(text, textLimit));
                    if (!chunks.length) {
                        return;
                    }
...
export async function processReaction(reaction, target) {
    ...
    const peerId = reaction.isGroup
        ? (chatGuid ?? chatIdentifier ?? (chatId ? String(chatId) : "group"))
        : reaction.senderId;
    const requireMention = reaction.isGroup &&
    ...
    const text = reaction.action === "removed"
