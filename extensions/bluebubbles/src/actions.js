...
import { BLUEBUBBLES_ACTION_NAMES, BLUEBUBBLES_ACTIONS, createActionGate, extractToolSend, jsonResult, readNumberParam, readBooleanParam, readReactionParams, readStringParam, } from "./runtime-api.js";
import { normalizeSecretInputString } from "./secret-input.js";
import { normalizeBlueBubblesHandle, normalizeBlueBubblesMessagingTarget, parseBlueBubblesTarget, } from "./targets.js";
const loadBlueBubblesActionsRuntime = createLazyRuntimeNamedExport(() => import("./actions.runtime.js"), "blueBubblesActionsRuntime");
...
export const bluebubblesMessageActions = {
    ...
    handleAction: async ({ action, params, cfg, accountId, toolContext }) => {
        ...
        if (action === "edit") {
            ...
            const rawMessageId = readStringParam(params, "messageId");
            const newText = readStringParam(params, "text") ??
            ...
            if (!rawMessageId || !newText) {
                ...
                throw new Error(`BlueBubbles edit requires: ${missing.join(", ")}. ` +
        ...
        if (action === "reply") {
            ...
            if (!rawMessageId || !text || !to) {
                ...
                throw new Error(`BlueBubbles reply requires: ${missing.join(", ")}. ` +
        ...
        if (action === "sendWithEffect") {
            ...
            if (!text || !to || !effectId) {
                ...
                throw new Error(`BlueBubbles sendWithEffect requires: ${missing.join(", ")}. ` +
        ...
        if (action === "sendAttachment" || action === "upload-file") {
            ...
            if (base64Buffer) {
                ...
                throw new Error(`BlueBubbles ${action}: filePath not supported in action, provide buffer as base64.`);
            }
            else {
