...
export function buildMessagePlaceholder(message) {
...
function extractReplyMetadata(message) {
    const replyRaw = message["replyTo"] ??
        message["reply_to"] ??
    ...
    const replySenderRaw = readString(replyHandle, "address") ??
    ...
    const normalizedSender = replySenderRaw
        ...
        : undefined;
    const replyToBody = readString(replyRecord, "text") ??
        readString(replyRecord, "body") ??
        readString(replyRecord, "message") ??
        readString(replyRecord, "subject") ??
        undefined;
    const directReplyId = readString(message, "replyToMessageGuid") ??
    ...
    const associatedType = readNumberLike(message, "associatedMessageType") ??
        readNumberLike(message, "associated_message_type");
    const associatedGuid = readString(message, "associatedMessageGuid") ??
    ...
    const fallbackReplyId = !replyToId && threadOriginatorGuid && threadOriginatorGuid !== messageGuid
...
function extractSenderInfo(message) {
    ...
    const senderIdRaw = readString(handle, "address") ??
    ...
    const senderName = readString(handle, "displayName") ??
...
function extractChatContext(message) {
    ...
    const chatGuid = readString(message, "chatGuid") ??
    ...
    const chatIdentifier = readString(message, "chatIdentifier") ??
    ...
    const chatId = readNumberLike(message, "chatId") ??
    ...
    const explicitIsGroup = readBoolean(message, "isGroup") ??
        readBoolean(message, "is_group") ??
        readBoolean(chat, "isGroup") ??
        readBoolean(message, "group");
    const isGroup = typeof groupFromChatGuid === "boolean"
        ? groupFromChatGuid
...
function normalizeParticipantEntry(entry) {
    ...
    const idRaw = readString(record, "address") ??
    ...
    const nameRaw = readString(record, "displayName") ??
...
export function normalizeParticipantList(raw) {
...
export function resolveGroupFlagFromChatGuid(chatGuid) {
...
export function resolveTapbackContext(message) {
    ...
    const replyToId = normalizeOptionalString(message.associatedMessageGuid) ??
...
export function normalizeWebhookMessage(payload) {
    ...
    const messageId = readString(message, "guid") ??
        readString(message, "id") ??
        readString(message, "messageId") ??
        undefined;
    const balloonBundleId = readString(message, "balloonBundleId");
    const associatedMessageGuid = readString(message, "associatedMessageGuid") ??
        readString(message, "associated_message_guid") ??
        readString(message, "associatedMessageId") ??
        undefined;
    const associatedMessageType = readNumberLike(message, "associatedMessageType") ??
        readNumberLike(message, "associated_message_type");
    const associatedMessageEmoji = readString(message, "associatedMessageEmoji") ??
        readString(message, "associated_message_emoji") ??
        readString(message, "reactionEmoji") ??
        readString(message, "reaction_emoji") ??
        undefined;
    const isTapback = readBoolean(message, "isTapback") ??
        readBoolean(message, "is_tapback") ??
        readBoolean(message, "tapback") ??
        undefined;
    const timestampRaw = readNumber(message, "date") ??
...
export function normalizeWebhookReaction(payload) {
    const message = extractMessagePayload(payload);
    if (!message) {
        return null;
    }
    ...
    const associatedEmoji = readString(message, "associatedMessageEmoji") ??
        readString(message, "associated_message_emoji") ??
        readString(message, "reactionEmoji") ??
        readString(message, "reaction_emoji");
    ...
    const fromMe = readBoolean(message, "isFromMe") ?? readBoolean(message, "is_from_me");
    const timestampRaw = readNumberLike(message, "date") ??
