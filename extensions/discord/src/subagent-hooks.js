...
import { autoBindSpawnedDiscordSubagent, listThreadBindingsBySessionKey, unbindThreadBindingsBySessionKey, } from "./monitor/thread-bindings.js";
...
function resolveThreadBindingFlags(api, accountId) {
    ...
    return {
        enabled: accountThreadBindings?.enabled ??
            baseThreadBindings?.enabled ??
            api.config.session?.threadBindings?.enabled ??
            true,
        spawnSubagentSessions: accountThreadBindings?.spawnSubagentSessions ??
            baseThreadBindings?.spawnSubagentSessions ??
            false,
    };
}
export async function handleDiscordSubagentSpawning(api, event) {
...
export function handleDiscordSubagentEnded(event) {
...
export function handleDiscordSubagentDeliveryTarget(event) {
    ...
    const requesterThreadId = event.requesterOrigin?.threadId != null && event.requesterOrigin.threadId !== ""
...
export function registerDiscordSubagentHooks(api) {
