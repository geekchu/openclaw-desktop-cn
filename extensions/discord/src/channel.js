...
import { buildPluginApprovalRequestMessage, buildPluginApprovalResolvedMessage, } from "openclaw/plugin-sdk/approval-runtime";
...
import { isDiscordExecApprovalClientEnabled, shouldSuppressLocalDiscordExecApprovalPrompt, } from "./exec-approvals.js";
import { resolveDiscordGroupRequireMention, resolveDiscordGroupToolPolicy, } from "./group-policy.js";
import { looksLikeDiscordTargetId, normalizeDiscordMessagingTarget, normalizeDiscordOutboundTarget, } from "./normalize.js";
import { resolveDiscordOutboundSessionRoute } from "./outbound-session-route.js";
import { resolveDiscordUserAllowlist } from "./resolve-users.js";
import { buildTokenChannelStatusSummary, DEFAULT_ACCOUNT_ID, getChatChannelMeta, PAIRING_APPROVED_MESSAGE, projectCredentialSnapshotFields, resolveConfiguredFromCredentialStatuses, } from "./runtime-api.js";
...
function buildDiscordExecApprovalCustomId(approvalId, action) {
    return [
        `${DISCORD_EXEC_APPROVAL_KEY}:id=${encodeCustomIdValue(approvalId)}`,
        `action=${action}`,
    ].join(";");
}
function formatDiscordApprovalPreview(value, maxChars) {
    const trimmed = value
...
function buildDiscordPluginPendingComponentSpec(params) {
    ...
    const severityLabel = severity === "critical" ? "Critical" : severity === "info" ? "Info" : "Warning";
    ...
    const metadataLines = [`- Severity: ${severityLabel}`];
    if (request.toolName) {
...
function buildDiscordPluginResolvedComponentSpec(params) {
    ...
    const decisionLabel = params.resolved.decision === "allow-once"
        ? "Allowed (once)"
        : params.resolved.decision === "allow-always"
            ? "Allowed (always)"
            : "Denied";
    const accentColor = params.resolved.decision === "deny"
        ? "#ED4245"
        : params.resolved.decision === "allow-always"
            ? "#5865F2"
            : "#57F287";
    ...
    base: {
        ...
        allowlist: {
            ...buildLegacyDmAccountAllowlistAdapter({
                channelId: "discord",
                resolveAccount: resolveDiscordAccount,
                normalize: ({ cfg, accountId, values }) => discordConfigAdapter.formatAllowFrom({ cfg, accountId, allowFrom: values }),
        ...
        execApprovals: {
            getInitiatingSurfaceState: ({ cfg, accountId }) => isDiscordExecApprovalClientEnabled({ cfg, accountId })
                ? { kind: "enabled" }
                : { kind: "disabled" },
            ...
            shouldSuppressForwardingFallback: ({ cfg, target }) => (normalizeMessageChannel(target.channel) ?? target.channel) === "discord" &&
                isDiscordExecApprovalClientEnabled({ cfg, accountId: target.accountId }),
            buildPluginPendingPayload: ({ cfg, request, target, nowMs }) => {
                const text = formatDiscordApprovalPreview(buildPluginApprovalRequestMessage(request, nowMs), 10_000);
                const execApproval = {
                ...
                const interactiveEnabled = normalizedChannel === "discord" &&
                    isDiscordExecApprovalClientEnabled({ cfg, accountId: target.accountId });
            ...
            buildPluginResolvedPayload: ({ resolved }) => {
                ...
                const text = formatDiscordApprovalPreview(buildPluginApprovalResolvedMessage(resolved), 10_000);
        ...
        status: createComputedAccountStatusAdapter({
            ...
            probeAccount: async ({ account, timeoutMs }) => (await loadDiscordProbeRuntime()).probeDiscord(account.token, timeoutMs, {
        ...
        gateway: {
            startAccount: async (ctx) => {
                ...
                let discordBotLabel = "";
                try {
                    const probe = await (await loadDiscordProbeRuntime()).probeDiscord(token, 2500, {
                        includeApplication: true,
                    });
