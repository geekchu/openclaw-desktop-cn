...
export async function sendMessageDiscord(to, text, opts = {}) {
    ...
    const mediaMaxBytes = typeof accountInfo.config.mediaMaxMb === "number"
    ...
    if (isForumLikeType(channelType)) {
        ...
        try {
            if (opts.mediaUrl) {
                ...
                await sendDiscordMedia(rest, threadId, mediaCaption ?? "", opts.mediaUrl, opts.filename, opts.mediaLocalRoots, mediaMaxBytes, undefined, request, accountInfo.config.maxLinesPerMessage, undefined, undefined, chunkMode, opts.silent);
    ...
    try {
        if (opts.mediaUrl) {
            result = await sendDiscordMedia(rest, channelId, textWithMentions, opts.mediaUrl, opts.filename, opts.mediaLocalRoots, mediaMaxBytes, opts.replyTo, request, accountInfo.config.maxLinesPerMessage, opts.components, opts.embeds, chunkMode, opts.silent);
        }
        else {
            result = await sendDiscordText(rest, channelId, textWithMentions, opts.replyTo, request, accountInfo.config.maxLinesPerMessage, opts.components, opts.embeds, chunkMode, opts.silent);
        }
    }
    catch (err) {
        throw await buildDiscordSendError(err, {
