...
import { parseFfprobeCodecAndSampleRate, runFfmpeg, runFfprobe, } from "openclaw/plugin-sdk/media-runtime";
...
function createRateLimitError(response, body, request) {
    const compatRequest = request ??
        new Request("https://discord.com/api/v10/channels/voice/messages", {
            method: "POST",
        });
    const RateLimitErrorCtor = RateLimitError;
 ...
 */
export async function sendDiscordVoiceMessage(rest, channelId, audioBuffer, metadata, replyTo, request, silent, token) {
