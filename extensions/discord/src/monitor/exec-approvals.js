...
import { getExecApprovalApproverDmNoticeText, resolveExecApprovalCommandDisplay, } from "openclaw/plugin-sdk/approval-runtime";
...
class ExecApprovalContainer extends DiscordUiContainer {
    constructor(params) {
        ...
        if (params.commandSecondaryPreview) {
            components.push(new TextDisplay(`### Shell Preview\n\`\`\`\n${params.commandSecondaryPreview}\n\`\`\``));
        }
...
function resolveApprovalAgentId(request) {
    return request.request.agentId?.trim() || null;
}
function resolveApprovalSessionKey(request) {
...
function createExecApprovalRequestContainer(params) {
    const { commandPreview, commandSecondaryPreview } = resolveExecApprovalPreviews(params.request.request, 1000, 500);
...
export class DiscordExecApprovalHandler {
    ...
    async start() {
        ...
        this.gatewayClient = await (this.opts.__testing?.createGatewayClient ??
    ...
    async handleApprovalRequested(request) {
        ...
        const pluginRequest = isPluginApprovalRequest(request)
            ? request
            : null;
        logDebug(`discord exec approvals: received ${pluginRequest ? "plugin" : "exec"} request ${request.id}`);
        let container;
        if (pluginRequest) {
        ...
        const { rest, request: discordRequest } = (this.opts.__testing?.createDiscordClient ?? sendShared.createDiscordClient)({ token: this.opts.token, accountId: this.opts.accountId }, this.opts.cfg);
        ...
        const originatingChannelId = sessionKey && target === "dm" ? extractDiscordChannelId(sessionKey) : null;
        if (target === "dm" && originatingChannelId) {
            try {
                await discordRequest(() => rest.post(Routes.channelMessages(originatingChannelId), {
        ...
        if (sendToChannel) {
            ...
            if (channelId) {
                ...
                if (!sendToDm) {
                    logError(`discord exec approvals: target is "channel" but could not extract channel id from session key "${sessionKey ?? "(none)"}" — falling back to DM delivery for approval ${request.id}`);
                    fallbackToDm = true;
                }
                else {
    ...
    async handleApprovalResolved(resolved) {
        ...
        logDebug(`discord exec approvals: resolved ${cached.kind} ${resolved.id} with ${resolved.decision}`);
        const container = cached.kind === "plugin"
    ...
    async handleApprovalTimeout(approvalId, source) {
        ...
        logDebug(`discord exec approvals: timeout for ${cached.kind} ${approvalId} (${source ?? "default"})`);
        const container = cached.kind === "plugin"
    ...
    async resolveApproval(approvalId, decision) {
        ...
        const method = resolveApprovalKindFromId(approvalId) === "plugin"
