...
export function createDiscordGatewayReconnectController(params) {
    ...
    const reconnectStallWatchdog = createArmableStallWatchdog({
        ...
        onTimeout: () => {
            ...
            const at = Date.now();
            const error = new Error(`discord reconnect watchdog timeout after ${DISCORD_GATEWAY_RECONNECT_STALL_TIMEOUT_MS}ms`);
            params.pushStatus({
                connected: false,
                lastEventAt: at,
                lastDisconnect: {
        ...
        await new Promise((resolve, reject) => {
            ...
            drainTimeout = setTimeout(() => {
                ...
                if (!terminateStarted) {
                    ...
                    rejectClose(new Error(`discord gateway socket did not close within ${DISCORD_GATEWAY_DISCONNECT_DRAIN_TIMEOUT_MS}ms before reconnect`));
                    return;
                }
    ...
    const ensureStartupReady = async () => {
        ...
        if (initialReady === "timeout") {
            ...
            if (reconnected === "timeout") {
                const error = new Error(`discord gateway did not reach READY within ${DISCORD_GATEWAY_READY_TIMEOUT_MS}ms after a forced reconnect`);
