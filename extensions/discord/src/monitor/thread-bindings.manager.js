...
import { createThreadForBinding, createWebhookForChannel, findReusableWebhook, isDiscordThreadGoneError, isThreadArchived, maybeSendBindingMessage, resolveChannelIdForBinding, summarizeDiscordError, } from "./thread-bindings.discord-api.js";
import { resolveThreadBindingFarewellText, resolveThreadBindingThreadName, } from "./thread-bindings.messages.js";
import { BINDINGS_BY_THREAD_ID, forgetThreadBindingToken, getThreadBindingToken, MANAGERS_BY_ACCOUNT_ID, PERSIST_BY_ACCOUNT_ID, ensureBindingsLoaded, rememberThreadBindingToken, normalizeTargetKind, normalizeThreadBindingDurationMs, normalizeThreadId, rememberRecentUnboundWebhookEcho, removeBindingRecord, resolveBindingIdsForSession, resolveBindingRecordKey, resolveThreadBindingIdleTimeoutMs, resolveThreadBindingInactivityExpiresAt, resolveThreadBindingMaxAgeExpiresAt, resolveThreadBindingMaxAgeMs, resolveThreadBindingsPath, saveBindingsToDisk, setBindingRecord, THREAD_BINDING_TOUCH_PERSIST_MIN_INTERVAL_MS, shouldDefaultPersist, resetThreadBindingsForTests, } from "./thread-bindings.state.js";
import { DEFAULT_THREAD_BINDING_IDLE_TIMEOUT_MS, DEFAULT_THREAD_BINDING_MAX_AGE_MS, THREAD_BINDINGS_SWEEP_INTERVAL_MS, } from "./thread-bindings.types.js";
...
const SWEEPERS_BY_ACCOUNT_ID = new Map();
...
export function createThreadBindingManager(params = {}) {
    ...
    const manager = {
        ...
        touchThread: (touchParams) => {
            ...
            const now = Date.now();
            const at = typeof touchParams.at === "number" && Number.isFinite(touchParams.at)
    ...
    const sessionBindingAdapter = {
        ...
        bind: async (input) => {
            ...
            const threadName = typeof metadata.threadName === "string"
...
export function createNoopThreadBindingManager(accountId) {
    return createNoopManager(accountId);
}
export function getThreadBindingManager(accountId) {
