...
import { createDiscordFormModal, formatDiscordComponentEventText, parseDiscordComponentCustomIdForCarbon, parseDiscordModalCustomIdForCarbon, } from "../components.js";
import { editDiscordComponentMessage } from "../send.components.js";
import { AGENT_BUTTON_KEY, AGENT_SELECT_KEY, ackComponentInteraction, ensureAgentComponentInteractionAllowed, ensureComponentUserAllowed, ensureGuildComponentMemberAllowed, formatModalSubmissionText, mapSelectValues, parseAgentComponentData, parseDiscordComponentData, parseDiscordModalId, resolveAgentComponentRoute, resolveComponentCommandAuthorized, resolveDiscordChannelContext, resolveDiscordInteractionId, resolveInteractionContextWithDmAuth, resolveInteractionCustomId, resolveModalFieldValues, resolvePinnedMainDmOwnerFromAllowlist, } from "./agent-components-helpers.js";
import { normalizeDiscordAllowList, resolveDiscordChannelConfigWithFallback, resolveDiscordGuildEntry, } from "./allow-list.js";
...
async function dispatchPluginDiscordInteractiveEvent(params) {
    const normalizedConversationId = params.interactionCtx.rawGuildId || params.channelCtx.channelType === ChannelType.GroupDM
    ...
    if (pluginBindingApproval) {
        ...
        if (approvalMessageId) {
            try {
                await editDiscordComponentMessage(normalizedConversationId, approvalMessageId, {
...
async function handleDiscordComponentEvent(params) {
    ...
    const eventText = (consumed.kind === "button" ? consumed.callbackData?.trim() : undefined) ||
