import { Button, ChannelType, Container, Row, StringSelectMenu, TextDisplay, } from "@buape/carbon";
import { ButtonStyle } from "discord-api-types/v10";
import { resolveDefaultModelForAgent } from "openclaw/plugin-sdk/agent-runtime";
import { buildCommandTextFromArgs, findCommandByNativeName, listChatCommands, resolveStoredModelOverride, serializeCommandArgs, } from "openclaw/plugin-sdk/command-auth";
...
export async function handleDiscordModelPickerInteraction(params) {
    ...
    if (parsed.action === "submit" || parsed.action === "reset" || parsed.action === "quick") {
        ...
        const updateResult = await params.safeInteractionCall("model picker update", () => interaction.update(buildDiscordModelPickerNoticePayload(`Applying model change to ${resolvedModelRef}...`)));
...
export async function handleDiscordCommandArgInteraction(params) {
    ...
    const argUpdateResult = await params.safeInteractionCall("command arg update", () => interaction.update({
