...
import { Client, RateLimitError, } from "@buape/carbon";
...
import { isNativeCommandsExplicitlyDisabled, resolveNativeCommandsEnabled, resolveNativeSkillsEnabled, } from "openclaw/plugin-sdk/config-runtime";
import { loadConfig } from "openclaw/plugin-sdk/config-runtime";
import { GROUP_POLICY_BLOCKED_LABEL, resolveOpenProviderRuntimeGroupPolicy, resolveDefaultGroupPolicy, warnMissingProviderGroupPolicyFallbackOnce, } from "openclaw/plugin-sdk/config-runtime";
import { createConnectedChannelStatusPatch } from "openclaw/plugin-sdk/gateway-runtime";
import { getPluginCommandSpecs } from "openclaw/plugin-sdk/plugin-runtime";
import { resolveTextChunkLimit } from "openclaw/plugin-sdk/reply-runtime";
import { danger, isVerbose, logVerbose, shouldLogVerbose, warn, } from "openclaw/plugin-sdk/runtime-env";
...
import { createDiscordMonitorClient, fetchDiscordBotIdentity, registerDiscordMonitorListeners, } from "./provider.startup.js";
...
function normalizeBooleanForTesting(value) {
...
function resolveThreadBindingsEnabledForTesting(params) {
    return (normalizeBooleanForTesting(params.channelEnabledRaw) ??
...
function isLegacyMissingSessionError(message) {
    return (message.includes("Session is not ACP-enabled") ||
...
async function probeDiscordAcpBindingHealth(params) {
    ...
    const statusPromise = manager
        ...
        .then((status) => ({ kind: "status", status }))
    ...
    const runningForMs = params.storedState === "running" && Number.isFinite(params.lastActivityAt)
...
async function deployDiscordCommands(params) {
    ...
    const isDailyCreateLimit = (err) => err instanceof RateLimitError &&
    ...
    restClient.put = async (path, data, query) => {
        ...
        const body = data && typeof data === "object" && "body" in data
            ? data.body
            : undefined;
        ...
        if ((shouldLogVerboseForTesting ?? shouldLogVerbose)()) {
            params.runtime.log?.(`discord startup [${accountId}] deploy-rest:put:start ${Math.max(0, Date.now() - startupStartedAt)}ms path=${path}${typeof commandCount === "number" ? ` commands=${commandCount}` : ""}${typeof bodyBytes === "number" ? ` bytes=${bodyBytes}` : ""}`);
        }
        try {
            const result = await originalPut(path, data, query);
            if ((shouldLogVerboseForTesting ?? shouldLogVerbose)()) {
                params.runtime.log?.(`discord startup [${accountId}] deploy-rest:put:done ${Math.max(0, Date.now() - startupStartedAt)}ms path=${path} requestMs=${Date.now() - startedAt}`);
            }
            return result;
        }
        catch (err) {
    ...
    try {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
            try {
                ...
                if ((shouldLogVerboseForTesting ?? shouldLogVerbose)()) {
                    params.runtime.log?.(`discord startup [${accountId}] deploy-retry ${Math.max(0, Date.now() - startupStartedAt)}ms attempt=${attempt}/${maxAttempts - 1} retryAfterMs=${retryAfterMs} scope=${err.scope ?? "unknown"} code=${err.discordCode ?? "unknown"}`);
                }
...
function formatDiscordStartupGatewayState(gateway) {
    ...
    const reconnectAttempts = gateway
        .reconnectAttempts;
...
function readDiscordDeployRejectedFields(value) {
    if (Array.isArray(value)) {
        return value.filter((entry) => typeof entry === "string").slice(0, 6);
    }
...
export async function monitorDiscordProvider(opts = {}) {
    ...
    const applicationId = await (fetchDiscordApplicationIdForTesting ?? fetchDiscordApplicationId)(token, 4000, discordRestFetch);
    if (!applicationId) {
    ...
    if (nativeEnabled && nativeSkillsEnabled && commandSpecs.length > maxDiscordCommands) {
        skillCommands = [];
        commandSpecs = (listNativeCommandSpecsForConfigForTesting ?? listNativeCommandSpecsForConfig)(cfg, { skillCommands: [], provider: "discord" });
        commandSpecs = appendPluginCommandSpecs({ commandSpecs, runtime });
    ...
    if (threadBindingsEnabled) {
        const uncertainProbeKeys = new Set();
        ...
        if (reconciliation.removed > 0) {
            logVerbose(`discord: removed ${reconciliation.removed}/${reconciliation.checked} stale ACP thread bindings on startup for account ${account.accountId}: ${reconciliation.staleSessionKeys.join(", ")}`);
        }
        if (uncertainProbeKeys.size > 0) {
            logVerbose(`discord: ACP thread-binding health probe uncertain for account ${account.accountId}: ${[...uncertainProbeKeys].join(", ")}`);
        }
    }
