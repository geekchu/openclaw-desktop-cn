...
import { inspectDiscordSetupAccount, listDiscordSetupAccountIds, resolveDiscordSetupAccountConfig, } from "./setup-account-state.js";
import { createAccountScopedAllowFromSection, createAccountScopedGroupAccessSection, createAllowlistSetupWizardProxy, createLegacyCompatChannelDmPolicy, parseMentionOrPrefixedId, patchChannelConfigForAccount, setSetupChannelEnabled, } from "./setup-runtime-helpers.js";
...
export function setDiscordGuildChannelAllowlist(cfg, accountId, entries) {
...
export function parseDiscordAllowFromId(value) {
...
export const discordSetupAdapter = createEnvPatchedAccountSetupAdapter({
    channelKey: channel,
...
export function createDiscordSetupWizardBase(handlers) {
    ...
    return {
        ...
        groupAccess: createAccountScopedGroupAccessSection({
            ...
            currentPolicy: ({ cfg, accountId }) => resolveDiscordSetupAccountConfig({ cfg, accountId }).config.groupPolicy ?? "allowlist",
            currentEntries: ({ cfg, accountId }) => Object.entries(resolveDiscordSetupAccountConfig({ cfg, accountId }).config.guilds ?? {}).flatMap(([guildKey, value]) => {
            ...
            updatePrompt: ({ cfg, accountId }) => Boolean(resolveDiscordSetupAccountConfig({ cfg, accountId }).config.guilds),
...
export function createDiscordSetupWizardProxy(loadWizard) {
    return createAllowlistSetupWizardProxy({
        loadWizard,
        createBase: createDiscordSetupWizardBase,
        fallbackResolvedGroupAllowlist: (entries) => entries.map((input) => ({ input, resolved: false })),
    });
}
