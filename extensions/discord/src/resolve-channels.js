...
import { buildDiscordUnresolvedResults, filterDiscordGuilds, resolveDiscordAllowlistToken, } from "./resolve-allowlist-common.js";
...
export async function resolveDiscordChannelAllowlist(params) {
    ...
    for (const input of params.entries) {
        ...
        if (parsed.channelId) {
            ...
            if (result.status === "not-found" && parsed.guildId) {
                ...
                if (guild) {
                    ...
                    const matches = channels.filter((channel) => normalizeDiscordSlug(channel.name) === normalizeDiscordSlug(channelId));
        ...
        if (parsed.guildId || parsed.guild) {
            ...
            let matches = channels.filter((channel) => isNumericId
