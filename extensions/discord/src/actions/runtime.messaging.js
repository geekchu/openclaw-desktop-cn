...
import { assertMediaNotDataUrl, jsonResult, readNumberParam, readReactionParams, readStringArrayParam, readStringParam, resolvePollMaxSelections, withNormalizedTimestamp, readBooleanParam, } from "../runtime-api.js";
...
export async function handleDiscordMessagingAction(action, params, isActionEnabled, options, cfg) {
    const resolveChannelId = () => discordMessagingActionRuntime.resolveDiscordChannelId(readStringParam(params, "channelId", {
        required: true,
    }));
    ...
    switch (action) {
        case "react": {
            ...
            if (remove) {
                if (accountId) {
                    ...
                    await discordMessagingActionRuntime.removeReactionDiscord(channelId, messageId, emoji, cfgOptions);
                }
            ...
            if (isEmpty) {
                const removed = accountId
                    ...
                    : await discordMessagingActionRuntime.removeOwnReactionsDiscord(channelId, messageId, cfgOptions);
            ...
            if (accountId) {
                ...
                await discordMessagingActionRuntime.reactMessageDiscord(channelId, messageId, emoji, cfgOptions);
            }
        ...
        case "reactions": {
            ...
            const reactions = await discordMessagingActionRuntime.fetchReactionsDiscord(channelId, messageId, {
                ...cfgOptions,
                ...(accountId ? { accountId } : {}),
                limit,
            });
        ...
        case "poll": {
            ...
            await discordMessagingActionRuntime.sendPollDiscord(to, { question, options: answers, maxSelections, durationHours }, { ...cfgOptions, ...(accountId ? { accountId } : {}), content });
            return jsonResult({ ok: true });
        }
        case "permissions": {
        ...
        case "editMessage": {
            ...
            const message = accountId
                ? await discordMessagingActionRuntime.editMessageDiscord(channelId, messageId, { content }, { ...cfgOptions, accountId })
        ...
        case "threadReply": {
            ...
            const replyTo = readStringParam(params, "replyTo");
            const result = await discordMessagingActionRuntime.sendMessageDiscord(`channel:${channelId}`, content, {
                ...cfgOptions,
                ...(accountId ? { accountId } : {}),
                mediaUrl,
                mediaLocalRoots: options?.mediaLocalRoots,
