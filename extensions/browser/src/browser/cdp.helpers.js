...
export async function assertCdpEndpointAllowed(cdpUrl, ssrfPolicy) {
    if (!ssrfPolicy) {
        return;
    }
...
export function redactCdpUrl(cdpUrl) {
...
export function normalizeCdpHttpBaseForJsonEndpoints(cdpUrl) {
    try {
        const url = new URL(cdpUrl);
        if (url.protocol === "ws:") {
            url.protocol = "http:";
