...
export function registerBrowserAgentActDownloadRoutes(app, ctx) {
        ...
        await withRouteTabContext({
            ...
            run: async ({ profileCtx, cdpUrl, tab }) => {
                if (getBrowserProfileCapabilities(profileCtx.profile).usesChromeMcp) {
                    return jsonError(res, 501, "download waiting is not supported for existing-session profiles yet.");
                }
    ...
    app.post("/download", async (req, res) => {
        ...
        await withRouteTabContext({
            ...
            run: async ({ profileCtx, cdpUrl, tab }) => {
                if (getBrowserProfileCapabilities(profileCtx.profile).usesChromeMcp) {
                    return jsonError(res, 501, "downloads are not supported for existing-session profiles yet.");
                }
