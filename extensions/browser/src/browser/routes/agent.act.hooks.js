...
import { readBody, requirePwAi, resolveTargetIdFromBody, withRouteTabContext, } from "./agent.shared.js";
        ...
        await withRouteTabContext({
            ...
            run: async ({ profileCtx, cdpUrl, tab }) => {
                ...
                if (getBrowserProfileCapabilities(profileCtx.profile).usesChromeMcp) {
                    if (element) {
                        return jsonError(res, 501, "existing-session file uploads do not support element selectors; use ref/inputRef.");
                    }
                    if (resolvedPaths.length !== 1) {
                        return jsonError(res, 501, "existing-session file uploads currently support one file at a time.");
                    }
    ...
    app.post("/hooks/dialog", async (req, res) => {
        ...
        await withRouteTabContext({
            ...
            run: async ({ profileCtx, cdpUrl, tab }) => {
                if (getBrowserProfileCapabilities(profileCtx.profile).usesChromeMcp) {
                    if (timeoutMs) {
                        return jsonError(res, 501, "existing-session dialog handling does not support timeoutMs.");
                    }
