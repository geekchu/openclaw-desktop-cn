import { clickChromeMcpElement, closeChromeMcpTab, dragChromeMcpElement, evaluateChromeMcpScript, fillChromeMcpElement, fillChromeMcpForm, hoverChromeMcpElement, pressChromeMcpKey, resizeChromeMcpPage, } from "../chrome-mcp.js";
...
import { readBody, requirePwAi, resolveTargetIdFromBody, withRouteTabContext, SELECTOR_UNSUPPORTED_MESSAGE, } from "./agent.shared.js";
...
async function waitForExistingSessionCondition(params) {
    ...
    while (Date.now() < deadline) {
        let ready = true;
        if (predicate) {
            ready = Boolean(await evaluateChromeMcpScript({
...
export function registerBrowserAgentActRoutes(app, ctx) {
    app.post("/act", async (req, res) => {
        ...
        if ((kind === "evaluate" || (kind === "wait" && earlyFn)) &&
            !ctx.state().resolved.evaluateEnabled) {
            return jsonError(res, 403, browserEvaluateDisabledMessage(kind === "evaluate" ? "evaluate" : "wait"));
        }
                ...
                switch (kind) {
                    case "click": {
                        ...
                        if (isExistingSession) {
                            if (selector) {
                                return jsonError(res, 501, "existing-session click does not support selector targeting yet; use ref.");
                            }
                            if ((button && button !== "left") || (modifiers && modifiers.length > 0)) {
                                return jsonError(res, 501, "existing-session click currently supports left-click only (no button overrides/modifiers).");
                            }
                    ...
                    case "type": {
                        ...
                        if (isExistingSession) {
                            if (selector) {
                                return jsonError(res, 501, "existing-session type does not support selector targeting yet; use ref.");
                            }
                            if (slowly) {
                                return jsonError(res, 501, "existing-session type does not support slowly=true; use fill/press instead.");
                            }
                    ...
                    case "hover": {
                        ...
                        if (isExistingSession) {
                            if (selector) {
                                return jsonError(res, 501, "existing-session hover does not support selector targeting yet; use ref.");
                            }
                            if (timeoutMs) {
                                return jsonError(res, 501, "existing-session hover does not support timeoutMs overrides.");
                            }
                    ...
                    case "scrollIntoView": {
                        ...
                        if (isExistingSession) {
                            if (selector) {
                                return jsonError(res, 501, "existing-session scrollIntoView does not support selector targeting yet; use ref.");
                            }
                            if (timeoutMs) {
                                return jsonError(res, 501, "existing-session scrollIntoView does not support timeoutMs overrides.");
                            }
                    ...
                    case "drag": {
                        ...
                        if (isExistingSession) {
                            if (startSelector || endSelector) {
                                return jsonError(res, 501, "existing-session drag does not support selector targeting yet; use startRef/endRef.");
                            }
                            if (timeoutMs) {
                                return jsonError(res, 501, "existing-session drag does not support timeoutMs overrides.");
                            }
                    ...
                    case "select": {
                        ...
                        if (isExistingSession) {
                            if (selector) {
                                return jsonError(res, 501, "existing-session select does not support selector targeting yet; use ref.");
                            }
                            if (values.length !== 1) {
                                return jsonError(res, 501, "existing-session select currently supports a single value only.");
                            }
                            if (timeoutMs) {
                                return jsonError(res, 501, "existing-session select does not support timeoutMs overrides.");
                            }
                            await fillChromeMcpElement({
                                profileName,
                                userDataDir: profileCtx.profile.userDataDir,
                    ...
                    case "fill": {
                        ...
                        if (isExistingSession) {
                            if (timeoutMs) {
                                return jsonError(res, 501, "existing-session fill does not support timeoutMs overrides.");
                            }
                    ...
                    case "wait": {
                        ...
                        if (isExistingSession) {
                            if (loadState === "networkidle") {
                                return jsonError(res, 501, "existing-session wait does not support loadState=networkidle yet.");
                            }
                    ...
                    case "evaluate": {
                        ...
                        if (isExistingSession) {
                            if (evalTimeoutMs !== undefined) {
                                return jsonError(res, 501, "existing-session evaluate does not support timeoutMs overrides.");
                            }
                    ...
                    case "batch": {
                        if (isExistingSession) {
                            return jsonError(res, 501, "existing-session batch is not supported yet; send actions individually.");
                        }
                        const pw = await requirePwAi(res, `act:${kind}`);
                        if (!pw) {
                            return;
        ...
        await withRouteTabContext({
            ...
            run: async ({ profileCtx, cdpUrl, tab }) => {
                if (getBrowserProfileCapabilities(profileCtx.profile).usesChromeMcp) {
                    return jsonError(res, 501, "response body is not supported for existing-session profiles yet.");
                }
