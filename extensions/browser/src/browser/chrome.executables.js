...
function findFirstChromeExecutable(candidates) {
    for (const candidate of candidates) {
        if (exists(candidate)) {
            return {
                kind: candidate.toLowerCase().includes("sxs") || candidate.toLowerCase().includes("canary")
                    ? "canary"
                    : "chrome",
                path: candidate,
            };
...
export function findGoogleChromeExecutableMac() {
...
export function findGoogleChromeExecutableLinux() {
    return findFirstChromeExecutable([
...
export function findGoogleChromeExecutableWindows() {
    const localAppData = process.env.LOCALAPPDATA ?? "";
    const programFiles = process.env.ProgramFiles ?? "C:\\Program Files";
    const programFilesX86 = process.env["ProgramFiles(x86)"] ?? "C:\\Program Files (x86)";
    const joinWin = path.win32.join;
...
export function resolveGoogleChromeExecutableForPlatform(platform) {
    if (platform === "darwin") {
...
export function readBrowserVersion(executablePath) {
...
export function parseBrowserMajorVersion(rawVersion) {
