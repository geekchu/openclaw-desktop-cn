...
import { BrowserConflictError, BrowserProfileNotFoundError, BrowserResourceExhaustedError, BrowserValidationError, } from "./errors.js";
...
export function createBrowserProfilesService(ctx) {
    ...
    const createProfile = async (params) => {
        ...
        if (!isValidProfileName(name)) {
            throw new BrowserValidationError("invalid profile name: use lowercase letters, numbers, and hyphens only");
        }
        ...
        if (normalizedUserDataDir && driver !== "existing-session") {
            throw new BrowserValidationError("driver=existing-session is required when userDataDir is provided");
        }
        if (normalizedUserDataDir && !fs.existsSync(normalizedUserDataDir)) {
            throw new BrowserValidationError(`browser user data directory not found: ${normalizedUserDataDir}`);
        }
        if (rawCdpUrl) {
            let parsed;
            try {
            ...
            if (driver === "existing-session") {
                throw new BrowserValidationError("driver=existing-session does not accept cdpUrl; it attaches via the Chrome MCP auto-connect flow");
            }
            profileConfig = {
    ...
    const deleteProfile = async (nameRaw) => {
        ...
        if (name === defaultProfile) {
            throw new BrowserValidationError(`cannot delete the default profile "${name}"; change browser.defaultProfile first`);
        }
        if (!(name in profiles)) {
