...
const BROWSER_TOOL_MODEL_HINT = "Do NOT retry the browser tool — it will keep failing. " +
    "Use an alternative approach or inform the user that the browser is currently unavailable.";
const BROWSER_SERVICE_RATE_LIMIT_MESSAGE = "Browser service rate limit reached. " +
    "Wait for the current session to complete, or retry later.";
const BROWSERBASE_RATE_LIMIT_MESSAGE = "Browserbase rate limit reached (max concurrent sessions). " +
...
export function resolveBrowserRateLimitMessage(url) {
...
function enhanceBrowserFetchError(url, err, timeoutMs) {
    ...
    if (looksLikeTimeout) {
        return new Error(appendBrowserToolModelHint(`Can't reach the OpenClaw browser control service (timed out after ${timeoutMs}ms). ${operatorHint}`));
    }
