...
import { assertBrowserNavigationAllowed, assertBrowserNavigationResultAllowed, InvalidBrowserNavigationUrlError, requiresInspectableBrowserNavigationRedirects, withBrowserNavigationPolicy, } from "./navigation-guard.js";
...
export function createProfileTabOps({ profile, state, getProfileState, }) {
    ...
    const listTabs = async () => {
        ...
        const raw = await fetchJson(appendCdpPath(cdpHttpBase, "/json/list"));
        return raw
            .map((t) => ({
            targetId: t.id ?? "",
            title: t.title ?? "",
    ...
    const enforceManagedTabLimit = async (keepTargetId) => {
        ...
        if (!capabilities.supportsManagedTabLimit ||
    ...
    const openTab = async (url) => {
        ...
        if (requiresInspectableBrowserNavigationRedirects(state().resolved.ssrfPolicy)) {
            throw new InvalidBrowserNavigationUrlError("Navigation blocked: strict browser SSRF policy requires Playwright-backed redirect-hop inspection");
        }
