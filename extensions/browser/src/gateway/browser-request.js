...
export async function handleBrowserGatewayRequest({ params, respond, context, }) {
    const typed = params;
    ...
    if (isPersistentBrowserProfileMutation(methodRaw, path)) {
        respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "browser.request cannot mutate persistent browser profiles"));
        return;
    }
    ...
    if (nodeTarget) {
        ...
        if (!allowed.ok) {
            ...
            respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, hint, {
