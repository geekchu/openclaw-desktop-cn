...
function resolveInteractiveRepliesFromCapabilities(capabilities) {
    ...
    if (Array.isArray(capabilities)) {
        return capabilities.some((entry) => normalizeLowercaseStringOrEmpty(String(entry)) === "interactivereplies");
    }
...
export function compileSlackInteractiveReplies(payload) {
    ...
    for (const match of text.matchAll(SLACK_DIRECTIVE_RE)) {
        ...
        const block = normalizeLowercaseStringOrEmpty(directiveType) === "slack_buttons"
...
export function parseSlackOptionsLine(payload) {
    const text = payload.text;
