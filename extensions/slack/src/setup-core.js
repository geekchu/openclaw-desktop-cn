import { createAllowlistSetupWizardProxy, createAccountScopedAllowFromSection, createAccountScopedGroupAccessSection, createLegacyCompatChannelDmPolicy, createStandardChannelSetupStatus, DEFAULT_ACCOUNT_ID, createEnvPatchedAccountSetupAdapter, hasConfiguredSecretInput, parseMentionOrPrefixedId, patchChannelConfigForAccount, setSetupChannelEnabled, } from "openclaw/plugin-sdk/setup";
import { formatDocsLink } from "openclaw/plugin-sdk/setup-tools";
import { inspectSlackAccount } from "./account-inspect.js";
import { listSlackAccountIds, resolveSlackAccount } from "./accounts.js";
import { buildSlackSetupLines, isSlackSetupAccountConfigured, setSlackChannelAllowlist, SLACK_CHANNEL as channel, } from "./shared.js";
...
export const slackSetupAdapter = createEnvPatchedAccountSetupAdapter({
    channelKey: channel,
...
export function createSlackSetupWizardBase(handlers) {
    ...
    return {
        ...
        introNote: {
            ...
            shouldShow: ({ cfg, accountId }) => !isSlackSetupAccountConfigured(resolveSlackAccount({ cfg, accountId })),
        },
        envShortcut: {
        ...
        groupAccess: createAccountScopedGroupAccessSection({
            ...
            currentPolicy: ({ cfg, accountId }) => resolveSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
            currentEntries: ({ cfg, accountId }) => Object.entries(resolveSlackAccount({ cfg, accountId }).config.channels ?? {})
                .filter(([, value]) => value?.allow !== false && value?.enabled !== false)
                .map(([key]) => key),
            updatePrompt: ({ cfg, accountId }) => Boolean(resolveSlackAccount({ cfg, accountId }).config.channels),
...
export function createSlackSetupWizardProxy(loadWizard) {
