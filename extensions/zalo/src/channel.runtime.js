...
export async function sendZaloText(params) {
...
export async function startZaloGatewayAccount(ctx) {
    ...
    try {
        ...
        if (!probe.ok) {
            ctx.log?.warn?.(`[${account.accountId}] Zalo probe failed before provider start (${String(probe.elapsedMs)}ms): ${probe.error}`);
        }
        ctx.setStatus({
