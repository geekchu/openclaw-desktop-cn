...
export function buildXaiModelDefinition() {
    return toModelDefinition(XAI_MODEL_CATALOG.find((entry) => entry.id === XAI_DEFAULT_MODEL_ID) ?? {
...
export function buildXaiCatalogModels() {
    return XAI_MODEL_CATALOG.map((entry) => toModelDefinition(entry));
}
export function resolveXaiCatalogEntry(modelId) {
    ...
    if (lower.startsWith("grok-3-mini-fast") ||
