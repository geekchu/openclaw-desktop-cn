...
请根据图片内容进行回复。`);
                ...
                // ============ 语音消息处理：SILK → WAV ============
                log?.info(
                  `[qqbot:${account.accountId}] Voice attachment detected: ${att.filename}, converting SILK to WAV...`,
                );
                try {
                  ...
                  if (result) {
                    ...
                    log?.info(
                      `[qqbot:${account.accountId}] Voice converted: ${result.wavPath} (duration: ${durationStr})`,
                    );
...
- 发送时间：${timestamp}`);
                    ...
                    // SILK 解码失败，保留原始文件
                    log?.info(
                      `[qqbot:${account.accountId}] Voice file is not SILK format, keeping original: ${localPath}`,
                    );
                    voiceDescriptions.push(`
