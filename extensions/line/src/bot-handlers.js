...
import { resolveAllowlistProviderRuntimeGroupPolicy, resolveDefaultGroupPolicy, warnMissingProviderGroupPolicyFallbackOnce, } from "openclaw/plugin-sdk/config-runtime";
import { readChannelAllowFromStore, resolvePairingIdLabel, upsertChannelPairingRequest, } from "openclaw/plugin-sdk/conversation-runtime";
...
import { buildLineMessageContext, buildLinePostbackContext, getLineSourceInfo, } from "./bot-message-context.js";
...
function getLineReplayCandidate(event, context) {
    ...
    if (nowMs - cache.lastPruneAtMs >= LINE_WEBHOOK_REPLAY_PRUNE_INTERVAL_MS ||
...
async function shouldProcessLineEvent(event, context) {
    ...
    const storeAllowFrom = await readChannelAllowFromStore("line", undefined, account.accountId).catch(() => []);
    ...
    if (isGroup) {
        ...
        const senderGroupAccess = evaluateMatchedGroupAccessForPolicy({
            ...
            allowlistMatched: Boolean(senderId) &&
                isSenderAllowed({
