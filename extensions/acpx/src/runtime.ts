...
export class AcpxRuntime implements AcpRuntime {
  ...
  /**
   * Resolve the cwd for probe commands. Falls back to command directory or process.cwd()
   * when the configured cwd does not exist (common during cross-platform bundle builds).
   */
  private resolveProbeCwd(): string {
    if (directoryExists(this.config.cwd)) {
    ...
    if (
      ...
      if (directoryExists(commandDir)) {
        this.logger?.debug?.(
          `acpx probe cwd fallback: config cwd missing (${this.config.cwd}); using command dir ${commandDir}`,
        );
        return commandDir;
      }
    }
    ...
    this.logger?.debug?.(
      `acpx probe cwd fallback: config cwd missing (${this.config.cwd}); using process cwd ${fallback}`,
    );
    return fallback;
  }

  async probeAvailability(): Promise<void> {
    const probeCwd = this.resolveProbeCwd();
...
function directoryExists(dir: string | undefined): boolean {
  if (!dir) {
    return false;
  }
  try {
