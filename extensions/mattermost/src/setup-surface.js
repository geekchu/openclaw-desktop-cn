...
import { applySetupAccountConfigPatch, DEFAULT_ACCOUNT_ID, } from "./runtime-api.js";
...
export const mattermostSetupWizard = {
    channel,
    ...
    envShortcut: {
        ...
        isAvailable: ({ cfg, accountId }) => {
            ...
            return Boolean(process.env.MATTERMOST_BOT_TOKEN?.trim() &&
    ...
    textInputs: [
        {
            ...
            currentValue: ({ cfg, accountId }) => resolveMattermostAccountWithSecrets(cfg, accountId).baseUrl ??
                process.env.MATTERMOST_URL?.trim(),
            initialValue: ({ cfg, accountId }) => resolveMattermostAccountWithSecrets(cfg, accountId).baseUrl ??
                process.env.MATTERMOST_URL?.trim(),
            ...
            validate: ({ value }) => normalizeMattermostBaseUrl(value)
                ? undefined
