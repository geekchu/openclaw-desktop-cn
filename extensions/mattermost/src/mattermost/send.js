...
import { createMattermostClient, createMattermostDirectChannelWithRetry, createMattermostPost, fetchMattermostChannelByName, fetchMattermostMe, fetchMattermostUserByUsername, fetchMattermostUserTeams, normalizeMattermostBaseUrl, uploadMattermostFile, } from "./client.js";
import { buildButtonProps, resolveInteractionCallbackUrl, setInteractionSecret, } from "./interactions.js";
...
export function parseMattermostTarget(raw) {
...
async function resolveTargetChannelId(params) {
    ...
    const channel = await createMattermostDirectChannelWithRetry(client, [botUser.id, userId], {
        ...
        onRetry: (attempt, delayMs, error) => {
            ...
            if (params.logger) {
                params.logger.warn?.(`DM channel creation retry ${attempt} after ${delayMs}ms: ${error.message}`);
            }
        },
    });
...
export async function resolveMattermostSendChannelId(to, opts = {}) {
    return (await resolveMattermostSendContext(to, opts)).channelId;
}
export async function sendMessageMattermost(to, text, opts = {}) {
    const core = getCore();
    const logger = core.logging.getChildLogger({ module: "mattermost" });
