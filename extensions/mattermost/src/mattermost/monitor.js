...
import { createMattermostClient, fetchMattermostMe, normalizeMattermostBaseUrl, } from "./client.js";
import { computeInteractionCallbackUrl, createMattermostInteractionHandler, resolveInteractionCallbackPath, setInteractionCallbackUrl, setInteractionSecret, } from "./interactions.js";
import { buildMattermostAllowedModelRefs, parseMattermostModelPickerContext, renderMattermostModelsPickerView, renderMattermostProviderPickerView, resolveMattermostModelPickerCurrentModel, } from "./model-picker.js";
import { authorizeMattermostCommandInvocation, isMattermostSenderAllowed, normalizeMattermostAllowList, } from "./monitor-auth.js";
import { evaluateMattermostMentionGate, mapMattermostChannelTypeToChatType, } from "./monitor-gating.js";
import { createDedupeCache, formatInboundFromLabel, normalizeMention, resolveThreadSessionKeys, } from "./monitor-helpers.js";
...
export function resolveMattermostReactionChannelId(payload) {
...
export async function monitorMattermostProvider(opts = {}) {
    ...
    if (!botToken) {
        throw new Error(`Mattermost bot token missing for account "${account.accountId}" (set channels.mattermost.accounts.${account.accountId}.botToken or MATTERMOST_BOT_TOKEN for default).`);
    }
    const baseUrl = normalizeMattermostBaseUrl(opts.baseUrl ?? account.baseUrl);
    if (!baseUrl) {
        throw new Error(`Mattermost baseUrl missing for account "${account.accountId}" (set channels.mattermost.accounts.${account.accountId}.baseUrl or MATTERMOST_URL for default).`);
    }
    ...
    try {
        ...
        if (isLoopbackHost(callbackHost) && !isLoopbackHost(mmHost)) {
            runtime.error?.(`mattermost: interactions callbackUrl resolved to ${callbackUrl} (loopback) while baseUrl is ${baseUrl}. This MAY be unreachable depending on your deployment. If button clicks don't work, set channels.mattermost.interactions.callbackBaseUrl to a URL reachable from the Mattermost server (e.g. your public reverse proxy URL).`);
        }
        if (!isLoopbackHost(callbackHost) && allowedInteractionSourceIps.length === 0) {
            runtime.error?.(`mattermost: interactions callbackUrl resolved to ${callbackUrl} without channels.mattermost.interactions.allowedSourceIps. For safety, non-loopback callback sources will be rejected until you allowlist the Mattermost server or trusted ingress IPs.`);
        }
    }
    catch {
    ...
    const runModelPickerCommand = async (params) => {
        ...
        const fromLabel = params.kind === "direct"
        ...
        const ctxPayload = core.channel.reply.finalizeInboundContext({
            ...
            GroupSubject: params.kind !== "direct" ? params.channelDisplay || params.roomLabel : undefined,
    ...
    async function handleModelPickerInteraction(params) {
        ...
        const pickerCommandText = pickerState.action === "select"
        ...
        if (!auth.ok) {
            ...
            const denyText = auth.denyReason === "unknown-channel"
    ...
    const handlePost = async (post, payload, messageIds) => {
        ...
        const chatType = channelChatType(kind);
        const senderName = payload.data?.sender_name?.trim() ||
        ...
        const inboundLabel = kind === "direct"
    ...
    const handleReactionEvent = async (payload) => {
        ...
        if (reactionAccess.decision !== "allow") {
            if (kind === "direct") {
                logVerboseMessage(`mattermost: drop reaction (dmPolicy=${dmPolicy} sender=${userId} reason=${reactionAccess.reason})`);
            }
            else {
                logVerboseMessage(`mattermost: drop reaction (groupPolicy=${groupPolicy} sender=${userId} reason=${reactionAccess.reason} channel=${channelId})`);
            }
            return;
        }
        const teamId = channelInfo?.team_id ?? undefined;
    ...
    const debouncer = core.channel.debounce.createInboundDebouncer({
        ...
        buildKey: (entry) => {
            const channelId = entry.post.channel_id ??
