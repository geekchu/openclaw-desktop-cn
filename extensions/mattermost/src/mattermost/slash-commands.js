 ...
 */
export const DEFAULT_COMMAND_SPECS = [
    {
        trigger: "oc_status",
        originalName: "status",
 ...
 */
export async function listMattermostCommands(client, teamId) {
 ...
 */
export async function createMattermostCommand(client, params) {
 ...
 */
export async function deleteMattermostCommand(client, commandId) {
 ...
 */
export async function updateMattermostCommand(client, params) {
...
export async function registerSlashCommands(params) {
    ...
    for (const spec of commands) {
        ...
        if (ownedCommands.length > 1) {
            log?.(`mattermost: multiple owned commands found for /${spec.trigger}; using the first and leaving extras untouched`);
        }
 ...
 */
export function parseSlashCommandPayload(body, contentType) {
    if (!body) {
        return null;
    }
    try {
 ...
 */
export function resolveCommandText(trigger, text, triggerMap) {
...
export function resolveSlashCommandConfig(raw) {
    return {
...
export function isSlashCommandsEnabled(config) {
...
export function resolveCallbackUrl(params) {
    ...
    let host = params.gatewayHost && !isWildcardBindHost(params.gatewayHost)
        ? params.gatewayHost
        : "localhost";
