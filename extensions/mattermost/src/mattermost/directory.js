 ...
 */
export async function listMattermostDirectoryGroups(params) {
 ...
 */
export async function listMattermostDirectoryPeers(params) {
    const clients = buildClients(params);
    if (!clients.length) {
        return [];
    }
    ...
    try {
        ...
        if (q) {
            ...
            const members = await client.request(`/teams/${teamId}/members?per_page=200`);
