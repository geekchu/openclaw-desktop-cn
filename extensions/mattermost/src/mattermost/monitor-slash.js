import { fetchMattermostUserTeams, normalizeMattermostBaseUrl, } from "./client.js";
import { listSkillCommandsForAgents, parseStrictPositiveInteger, } from "./runtime-api.js";
import { DEFAULT_COMMAND_SPECS, isSlashCommandsEnabled, registerSlashCommands, resolveCallbackUrl, resolveSlashCommandConfig, } from "./slash-commands.js";
...
function warnOnSuspiciousCallbackUrl(params) {
    try {
        ...
        if (isLoopbackHost(callbackHost) && !isLoopbackHost(mmHost)) {
            params.runtime.error?.(`mattermost: slash commands callbackUrl resolved to ${params.callbackUrl} (loopback) while baseUrl is ${params.baseUrl}. This MAY be unreachable depending on your deployment. If native slash commands don't work, set channels.mattermost.commands.callbackUrl to a URL reachable from the Mattermost server (e.g. your public reverse proxy URL).`);
        }
    }
    catch {
...
export async function registerMattermostMonitorSlashCommands(params) {
    ...
    try {
        ...
        if (teamRegistrationFailures > 0) {
            params.runtime.error?.(`mattermost: slash command registration completed with ${teamRegistrationFailures} team error(s)`);
        }
