...
import { listMattermostAccountIds, resolveDefaultMattermostAccountId, resolveMattermostAccount, resolveMattermostReplyToMode, } from "./mattermost/accounts.js";
...
import { chunkTextForOutbound, createAccountStatusSink, DEFAULT_ACCOUNT_ID, } from "./runtime-api.js";
...
function describeMattermostMessageTool({ cfg, }) {
    ...
    return {
        ...
        schema: enabledAccounts.length > 0
            ? {
                properties: {
...
const mattermostMessageActions = {
    ...
    handleAction: async ({ action, params, cfg, accountId }) => {
        if (action === "react") {
            ...
            const reactionsEnabled = account.config.actions?.reactions ?? mattermostConfig?.actions?.reactions ?? true;
            if (!reactionsEnabled) {
...
export const mattermostPlugin = createChatChannelPlugin({
    base: {
        id: "mattermost",
        meta: {
            ...meta,
    ...
    threading: {
        scopedAccountReplyToMode: {
            resolveAccount: (cfg, accountId) => resolveMattermostAccount({ cfg, accountId: accountId ?? "default" }),
            resolveReplyToMode: (account, chatType) => resolveMattermostReplyToMode(account, chatType === "direct" || chatType === "group" || chatType === "channel"
                ? chatType
                : "channel"),
        },
    },
