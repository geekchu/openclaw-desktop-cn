...
export async function promptSignalAllowFrom(params) {
    return promptParsedAllowFromForAccount({
        ...
        getExistingAllowFrom: ({ cfg, accountId }) => resolveSignalAccount({ cfg, accountId }).config.allowFrom ?? [],
...
export function createSignalCliPathTextInput(shouldPrompt) {
    return createCliPathTextInput({
        inputKey: "cliPath",
        message: "signal-cli path",
        resolvePath: ({ cfg, accountId, credentialValues }) => resolveSignalCliPath({ cfg, accountId, credentialValues }),
        shouldPrompt,
        helpTitle: "Signal",
        helpLines: [
...
export const signalNumberTextInput = {
    inputKey: "signalNumber",
    message: "Signal bot number (E.164)",
    currentValue: ({ cfg, accountId }) => normalizeSignalAccountInput(resolveSignalAccount({ cfg, accountId }).config.account) ??
        undefined,
    keepPrompt: (value) => `Signal account set (${value}). Keep it?`,
    validate: ({ value }) => normalizeSignalAccountInput(value) ? undefined : INVALID_SIGNAL_ACCOUNT_ERROR,
...
export const signalSetupAdapter = createPatchedAccountSetupAdapter({
    channelKey: channel,
...
export function createSignalSetupWizardProxy(loadWizard) {
    return createDelegatedSetupWizardProxy({
        ...
        credentials: [],
        textInputs: [
            createSignalCliPathTextInput(createDelegatedTextInputShouldPrompt({
                loadWizard,
                inputKey: "cliPath",
            })),
