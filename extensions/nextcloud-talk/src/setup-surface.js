...
import { createStandardChannelSetupStatus, formatDocsLink, setSetupChannelEnabled, } from "openclaw/plugin-sdk/setup";
import { listNextcloudTalkAccountIds, resolveNextcloudTalkAccount } from "./accounts.js";
import { clearNextcloudTalkAccountFields, nextcloudTalkDmPolicy, nextcloudTalkSetupAdapter, normalizeNextcloudTalkBaseUrl, setNextcloudTalkAccountConfig, validateNextcloudTalkBaseUrl, } from "./setup-core.js";
const channel = "nextcloud-talk";
const CONFIGURE_API_FLAG = "__nextcloudTalkConfigureApiCredentials";
export const nextcloudTalkSetupWizard = {
    channel,
    ...
    status: createStandardChannelSetupStatus({
        ...
        resolveConfigured: ({ cfg }) => listNextcloudTalkAccountIds(cfg).some((accountId) => {
            const account = resolveNextcloudTalkAccount({ cfg: cfg, accountId });
    ...
    prepare: async ({ cfg, accountId, credentialValues, prompter }) => {
        const resolvedAccount = resolveNextcloudTalkAccount({ cfg: cfg, accountId });
        const hasApiCredentials = Boolean(resolvedAccount.config.apiUser?.trim() &&
    ...
    credentials: [
        {
            ...
            inspect: ({ cfg, accountId }) => {
                ...
                return {
                    ...
                    hasConfiguredValue: Boolean(hasConfiguredSecretInput(resolvedAccount.config.botSecret) ||
            ...
            applyUseEnv: async (params) => {
                ...
                const cleared = clearNextcloudTalkAccountFields(params.cfg, params.accountId, ["botSecret", "botSecretFile"]);
                return setNextcloudTalkAccountConfig(cleared, params.accountId, {
                    baseUrl: resolvedAccount.baseUrl,
                });
            },
            applySet: async (params) => setNextcloudTalkAccountConfig(clearNextcloudTalkAccountFields(params.cfg, params.accountId, [
                "botSecret",
                "botSecretFile",
            ]), params.accountId, {
        ...
        },
        {
            ...
            inspect: ({ cfg, accountId }) => {
                ...
                const apiPasswordConfigured = Boolean(hasConfiguredSecretInput(resolvedAccount.config.apiPassword) ||
                    resolvedAccount.config.apiPasswordFile);
                return {
    ...
    textInputs: [
        {
            ...
            currentValue: ({ cfg, accountId }) => resolveNextcloudTalkAccount({ cfg: cfg, accountId }).baseUrl || undefined,
            ...
            applySet: async (params) => setNextcloudTalkAccountConfig(params.cfg, params.accountId, {
        ...
        },
        {
            ...
            currentValue: ({ cfg, accountId }) => resolveNextcloudTalkAccount({ cfg: cfg, accountId }).config.apiUser?.trim() ||
                undefined,
            shouldPrompt: ({ credentialValues }) => credentialValues[CONFIGURE_API_FLAG] === "1",
