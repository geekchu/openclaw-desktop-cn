...
import { createSetupInputPresenceValidator, mergeAllowFromEntries, createTopLevelChannelDmPolicy, promptParsedAllowFromForAccount, resolveSetupAccountId, } from "openclaw/plugin-sdk/setup-runtime";
...
export function normalizeNextcloudTalkBaseUrl(value) {
    return value?.trim().replace(/\/+$/, "") ?? "";
}
export function validateNextcloudTalkBaseUrl(value) {
    if (!value) {
        return "Required";
    }
...
export function setNextcloudTalkAccountConfig(cfg, accountId, updates) {
...
export function clearNextcloudTalkAccountFields(cfg, accountId, fields) {
...
async function promptNextcloudTalkAllowFrom(params) {
    return await promptParsedAllowFromForAccount({
        ...
        getExistingAllowFrom: ({ cfg, accountId }) => resolveNextcloudTalkAccount({ cfg, accountId }).config.allowFrom ?? [],
...
export const nextcloudTalkDmPolicy = createTopLevelChannelDmPolicy({
...
export const nextcloudTalkSetupAdapter = {
    ...
    applyAccountConfig: ({ cfg, accountId, input }) => {
        ...
        const next = setupInput.useEnv
            ? clearNextcloudTalkAccountFields(namedConfig, accountId, [
                "botSecret",
                "botSecretFile",
            ])
            : namedConfig;
        ...
        return setNextcloudTalkAccountConfig(next, accountId, patch);
    },
};
