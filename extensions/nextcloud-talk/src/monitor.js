...
import { resolveLoggerBackedRuntime, safeParseJsonWithSchema, } from "openclaw/plugin-sdk/extension-shared";
import { z } from "zod";
import { WEBHOOK_RATE_LIMIT_DEFAULTS, createAuthRateLimiter, isRequestBodyLimitError, readRequestBodyWithLimit, requestBodyErrorToText, } from "../runtime-api.js";
...
export function readNextcloudTalkWebhookBody(req, maxBodyBytes) {
...
export function createNextcloudTalkWebhookServer(opts) {
    ...
    const maxBodyBytes = typeof opts.maxBodyBytes === "number" &&
...
export async function monitorNextcloudTalkProvider(opts) {
    ...
    const publicUrl = account.config.webhookPublicUrl ??
