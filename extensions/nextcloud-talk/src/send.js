...
function resolveCredentials(explicit, account) {
    ...
    if (!baseUrl) {
        throw new Error(`Nextcloud Talk baseUrl missing for account "${account.accountId}" (set channels.nextcloud-talk.baseUrl).`);
    }
    if (!secret) {
        throw new Error(`Nextcloud Talk bot secret missing for account "${account.accountId}" (set channels.nextcloud-talk.botSecret/botSecretFile or NEXTCLOUD_TALK_BOT_SECRET for default).`);
    }
...
export async function sendMessageNextcloudTalk(to, text, opts = {}) {
...
export async function sendReactionNextcloudTalk(roomToken, messageId, reaction, opts = {}) {
