...
import { createAllowFromSection, createPromptParsedAllowFromForAccount, createStandardChannelSetupStatus, setSetupChannelEnabled, } from "openclaw/plugin-sdk/setup";
import { formatDocsLink } from "openclaw/plugin-sdk/setup";
import { listIrcAccountIds, resolveDefaultIrcAccountId, resolveIrcAccount } from "./accounts.js";
import { isChannelTarget, normalizeIrcAllowEntry, normalizeIrcMessagingTarget, } from "./normalize.js";
import { ircSetupAdapter, parsePort, setIrcAllowFrom, setIrcDmPolicy, setIrcGroupAccess, setIrcNickServ, updateIrcAccountConfig, } from "./setup-core.js";
const channel = "irc";
...
async function promptIrcNickServConfig(params) {
    ...
    const useEnvPassword = params.accountId === DEFAULT_ACCOUNT_ID &&
    ...
    const registerEmail = register
        ? String(await params.prompter.text({
            ...
            initialValue: existing?.registerEmail ||
...
export const ircSetupWizard = {
    ...
    status: createStandardChannelSetupStatus({
        ...
        resolveConfigured: ({ cfg }) => listIrcAccountIds(cfg).some((accountId) => resolveIrcAccount({ cfg: cfg, accountId }).configured),
    }),
    introNote: {
        title: "IRC setup",
        lines: [
        ...
        shouldShow: ({ cfg, accountId }) => !resolveIrcAccount({ cfg: cfg, accountId }).configured,
    },
    prepare: async ({ cfg, accountId, credentialValues, prompter }) => {
        const resolved = resolveIrcAccount({ cfg: cfg, accountId });
    ...
    textInputs: [
        {
            ...
            currentValue: ({ cfg, accountId }) => resolveIrcAccount({ cfg: cfg, accountId }).config.host || undefined,
        ...
        },
        {
            ...
            currentValue: ({ cfg, accountId }) => String(resolveIrcAccount({ cfg: cfg, accountId }).config.port ?? ""),
            shouldPrompt: ({ credentialValues }) => credentialValues[USE_ENV_FLAG] !== "1",
        ...
        },
        {
            inputKey: "token",
            message: "IRC nick",
            currentValue: ({ cfg, accountId }) => resolveIrcAccount({ cfg: cfg, accountId }).config.nick || undefined,
            shouldPrompt: ({ credentialValues }) => credentialValues[USE_ENV_FLAG] !== "1",
            validate: ({ value }) => (String(value ?? "").trim() ? undefined : "Required"),
            normalizeValue: ({ value }) => String(value).trim(),
            applySet: async ({ cfg, accountId, value }) => updateIrcAccountConfig(cfg, accountId, {
        ...
        },
        {
            ...
            currentValue: ({ cfg, accountId }) => resolveIrcAccount({ cfg: cfg, accountId }).config.username || undefined,
            shouldPrompt: ({ credentialValues }) => credentialValues[USE_ENV_FLAG] !== "1",
            initialValue: ({ cfg, accountId, credentialValues }) => resolveIrcAccount({ cfg: cfg, accountId }).config.username ||
        ...
        },
        {
            ...
            currentValue: ({ cfg, accountId }) => resolveIrcAccount({ cfg: cfg, accountId }).config.realname || undefined,
            shouldPrompt: ({ credentialValues }) => credentialValues[USE_ENV_FLAG] !== "1",
            initialValue: ({ cfg, accountId }) => resolveIrcAccount({ cfg: cfg, accountId }).config.realname || "OpenClaw",
            validate: ({ value }) => (String(value ?? "").trim() ? undefined : "Required"),
            normalizeValue: ({ value }) => String(value).trim(),
            applySet: async ({ cfg, accountId, value }) => updateIrcAccountConfig(cfg, accountId, {
                enabled: true,
        ...
        },
        {
            ...
            currentValue: ({ cfg, accountId }) => resolveIrcAccount({ cfg: cfg, accountId }).config.channels?.join(", "),
            shouldPrompt: ({ credentialValues }) => credentialValues[USE_ENV_FLAG] !== "1",
            normalizeValue: ({ value }) => parseListInput(String(value))
    ...
    groupAccess: {
        label: "IRC channels",
        placeholder: "#openclaw, #ops, *",
        currentPolicy: ({ cfg, accountId }) => resolveIrcAccount({ cfg: cfg, accountId }).config.groupPolicy ?? "allowlist",
        currentEntries: ({ cfg, accountId }) => Object.keys(resolveIrcAccount({ cfg: cfg, accountId }).config.groups ?? {}),
        updatePrompt: ({ cfg, accountId }) => Boolean(resolveIrcAccount({ cfg: cfg, accountId }).config.groups),
        ...
        if (resolvedAfterGroups.config.groupPolicy === "allowlist") {
            ...
            if (groupKeys.length > 0) {
                ...
                if (!wantsMentions) {
                    ...
                    const patched = Object.fromEntries(Object.entries(groups).map(([key, value]) => [
                        key,
