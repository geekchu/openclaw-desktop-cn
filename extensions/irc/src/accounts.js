...
function resolveNickServConfig(accountId, nickserv) {
    ...
    const envRegisterEmail = accountId === DEFAULT_ACCOUNT_ID ? process.env.IRC_NICKSERV_REGISTER_EMAIL?.trim() : undefined;
...
export function resolveIrcAccount(params) {
    ...
    const resolve = (accountId) => {
        ...
        const tls = typeof merged.tls === "boolean"
            ? merged.tls
        ...
        const username = (merged.username?.trim() ||
            ...
            nick ||
            "openclaw").trim();
        const realname = (merged.realname?.trim() ||
...
export function listEnabledIrcAccounts(cfg) {
