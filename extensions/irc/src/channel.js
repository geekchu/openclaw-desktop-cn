...
import { composeAccountWarningCollectors, createAllowlistProviderOpenWarningCollector, } from "openclaw/plugin-sdk/channel-policy";
...
import { normalizeIrcMessagingTarget, looksLikeIrcTargetId, isChannelTarget, normalizeIrcAllowEntry, } from "./normalize.js";
import { resolveIrcGroupMatch, resolveIrcRequireMention } from "./policy.js";
import { probeIrc } from "./probe.js";
import { buildBaseChannelStatusSummary, createAccountStatusSink, chunkTextForOutbound, DEFAULT_ACCOUNT_ID, getChatChannelMeta, PAIRING_APPROVED_MESSAGE, } from "./runtime-api.js";
...
const collectIrcSecurityWarnings = composeAccountWarningCollectors(collectIrcGroupPolicyWarnings, (account) => !account.config.tls &&
    "- IRC TLS is disabled (channels.irc.tls=false); traffic and credentials are plaintext.", (account) => account.config.nickserv?.register &&
    '- IRC NickServ registration is enabled (channels.irc.nickserv.register=true); this sends "REGISTER" on every connect. Disable after first successful registration.', (account) => account.config.nickserv?.register &&
...
export const ircPlugin = createChatChannelPlugin({
    base: {
        id: "irc",
        meta: {
            ...meta,
        ...
        groups: {
            resolveRequireMention: ({ cfg, accountId, groupId }) => {
                const account = resolveIrcAccount({ cfg: cfg, accountId });
                if (!groupId) {
                    return true;
                }
