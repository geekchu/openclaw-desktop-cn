...
export function normalizeIrcMessagingTarget(raw) {
...
export function looksLikeIrcTargetId(raw) {
    const trimmed = raw.trim();
    if (!trimmed) {
        return false;
    }
...
export function normalizeIrcAllowEntry(raw) {
...
export function normalizeIrcAllowlist(entries) {
    return (entries ?? []).map((entry) => normalizeIrcAllowEntry(String(entry))).filter(Boolean);
}
export function formatIrcSenderId(message) {
...
export function buildIrcAllowlistCandidates(message, params) {
