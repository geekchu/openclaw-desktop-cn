...
function toPersistedSyncData(value) {
    ...
    if (typeof value.next_batch === "string" && value.next_batch.trim()) {
        ...
        return {
            ...
            accountData: isRecord(value.account_data) && Array.isArray(value.account_data.events)
...
export class FileBackedMatrixSyncStore extends MemoryStore {
    storagePath;
    persistLock = createAsyncLock();
    accumulator = new SyncAccumulator();
    savedSync = null;
    savedClientOptions;
    hadSavedSyncOnLoad;
    hadCleanShutdownOnLoad;
    cleanShutdown = false;
    dirty = false;
    persistTimer = null;
    persistPromise = null;
    ...
    getClientOptions() {
        return Promise.resolve(this.savedClientOptions ? cloneJson(this.savedClientOptions) : undefined);
    }
