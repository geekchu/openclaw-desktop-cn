...
function assertLegacyMigrationAccountSelection(params) {
    ...
    if (requiresExplicitMatrixDefaultAccount(cfg)) {
        throw new Error("Legacy Matrix client storage cannot be migrated automatically because multiple Matrix accounts are configured and channels.matrix.defaultAccount is not set.");
    }
    ...
    if (selectedAccountId !== currentAccountId) {
        throw new Error(`Legacy Matrix client storage targets account "${selectedAccountId}", but the current client is starting account "${currentAccountId}". Start the selected account first so flat legacy storage is not migrated into the wrong account directory.`);
    }
}
...
function isCompatibleStorageRoot(params) {
    ...
    if (params.requireExplicitDeviceMatch &&
        params.deviceId &&
...
export async function maybeMigrateLegacyStorage(params) {
    ...
    try {
        if (shouldMigrateStorage) {
            ...
            skippedExistingTargets.push(`- sync store remains at ${legacy.storagePath} because ${params.storagePaths.storagePath} already exists`);
        }
        if (shouldMigrateCrypto) {
            ...
            skippedExistingTargets.push(`- crypto store remains at ${legacy.cryptoPath} because ${params.storagePaths.cryptoPath} already exists`);
        }
    }
    catch (err) {
    ...
    if (skippedExistingTargets.length > 0) {
        logger.warn?.(`matrix: legacy client storage still exists in the flat path because some account-scoped targets already existed.\n${skippedExistingTargets.join("\n")}`);
    }
}
function moveLegacyStoragePathOrThrow(params) {
    ...
    if (fs.existsSync(params.targetPath)) {
        throw new Error(`legacy Matrix ${params.label} target already exists (${params.targetPath}); refusing to overwrite it automatically`);
    }
