...
import { resolveMediaMaxBytes, withResolvedMatrixControlClient, withResolvedMatrixSendClient, } from "./send/client.js";
import { buildReplyRelation, buildTextContent, buildThreadRelation, diffMatrixMentions, enrichMatrixFormattedContent, extractMatrixMentions, resolveMatrixMentionsForBody, resolveMatrixMsgType, resolveMatrixVoiceDecision, } from "./send/formatting.js";
import { buildMediaContent, prepareImageInfo, resolveMediaDurationMs, uploadMediaMaybeEncrypted, } from "./send/media.js";
...
export function prepareMatrixSingleText(text, opts = {}) {
    const trimmedText = text.trim();
    const cfg = opts.cfg ?? getCore().config.loadConfig();
    const tableMode = opts.tableMode ??
...
export function chunkMatrixText(text, opts = {}) {
...
export async function sendPollMatrix(to, poll, opts = {}) {
    ...
    return await withResolvedMatrixSendClient({
        ...
        const fallbackText = pollContent["m.text"] ?? pollContent["org.matrix.msc1767.text"] ?? poll.question ?? "";
...
export async function sendSingleTextMessageMatrix(roomId, text, opts = {}) {
    ...
    if (!fitsInSingleEvent) {
        throw new Error(`Matrix single-message text exceeds limit (${convertedText.length} > ${singleEventLimit})`);
    }
    return await withResolvedMatrixSendClient({
        client: opts.client,
        cfg: opts.cfg,
        ...
        const relation = normalizedThreadId
            ...
            : buildReplyRelation(opts.replyToId);
        const content = withMatrixExtraContentFields(buildTextContent(convertedText, relation, {
...
export async function editMessageMatrix(roomId, originalEventId, newText, opts = {}) {
    return await withResolvedMatrixSendClient({
        client: opts.client,
        cfg: opts.cfg,
        accountId: opts.accountId,
        ...
        const newContent = withMatrixExtraContentFields(buildTextContent(convertedText, undefined, {
            msgtype: opts.msgtype,
        }), opts.extraContent);
        await enrichMatrixFormattedContent({
            client,
        ...
        const replaceMentions = opts.includeMentions === false
            ? undefined
            : diffMatrixMentions(extractMatrixMentions(newContent), await resolvePreviousEditMentions({
                client,
...
export async function reactMatrixMessage(roomId, messageId, emoji, opts) {
