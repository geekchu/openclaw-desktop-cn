import { buildTimeoutAbortSignal, closeDispatcher, createPinnedDispatcher, resolvePinnedHostnameWithPolicy, } from "../../runtime-api.js";
...
export async function performMatrixRequest(params) {
    ...
    if (isAbsoluteEndpoint && params.allowAbsoluteEndpoint !== true) {
        throw new Error(`Absolute Matrix endpoint is blocked by default: ${params.endpoint}. Set allowAbsoluteEndpoint=true to opt in.`);
    }
        ...
        if (params.raw) {
            ...
            if (params.maxBytes && contentLength) {
                ...
                if (Number.isFinite(length) && length > params.maxBytes) {
                    throw new Error(`Matrix media exceeds configured size limit (${length} bytes > ${params.maxBytes} bytes)`);
                }
            }
            const bytes = params.maxBytes
                ? await readResponseWithLimit(response, params.maxBytes, {
                    onOverflow: ({ maxBytes, size }) => new Error(`Matrix media exceeds configured size limit (${size} bytes > ${maxBytes} bytes)`),
