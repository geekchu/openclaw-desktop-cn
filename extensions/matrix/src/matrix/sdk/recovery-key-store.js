...
export function isRepairableSecretStorageAccessError(err) {
...
export class MatrixRecoveryKeyStore {
    recoveryKeyPath;
    secretStorageKeyCache = new Map();
    stagedRecoveryKey = null;
    stagedCacheKeyIds = new Set();
            ...
            getSecretStorageKey: async ({ keys }) => {
                ...
                if (staged?.privateKeyBase64) {
                    ...
                    if (privateKey.length > 0) {
                        const stagedKeyId = staged.keyId && requestedKeyIds.includes(staged.keyId)
                            ? staged.keyId
                            : requestedKeyIds[0];
                        if (stagedKeyId) {
    ...
    async bootstrapSecretStorageWithRecoveryKey(crypto, options = {}) {
        ...
        const ensureRecoveryKey = async () => {
            ...
            if (typeof crypto.createRecoveryKeyFromPassphrase !== "function") {
                throw new Error("Matrix crypto backend does not support recovery key generation (createRecoveryKeyFromPassphrase missing)");
            }
        ...
        const shouldRecreateSecretStorage = options.forceNewSecretStorage === true ||
        ...
        try {
            ...
            const shouldRecreateWithoutRecoveryKey = options.allowSecretStorageRecreateWithoutRecoveryKey === true &&
                hasDefaultSecretStorageKey &&
        ...
        if (generatedRecoveryKey && this.recoveryKeyPath) {
            LogService.warn("MatrixClientLite", `Generated Matrix recovery key and saved it to ${this.recoveryKeyPath}. Keep this file secure.`);
        }
    }
    ...
    loadStoredRecoveryKey() {
        ...
        try {
            ...
            return {
                ...
                encodedPrivateKey: typeof parsed.encodedPrivateKey === "string" ? parsed.encodedPrivateKey : undefined,
