...
import { ClientEvent, MatrixEventEvent, Preset, createClient as createMatrixJsClient, } from "matrix-js-sdk/lib/matrix.js";
...
import { createMatrixCryptoFacade } from "./sdk/crypto-facade.js";
...
function isUnsupportedAuthenticatedMediaEndpointError(err) {
    ...
    const message = (err instanceof Error ? err.message : String(err)).toLowerCase();
    return (message.includes("m_unrecognized") ||
...
export class MatrixClient {
    ...
    started = false;
    cryptoBootstrapped = false;
    selfUserId;
    dmRoomIds = new Set();
    cryptoInitialized = false;
    decryptBridge;
    verificationManager = new MatrixVerificationManager();
    sendQueue = new KeyedAsyncQueue();
    recoveryKeyStore;
    cryptoBootstrapper;
    autoBootstrapCrypto;
    stopPersistPromise = null;
    dms = {
        update: async () => {
            ...
            if (status.signedByOwner) {
                ...
                try {
                    ...
                    if (repaired.crossSigningPublished && repaired.ownDeviceVerified !== false) {
                        LogService.info("MatrixClientLite", "Cross-signing/bootstrap recovered after forced reset");
                    }
                }
                catch (err) {
                ...
                LogService.warn("MatrixClientLite", "Cross-signing/bootstrap incomplete and no password is configured for UIA fallback");
            }
        }
    ...
    async sendStateEvent(roomId, eventType, stateKey, content) {
        const sent = await this.client.sendStateEvent(roomId, eventType, content, stateKey);
        return sent.event_id;
    }
    async redactEvent(roomId, eventId, reason) {
        const sent = await this.client.redactEvent(roomId, eventId, undefined, reason?.trim() ? { reason } : undefined);
        return sent.event_id;
    }
    ...
    async getRoomKeyBackupStatus() {
        ...
        if (serverVersion && (shouldLoadBackupKey || shouldActivateBackup)) {
            if (shouldLoadBackupKey) {
                if (typeof crypto.loadSessionBackupPrivateKeyFromSecretStorage ===
    ...
    async resetRoomKeyBackup() {
        ...
        try {
            ...
            if (backup.activeVersion !== createdVersion) {
                return await fail("Matrix room key backup was recreated on the server but is not active on this device.");
            }
            if (backup.decryptionKeyCached === false) {
                return await fail("Matrix room key backup was recreated but its decryption key is not cached on this device.");
            }
            if (backup.matchesDecryptionKey === false) {
                return await fail("Matrix room key backup was recreated but this device does not have the matching backup decryption key.");
            }
            if (backup.trusted === false) {
                return await fail("Matrix room key backup was recreated but is not trusted on this device.");
            }
            return {
                success: true,
                previousVersion,
    ...
    async listOwnDevices() {
        ...
        return entries.map((device) => ({
            ...
            lastSeenTs: typeof device.last_seen_ts === "number" && Number.isFinite(device.last_seen_ts)
                ? device.last_seen_ts
                : null,
    ...
    registerBridge() {
        ...
        this.client.on(ClientEvent.Event, (event) => {
            ...
            const membership = raw.type === "m.room.member"
