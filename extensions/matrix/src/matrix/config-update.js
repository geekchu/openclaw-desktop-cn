...
export function shouldStoreMatrixAccountAtTopLevel(cfg, accountId) {
...
export function resolveMatrixConfigPath(cfg, accountId) {
    const normalizedAccountId = normalizeAccountId(accountId);
...
export function resolveMatrixConfigFieldPath(cfg, accountId, fieldPath) {
...
export function updateMatrixAccountConfig(cfg, accountId, patch) {
    const matrix = cfg.channels?.matrix ?? {};
    const normalizedAccountId = normalizeAccountId(accountId);
    const existingAccount = (findMatrixAccountConfig(cfg, normalizedAccountId) ??
        (normalizedAccountId === DEFAULT_ACCOUNT_ID ? matrix : {}));
    ...
    const nextAccounts = Object.fromEntries(Object.entries(matrix.accounts ?? {}).filter(([rawAccountId]) => rawAccountId === normalizedAccountId ||
