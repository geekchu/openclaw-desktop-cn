    ...
    for (const entry of value.bindings) {
        ...
        loaded.push({
            ...
            idleTimeoutMs: typeof entry?.idleTimeoutMs === "number" && Number.isFinite(entry.idleTimeoutMs)
                ...
                : undefined,
            maxAgeMs: typeof entry?.maxAgeMs === "number" && Number.isFinite(entry.maxAgeMs)
...
async function sendFarewellMessage(params) {
    ...
    await sendBindingMessage({
        ...
        accountId: params.accountId,
        roomId,
        threadId: params.record.parentConversationId &&
...
export async function createMatrixThreadBindingManager(params) {
    if (params.auth.accountId !== params.accountId) {
        throw new Error(`Matrix thread binding account mismatch: requested ${params.accountId}, auth resolved ${params.auth.accountId}`);
    }
    ...
    const manager = {
        ...
        touchBinding: (bindingId, at) => {
            ...
            const nextRecord = {
                ...record,
                lastActivityAt: typeof at === "number" && Number.isFinite(at)
    ...
    const sessionBindingAdapter = {
        ...
        bind: async (input) => {
            ...
            if (input.placement === "current" && introText) {
                ...
                const threadId = boundParentConversationId && boundParentConversationId !== boundConversationId
                    ? boundConversationId
                    : undefined;
                await sendBindingMessage({
                    client: params.client,
        ...
        unbind: async (input) => {
            const removed = await unbindRecords(listBindingsForAccount(params.accountId).filter((record) => {
    ...
    if (params.enableSweeper !== false) {
        sweepTimer = setInterval(() => {
            ...
            const expired = listBindingsForAccount(params.accountId)
                ...
                .filter((entry) => typeof entry.lifecycle.expiresAt === "number" &&
