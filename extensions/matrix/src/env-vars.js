...
export function resolveMatrixEnvAccountToken(accountId) {
    return Array.from(normalizeAccountId(accountId))
        .map((char) => /[a-z0-9]/.test(char)
        ? char.toUpperCase()
        : `_X${char.codePointAt(0)?.toString(16).toUpperCase() ?? "00"}_`)
        .join("");
}
export function getMatrixScopedEnvVarNames(accountId) {
...
export function listMatrixEnvAccountIds(env = process.env) {
    const ids = new Set();
