import { applyAccountNameToChannelSection, DEFAULT_ACCOUNT_ID, normalizeAccountId, normalizeSecretInputString, } from "openclaw/plugin-sdk/setup";
...
function moveSingleMatrixAccountConfigToNamedAccount(cfg) {
    ...
    const accounts = typeof base.accounts === "object" && base.accounts
        ? base.accounts
        : {};
    ...
    const defaultAccount = typeof base.defaultAccount === "string" && base.defaultAccount.trim()
        ? normalizeAccountId(base.defaultAccount)
        : undefined;
    const targetAccountId = defaultAccount && defaultAccount !== DEFAULT_ACCOUNT_ID
        ? (Object.entries(accounts).find(([accountId, value]) => accountId &&
            value &&
            typeof value === "object" &&
            normalizeAccountId(accountId) === defaultAccount)?.[0] ?? DEFAULT_ACCOUNT_ID)
        : (defaultAccount ??
...
export function applyMatrixSetupAccountConfig(params) {
    ...
    const migratedCfg = normalizedAccountId !== DEFAULT_ACCOUNT_ID
    ...
    return updateMatrixAccountConfig(next, normalizedAccountId, {
        ...
        allowPrivateNetwork: typeof params.input.allowPrivateNetwork === "boolean"
