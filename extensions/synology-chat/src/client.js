...
const ChatUserListResponseSchema = z.object({
    ...
    data: z
        ...
        users: z
            .array(z.unknown())
            .optional()
            .transform((users) => (users ?? []).flatMap((user) => {
 ...
 */
export async function sendMessage(incomingUrl, text, userId, allowInsecureSsl = true) {
 ...
 */
export async function sendFileUrl(incomingUrl, fileUrl, userId, allowInsecureSsl = true) {
 ...
 */
export async function fetchChatUsers(incomingUrl, allowInsecureSsl = true, log) {
    const now = Date.now();
