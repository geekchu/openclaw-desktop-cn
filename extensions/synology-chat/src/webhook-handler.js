...
const invalidTokenRateLimiters = new Map();
...
export function clearSynologyWebhookRateLimiterStateForTest() {
...
export function getSynologyWebhookRateLimiterCountForTest() {
...
function extractTokenFromHeaders(req) {
    const explicit = headerValue(req.headers["x-synology-token"]) ??
...
function parsePayload(req, body) {
    ...
    const userId = pickAlias(bodyFields, ["user_id", "userId", "user"]) ??
        pickAlias(queryFields, ["user_id", "userId", "user"]);
    const text = pickAlias(bodyFields, ["text", "message", "content"]) ??
    ...
    return {
        ...
        channel_name: pickAlias(bodyFields, ["channel_name"]) ??
            pickAlias(queryFields, ["channel_name"]) ??
            undefined,
        user_id: userId,
        username: pickAlias(bodyFields, ["username", "user_name", "name"]) ??
        ...
        text,
        trigger_word: pickAlias(bodyFields, ["trigger_word", "triggerWord"]) ??
...
async function processAuthorizedSynologyWebhook(params) {
    ...
    try {
        ...
        const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error("Agent response timeout (120s)")), 120_000));
        const reply = await Promise.race([deliverPromise, timeoutPromise]);
        if (!reply) {
            return;
        }
        await synologyClient.sendMessage(params.account.incomingUrl, reply, deliveryUserId, params.account.allowInsecureSsl);
        ...
        await synologyClient.sendMessage(params.account.incomingUrl, "Sorry, an error occurred while processing your message.", deliveryUserId, params.account.allowInsecureSsl);
    }
}
