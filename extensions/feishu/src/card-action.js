...
import { createApprovalCard, FEISHU_APPROVAL_CANCEL_ACTION, FEISHU_APPROVAL_CONFIRM_ACTION, FEISHU_APPROVAL_REQUEST_ACTION, } from "./card-ux-approval.js";
...
export function resetProcessedFeishuCardActionTokensForTests() {
...
async function sendInvalidInteractionNotice(params) {
    const reasonText = params.reason === "stale"
    ...
    try {
        ...
        if (decoded.kind === "structured") {
            ...
            if (envelope.a === FEISHU_APPROVAL_REQUEST_ACTION) {
                ...
                const prompt = typeof envelope.m?.prompt === "string" && envelope.m.prompt.trim()
                    ? envelope.m.prompt
