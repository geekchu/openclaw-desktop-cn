...
export function applyBotIdentityState(accountId, identity) {
...
async function retryBotIdentityProbe(account, accountId, runtime, abortSignal) {
    ...
    for (let i = 0; i < BOT_IDENTITY_RETRY_DELAYS_MS.length; i += 1) {
        ...
        error(`feishu[${accountId}]: bot identity background retry ${i + 1}/${BOT_IDENTITY_RETRY_DELAYS_MS.length} failed` +
...
export function startBotIdentityRecovery(params) {
    ...
    const log = runtime?.log ?? console.log;
    log(`feishu[${accountId}]: bot open_id unknown; starting background retry (delays: ${BOT_IDENTITY_RETRY_DELAYS_MS.map((delay) => `${delay / 1000}s`).join(", ")})`);
