...
function resolveFeishuRequesterConversation(params) {
    ...
    const parsedRequesterTopic = normalizedTarget && threadId && isChatTarget
    ...
    if (requesterSessionKey) {
        ...
        if (existingBindings.length > 1) {
            if (rawTo && normalizedTarget && !threadId && !isChatTarget) {
                const directMatches = existingBindings.filter((entry) => entry.accountId === manager.accountId &&
            ...
            if (parsedRequesterTopic) {
                const matchingTopicBindings = existingBindings.filter((entry) => {
                    ...
                    return (parsed?.chatId === parsedRequesterTopic.chatId &&
                ...
                if (senderScopedTopicBindings.length === 1 &&
...
function resolveMatchingChildBinding(params) {
    ...
    if (requesterConversation) {
        const matched = childBindings.find((entry) => entry.accountId === requesterConversation.accountId &&
...
export async function handleFeishuSubagentSpawning(event, ctx) {
    ...
    try {
        const binding = manager.bindConversation({
            ...
            metadata: {
                ...
                deliveryThreadId: event.requester?.threadId != null && event.requester.threadId !== ""
...
export function handleFeishuSubagentDeliveryTarget(event) {
