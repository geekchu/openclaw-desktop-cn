...
import { cleanBlocksForDescendant, insertTableRow, insertTableColumn, deleteTableRows, deleteTableColumns, mergeTableCells, } from "./docx-table-ops.js";
...
function normalizeConvertedBlockTree(blocks, firstLevelIds) {
    ...
    const inferredTopLevelIds = blocks
        ...
        .sort((a, b) => (originalOrder.get(a.block_id ?? "__missing__") ?? 0) -
    ...
    return { orderedBlocks, rootIds: rootIds.filter((id) => typeof id === "string") };
}
...
async function resolveUploadInput(url, filePath, maxBytes, explicitFileName, imageInput) {
    ...
    if (imageInput) {
        ...
        if (trimmed.length === 0 || !/^[A-Za-z0-9+/]+=*$/.test(trimmed)) {
            throw new Error(`Invalid base64: image input contains characters outside the standard base64 alphabet. ` +
...
async function writeDoc(client, docToken, markdown, maxBytes, logger) {
    ...
    const { orderedBlocks, rootIds } = normalizeConvertedBlockTree(blocks, firstLevelBlockIds);
    const { children: inserted } = blocks.length > BATCH_SIZE
...
async function insertDoc(client, docToken, markdown, afterBlockId, maxBytes, logger) {
    ...
    if (blockIndex === -1) {
        throw new Error(`after_block_id "${afterBlockId}" was not found among the children of parent block "${parentId}". ` +
...
async function writeTableCells(client, docToken, tableBlockId, values) {
    ...
    if (!rows || !cols || !cellIds.length) {
        throw new Error("Table cell IDs unavailable from table block. Use list_blocks/get_block and pass explicit cell block IDs if needed.");
    }
...
export function registerFeishuDocTools(api) {
    ...
    const getMediaMaxBytes = (params, defaultAccountId) => (resolveFeishuToolAccount({ api, executeParams: params, defaultAccountId }).config
        ?.mediaMaxMb ?? 30) *
        1024 *
        1024;
    ...
    if (toolsCfg.doc) {
        api.registerTool((ctx) => {
            ...
            const trustedRequesterOpenId = ctx.messageChannel === "feishu" ? ctx.requesterSenderId?.trim() || undefined : undefined;
            return {
                name: "feishu_doc",
                label: "Feishu Doc",
                ...
                async execute(_toolCallId, params) {
                    ...
                    try {
                        ...
                        switch (p.action) {
                            ...
                            case "write":
                                return json(await writeDoc(client, p.doc_token, p.content, getMediaMaxBytes(p, defaultAccountId), api.logger));
                            case "append":
                                return json(await appendDoc(client, p.doc_token, p.content, getMediaMaxBytes(p, defaultAccountId), api.logger));
                            case "insert":
                                return json(await insertDoc(client, p.doc_token, p.content, p.after_block_id, getMediaMaxBytes(p, defaultAccountId), api.logger));
                            case "create":
                            ...
                            case "create_table":
                                return json(await createTable(client, p.doc_token, p.row_size, p.column_size, p.parent_block_id, p.column_width));
                            ...
                            case "create_table_with_values":
                                return json(await createTableWithValues(client, p.doc_token, p.row_size, p.column_size, p.values, p.parent_block_id, p.column_width));
                            case "upload_image":
                                return json(await uploadImageBlock(client, p.doc_token, getMediaMaxBytes(p, defaultAccountId), p.url, p.file_path, p.parent_block_id, p.filename, p.index, p.image));
                            case "upload_file":
                                return json(await uploadFileBlock(client, p.doc_token, getMediaMaxBytes(p, defaultAccountId), p.url, p.file_path, p.parent_block_id, p.filename));
                            case "color_text":
                            ...
                            case "delete_table_columns":
                                return json(await deleteTableColumns(client, p.doc_token, p.block_id, p.column_start, p.column_count));
                            case "merge_table_cells":
                                return json(await mergeTableCells(client, p.doc_token, p.block_id, p.row_start, p.row_end, p.column_start, p.column_end));
                            default:
