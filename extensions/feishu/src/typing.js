 ...
 */
export function isFeishuBackoffError(err) {
 ...
 */
export function getBackoffCodeFromResponse(response) {
...
export async function removeTypingIndicator(params) {
    ...
    try {
        ...
        if (backoffCode !== undefined) {
            if (getFeishuRuntime().logging.shouldLogVerbose()) {
                runtime?.log?.(`[feishu] typing indicator removal response contains backoff code ${backoffCode}, stopping keepalive`);
            }
            throw new FeishuBackoffError(backoffCode);
        }
    }
