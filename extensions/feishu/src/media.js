...
import { withTempDownloadPath } from "../runtime-api.js";
...
function extractFeishuUploadKey(response, params) {
    ...
    if (wrappedResponse.code !== undefined && wrappedResponse.code !== 0) {
        throw new Error(`${params.errorPrefix}: ${wrappedResponse.msg || `code ${wrappedResponse.code}`}`);
    }
    const key = params.key === "image_key"
...
function extractFeishuDownloadMetadata(response) {
    ...
    const headers = asHeaderMap(responseWithOptionalFields.headers) ??
...
async function readFeishuResponseBuffer(params) {
    ...
    if (responseWithOptionalFields.code !== undefined && responseWithOptionalFields.code !== 0) {
        throw new Error(`${params.errorPrefix}: ${responseWithOptionalFields.msg || `code ${responseWithOptionalFields.code}`}`);
    }
 ...
 */
export function sanitizeFileNameForUpload(fileName) {
