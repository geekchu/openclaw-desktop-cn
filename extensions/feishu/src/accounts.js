...
const { listConfiguredAccountIds, listAccountIds: listFeishuAccountIds, resolveDefaultAccountId, } = createAccountListHelpers("feishu", {
...
export class FeishuSecretRefUnavailableError extends Error {
    ...
    constructor(path, ref) {
        super(`${path}: unresolved SecretRef "${formatSecretRefLabel(ref)}". ` +
            "Resolve this command against an active gateway runtime snapshot before reading it.");
        this.name = "FeishuSecretRefUnavailableError";
        this.path = path;
    }
}
export function isFeishuSecretRefUnavailableError(error) {
...
function resolveFeishuEventSecrets(cfg, mode) {
    return {
        encryptKey: (cfg?.connectionMode ?? "websocket") === "webhook"
...
export function resolveFeishuCredentials(cfg, options) {
...
export function inspectFeishuCredentials(cfg) {
 ...
 */
export function resolveFeishuRuntimeAccount(params, options) {
    return buildResolvedFeishuAccount({
        ...params,
        baseMode: "strict",
