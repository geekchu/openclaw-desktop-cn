...
import { buildChannelConfigSchema, buildProbeChannelStatusSummary, chunkTextForOutbound, createActionGate, createDefaultChannelRuntimeState, DEFAULT_ACCOUNT_ID, PAIRING_APPROVED_MESSAGE, } from "../runtime-api.js";
import { inspectFeishuCredentials, resolveFeishuAccount, resolveFeishuRuntimeAccount, listFeishuAccountIds, listEnabledFeishuAccounts, resolveDefaultFeishuAccountId, } from "./accounts.js";
import { FEISHU_CARD_INTERACTION_VERSION } from "./card-interaction.js";
import { createFeishuClient } from "./client.js";
import { FeishuConfigSchema } from "./config-schema.js";
import { buildFeishuConversationId, parseFeishuConversationId, parseFeishuDirectConversationId, parseFeishuTargetId, } from "./conversation-id.js";
...
const collectFeishuSecurityWarnings = createAllowlistProviderGroupPolicyWarningCollector({
    ...
    resolveGroupPolicy: ({ cfg, accountId }) => resolveFeishuAccount({ cfg, accountId }).config?.groupPolicy,
...
function describeFeishuMessageTool({ cfg, }) {
    const enabled = cfg.channels?.feishu?.enabled !== false &&
...
function isFeishuReactionsActionEnabled(params) {
    ...
    const gate = createActionGate((params.account.config.actions ??
        params.cfg.channels?.feishu?.actions));
...
function normalizeFeishuAcpConversationId(conversationId) {
    ...
    return {
        ...
        parentConversationId: parsed.scope === "group_topic" || parsed.scope === "group_topic_sender"
            ? parsed.chatId
            : undefined,
    };
}
function matchFeishuAcpConversation(params) {
    ...
    const matchesParentTopicForSenderScopedConversation = incoming.scope === "group_topic_sender" &&
    ...
    return {
        ...
        parentConversationId: incoming.scope === "group_topic" || incoming.scope === "group_topic_sender"
            ? incoming.chatId
            : undefined,
...
function resolveFeishuSenderScopedCommandConversation(params) {
    ...
    const boundConversation = getSessionBindingService()
        ...
        .find((binding) => {
        if (binding.conversation.channel !== "feishu" ||
...
function resolveFeishuCommandConversation(params) {
    if (params.threadId) {
        const parentConversationId = parseFeishuTargetId(params.originatingTo) ??
        ...
        return {
            conversationId: senderScopedConversationId ??
    ...
    const conversationId = parseFeishuDirectConversationId(params.originatingTo) ??
...
function resolveFeishuChatId(ctx) {
    const raw = readFirstString(ctx.params, ["chatId", "chat_id", "channelId", "channel_id", "to", "target"], ctx.toolContext?.currentChannelId);
    if (!raw) {
        return undefined;
    }
...
function resolveFeishuMemberIdType(params) {
    ...
    if (readFirstString(params, ["userId", "user_id"]) &&
        !readFirstString(params, ["openId", "open_id", "unionId", "union_id"])) {
        return "user_id";
    }
    if (readFirstString(params, ["unionId", "union_id"]) &&
        !readFirstString(params, ["openId", "open_id"])) {
        return "union_id";
    }
    return "open_id";
}
export const feishuPlugin = createChatChannelPlugin({
    base: {
        id: "feishu",
        meta: {
            ...meta,
        ...
        actions: {
            ...
            handleAction: async (ctx) => {
                ...
                if (ctx.action === "send" || ctx.action === "thread-reply") {
                    ...
                    const replyToMessageId = ctx.action === "thread-reply" ? resolveFeishuMessageId(ctx.params) : undefined;
                    if (ctx.action === "thread-reply" && !replyToMessageId) {
                        throw new Error("Feishu thread-reply requires messageId.");
                    }
                    const card = ctx.params.card && typeof ctx.params.card === "object"
                        ? ctx.params.card
                        : undefined;
                    ...
                    if (card) {
                        if (containsLegacyFeishuCardCommandValue(card)) {
                            throw new Error("Feishu card buttons that trigger text or commands must use structured interaction envelopes.");
                        }
                ...
                if (ctx.action === "channel-info") {
                    ...
                    const members = await runtime.getChatMembers(client, chatId, readOptionalNumber(ctx.params, ["pageSize", "page_size"]), readFirstString(ctx.params, ["pageToken", "page_token"]), resolveFeishuMemberIdType(ctx.params));
                    return jsonActionResult({
                        ok: true,
                        provider: "feishu",
                        action: "channel-info",
                ...
                if (ctx.action === "member-info") {
                    ...
                    if (memberId) {
                        const member = await runtime.getFeishuMemberInfo(client, memberId, resolveFeishuMemberIdType(ctx.params));
                        return jsonActionResult({
                            ok: true,
                            channel: "feishu",
                            action: "member-info",
                ...
                if (ctx.action === "react") {
                    ...
                    if (!emoji) {
                        if (!clearAll) {
                            throw new Error("Emoji is required to add a Feishu reaction. Set clearAll=true to remove all bot reactions.");
                        }
                        const { listReactionsFeishu, removeReactionFeishu } = await loadFeishuChannelRuntime();
        ...
        gateway: {
            startAccount: async (ctx) => {
                ...
                const account = resolveFeishuRuntimeAccount({ cfg: ctx.cfg, accountId: ctx.accountId }, { requireEventSecrets: true });
