...
export function resolveFeishuGroupSession(params) {
    ...
    const replyInThread = (groupConfig?.replyInThread ?? feishuCfg?.replyInThread ?? "disabled") === "enabled" ||
        threadReply;
    const legacyTopicSessionMode = groupConfig?.topicSessionMode ?? feishuCfg?.topicSessionMode ?? "disabled";
    const groupSessionScope = groupConfig?.groupSessionScope ??
...
export function parseMessageContent(content, messageType) {
    if (messageType === "post") {
...
export function checkBotMentioned(event, botOpenId) {
    if (!botOpenId) {
        return false;
    }
...
export function normalizeMentions(text, mentions, botStripId) {
    ...
    for (const mention of mentions) {
        ...
        const replacement = botStripId && mentionId === botStripId
            ? ""
            : mentionId
...
export function normalizeFeishuCommandProbeBody(text) {
    if (!text) {
        return "";
    }
    return text
...
export function parseMediaKeys(content, messageType) {
    try {
        const parsed = JSON.parse(content);
...
export function toMessageResourceType(messageType) {
