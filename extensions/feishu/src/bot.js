...
function resolveChannelContextVisibilityMode(params) {
  ...
  const matchingEntry =
    normalizedAccountId && accountEntries && typeof accountEntries === "object"
      ? Object.entries(accountEntries).find(
          ([accountId]) => accountId.trim().toLowerCase() === normalizedAccountId,
        )?.[1]
      : undefined;
  return (
    matchingEntry?.contextVisibility ??
