...
import { buildAgentMediaPayload, buildPendingHistoryContextFromMap, clearHistoryEntriesIfEnabled, createChannelPairingController, DEFAULT_GROUP_HISTORY_LIMIT, normalizeAgentId, recordPendingHistoryEntryIfEnabled, resolveAgentOutboundIdentity, resolveOpenProviderRuntimeGroupPolicy, resolveDefaultGroupPolicy, warnMissingProviderGroupPolicyFallbackOnce, } from "../runtime-api.js";
import { resolveFeishuRuntimeAccount } from "./accounts.js";
import { checkBotMentioned, normalizeFeishuCommandProbeBody, normalizeMentions, parseMergeForwardContent, parseMessageContent, resolveFeishuGroupSession, resolveFeishuMediaList, } from "./bot-content.js";
...
export function resolveBroadcastAgents(cfg, peerId) {
...
export function buildBroadcastSessionKey(baseSessionKey, originalAgentId, targetAgentId) {
...
export async function handleFeishuMessage(params) {
    ...
    try {
        ...
        const feishuAcpConversationSupported = !isGroup ||
        ...
        const isTopicSessionForThread = isGroup &&
            (groupSession?.groupSessionScope === "group_topic" ||
                groupSession?.groupSessionScope === "group_topic_sender");
        ...
        const resolveThreadContextForAgent = async (agentId, agentSessionKey) => {
            ...
            const threadContext = {
                threadLabel: (ctx.rootId || ctx.threadId) && isTopicSessionForThread
            ...
            try {
                ...
                const senderIds = new Set([ctx.senderOpenId, senderUserId]
                    .map((id) => id?.trim())
                    .filter((id) => id !== undefined && id.length > 0));
                const relevantMessages = (senderScoped
                    ? threadMessages.filter((msg) => msg.senderType === "app" ||
        ...
        const isTopicSession = isGroup &&
            (groupSession?.groupSessionScope === "group_topic" ||
                groupSession?.groupSessionScope === "group_topic_sender");
        const configReplyInThread = isGroup &&
            ...
            if (strategy === "sequential") {
                ...
                for (let i = 0; i < results.length; i++) {
                    if (results[i].status === "rejected") {
                        log(`feishu[${account.accountId}]: broadcast dispatch failed for agent=${broadcastAgents[i]}: ${String(results[i].reason)}`);
                    }
                }
            }
            if (isGroup && historyKey && chatHistories) {
