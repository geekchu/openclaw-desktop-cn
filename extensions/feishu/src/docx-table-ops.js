...
export function calculateAdaptiveColumnWidths(blocks, tableBlockId) {
    ...
    const totalWidth = originalWidths && originalWidths.length > 0
 ...
 */
export function cleanBlocksForDescendant(blocks) {
...
export async function insertTableRow(client, docToken, blockId, rowIndex = -1) {
...
export async function insertTableColumn(client, docToken, blockId, columnIndex = -1) {
    const res = await client.docx.documentBlock.patch({
        path: { document_id: docToken, block_id: blockId },
...
export async function deleteTableRows(client, docToken, blockId, rowStart, rowCount = 1) {
    const res = await client.docx.documentBlock.patch({
        path: { document_id: docToken, block_id: blockId },
...
export async function deleteTableColumns(client, docToken, blockId, columnStart, columnCount = 1) {
    const res = await client.docx.documentBlock.patch({
        path: { document_id: docToken, block_id: blockId },
        data: {
...
export async function mergeTableCells(client, docToken, blockId, rowStart, rowEnd, columnStart, columnEnd) {
    const res = await client.docx.documentBlock.patch({
        path: { document_id: docToken, block_id: blockId },
        data: {
            merge_table_cells: {
