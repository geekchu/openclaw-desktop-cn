 ...
 */
export async function insertBlocksInBatches(client, docToken, blocks, firstLevelBlockIds, logger, parentBlockId = docToken, startIndex = -1) {
    const allChildren = [];
    ...
    for (const firstLevelId of firstLevelBlockIds) {
        ...
        if (newBlocks.length > BATCH_SIZE) {
            throw new Error(`Block "${firstLevelId}" has ${newBlocks.length} descendants, which exceeds the ` +
        ...
        if (currentBatch.blocks.length + newBlocks.length > BATCH_SIZE &&
