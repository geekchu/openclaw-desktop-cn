...
export async function resolveReactionSyntheticEvent(params) {
    const { cfg, accountId, event, botOpenId, fetchMessage = getMessageFeishu, verificationTimeoutMs = FEISHU_REACTION_VERIFY_TIMEOUT_MS, logger, uuid = () => crypto.randomUUID(), action = "created", } = params;
    ...
    if (!reactedMsg || (reactionNotifications === "own" && !isBotMessage)) {
        logger?.(`feishu[${accountId}]: ignoring reaction on non-bot/unverified message ${messageId} ` +
...
function registerEventHandlers(eventDispatcher, context) {
    ...
    const recordSuppressedMessageIds = async (entries, dispatchMessageId) => {
        ...
        const suppressedIds = new Set(entries
