...
import { createInboundDebouncer, resolveInboundDebounceMs, } from "openclaw/plugin-sdk/channel-inbound";
import { buildCommandsMessagePaginated, buildCommandsPaginationKeyboard, formatModelsAvailableHeader, resolveStoredModelOverride, } from "openclaw/plugin-sdk/command-auth";
import { writeConfigFile } from "openclaw/plugin-sdk/config-runtime";
import { loadSessionStore, resolveSessionStoreEntry, updateSessionStore, } from "openclaw/plugin-sdk/config-runtime";
import { applyModelOverrideToSessionEntry } from "openclaw/plugin-sdk/config-runtime";
import { buildPluginBindingResolvedText, parsePluginBindingApprovalCustomId, resolvePluginConversationBindingApproval, } from "openclaw/plugin-sdk/conversation-runtime";
...
import { APPROVE_CALLBACK_DATA_RE, hasInboundMedia, hasReplyTargetMedia, isMediaSizeLimitError, isRecoverableMediaGroupError, resolveInboundMediaFileId, } from "./bot-handlers.media.js";
import { MEDIA_GROUP_TIMEOUT_MS, } from "./bot-updates.js";
import { resolveMedia } from "./bot/delivery.js";
import { getTelegramTextParts, buildTelegramGroupPeerId, buildTelegramParentPeer, resolveTelegramForumFlag, resolveTelegramForumThreadId, resolveTelegramGroupAllowFromContext, withResolvedTelegramForumFlag, } from "./bot/helpers.js";
import { resolveTelegramConversationBaseSessionKey, resolveTelegramConversationRoute, } from "./conversation-route.js";
import { enforceTelegramDmAccess } from "./dm-access.js";
import { isTelegramExecApprovalApprover, isTelegramExecApprovalClientEnabled, shouldEnableTelegramExecApprovalButtons, } from "./exec-approvals.js";
import { evaluateTelegramGroupBaseAccess, evaluateTelegramGroupPolicyAccess, } from "./group-access.js";
import { migrateTelegramGroupConfig } from "./group-migration.js";
import { resolveTelegramInlineButtonsScope } from "./inline-buttons.js";
import { buildModelsKeyboard, buildProviderKeyboard, calculateTotalPages, getModelsPageSize, parseModelCallbackData, resolveModelSelection, } from "./model-buttons.js";
import { buildInlineKeyboard } from "./send.js";
export const registerTelegramHandlers = ({ cfg, accountId, bot, opts, telegramTransport, runtime, mediaMaxBytes, telegramCfg, allowFrom, groupAllowFrom, resolveGroupPolicy, resolveTelegramGroupConfig, shouldSkipUpdate, processMessage, logger, telegramDeps = defaultTelegramBotDeps, }) => {
    ...
    const isSelfAuthoredTelegramMessage = (ctx, message) => {
    ...
    const inboundDebouncer = createInboundDebouncer({
        ...
        onError: (err, items) => {
            ...
            if (chatId != null) {
                ...
                void bot.api
                    .sendMessage(chatId, "Something went wrong while processing your message. Please try again.", threadId != null ? { message_thread_id: threadId } : undefined)
                    .catch((sendErr) => {
    ...
    bot.on("callback_query", async (ctx) => {
        ...
        try {
            ...
            const editCallbackMessage = async (text, params) => typeof ctx.editMessageText === "function"
                ? ctx.editMessageText(text, params)
                : bot.api.editMessageText(callbackMessage.chat.id, callbackMessage.message_id, text, params);
            ...
            const replyToCallbackChat = async (text, params) => typeof ctx.reply === "function"
            ...
            const execApprovalButtonsEnabled = isApprovalCallback &&
            ...
            const authorizationMode = !isGroup || (!execApprovalButtonsEnabled && inlineButtonsScope === "allowlist")
                ? "callback-allowlist"
                : "callback-scope";
            const senderAuthorization = authorizeTelegramEventSender({
                chatId,
            ...
            const pluginCallback = await dispatchPluginInteractiveHandler({
                ...
                respond: {
                    reply: async ({ text, buttons }) => {
                        await replyToCallbackChat(text, buttons ? { reply_markup: buildInlineKeyboard(buttons) } : undefined);
                    },
                    editMessage: async ({ text, buttons }) => {
                        await editCallbackMessage(text, buttons ? { reply_markup: buildInlineKeyboard(buttons) } : undefined);
                    },
            ...
            if (isApprovalCallback) {
                if (!isTelegramExecApprovalClientEnabled({ cfg: runtimeCfg, accountId }) ||
            ...
            if (modelCallback) {
                ...
                const modelData = await telegramDeps.buildModelsProviderData(runtimeCfg, sessionState.agentId);
