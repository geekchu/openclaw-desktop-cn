import { createEnvPatchedAccountSetupAdapter, patchChannelConfigForAccount, promptResolvedAllowFrom, splitSetupEntries, } from "openclaw/plugin-sdk/setup";
...
export async function promptTelegramAllowFromForAccount(params) {
    ...
    if (!resolved.token?.trim()) {
        await params.prompter.note("Telegram token missing; username lookup is unavailable.", "Telegram");
    }
...
export const telegramSetupAdapter = createEnvPatchedAccountSetupAdapter({
    channelKey: channel,
