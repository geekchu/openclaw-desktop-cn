import { chunkMarkdownIR, FILE_REF_EXTENSIONS_WITH_TLD, isAutoLinkedFileRef, markdownToIR, } from "openclaw/plugin-sdk/text-runtime";
...
function wrapSegmentFileRefs(text, codeDepth, preDepth, anchorDepth) {
    ...
    return wrappedStandalone.replace(getOrphanedTldPattern(), (match, prefix, tld) => prefix === ">" ? match : `${prefix}<code>${escapeHtml(tld)}</code>`);
}
...
function splitTelegramChunkByHtmlLimit(chunk, htmlLimit) {
...
function findLargestTelegramChunkTextLengthWithinHtmlLimit(chunk, htmlLimit) {
    const currentTextLength = chunk.text.length;
    if (currentTextLength <= 1) {
...
function coalesceWhitespaceOnlyMarkdownIRChunks(chunks, limit) {
    const coalesced = [];
    let index = 0;
    while (index < chunks.length) {
        ...
        const canMergePrev = (candidate) => renderTelegramChunkHtml(candidate).length <= limit;
        const canMergeNext = (candidate) => renderTelegramChunkHtml(candidate).length <= limit;
        if (prev) {
