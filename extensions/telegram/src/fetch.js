...
import { createPinnedLookup, hasEnvHttpProxyConfigured, resolveFetch, } from "openclaw/plugin-sdk/fetch-runtime";
...
function shouldBypassEnvProxyForTelegramApi(env = process.env) {
    ...
    for (let i = 0; i < noProxyEntries.length; i++) {
        ...
        if (targetHostname === entryHostname ||
...
function createTelegramDispatcher(policy) {
    ...
    if (policy.mode === "env-proxy") {
        ...
        try {
            ...
            return {
                dispatcher: new Agent(directPolicy.connect
...
export function shouldRetryTelegramTransportFallback(err) {
...
export function resolveTelegramTransport(proxyFetch, options) {
    ...
    const allowStickyFallback = defaultDispatcher.mode === "direct" ||
 ...
 */
export function resolveTelegramApiBase(apiRoot) {
