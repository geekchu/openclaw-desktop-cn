...
import { createAccountActionGate, DEFAULT_ACCOUNT_ID, listCombinedAccountIds, normalizeAccountId, normalizeOptionalAccountId, resolveAccountEntry, resolveListedDefaultAccountId, resolveAccountWithDefaultFallback, } from "openclaw/plugin-sdk/account-resolution";
import { listBoundAccountIds, resolveDefaultAgentBoundAccountId, } from "openclaw/plugin-sdk/routing";
...
export function resetMissingDefaultWarnFlag() {
...
export function resolveDefaultTelegramAccountId(cfg) {
    ...
    if (ids.length > 1 && !emittedMissingDefaultWarn) {
        ...
        getLog().warn(`channels.telegram: accounts.default is missing; falling back to "${ids[0]}". ` +
            `${formatSetExplicitDefaultInstruction("telegram")} to avoid routing surprises in multi-account setups.`);
    }
    return resolved;
}
export function resolveTelegramAccountConfig(cfg, accountId) {
    const normalized = normalizeAccountId(accountId);
    return resolveAccountEntry(cfg.channels?.telegram?.accounts, normalized);
}
export function mergeTelegramAccountConfig(cfg, accountId) {
...
export function resolveTelegramPollActionGateState(isActionEnabled) {
