...
import { buildTokenChannelStatusSummary, clearAccountEntryFields, DEFAULT_ACCOUNT_ID, PAIRING_APPROVED_MESSAGE, projectCredentialSnapshotFields, resolveConfiguredFromCredentialStatuses, } from "../runtime-api.js";
import { listTelegramAccountIds, resolveTelegramAccount, } from "./accounts.js";
import { resolveTelegramAutoThreadId } from "./action-threading.js";
import { buildTelegramExecApprovalButtons } from "./approval-buttons.js";
import * as auditModule from "./audit.js";
import { buildTelegramGroupPeerId } from "./bot/helpers.js";
import { listTelegramDirectoryGroupsFromConfig, listTelegramDirectoryPeersFromConfig, } from "./directory-config.js";
import { buildTelegramExecApprovalPendingPayload, shouldSuppressTelegramExecApprovalForwardingFallback, } from "./exec-approval-forwarding.js";
import { isTelegramExecApprovalClientEnabled, resolveTelegramExecApprovalTarget, } from "./exec-approvals.js";
import { resolveTelegramGroupRequireMention, resolveTelegramGroupToolPolicy, } from "./group-policy.js";
...
import { createTelegramPluginBase, findTelegramTokenOwnerAccountId, formatDuplicateTelegramTokenReason, telegramConfigAdapter, } from "./shared.js";
import { collectTelegramStatusIssues } from "./status-issues.js";
import { parseTelegramTarget } from "./targets.js";
function getTelegramStatusRuntimeHelpers() {
    try {
        return (getTelegramRuntime().channel?.telegram ?? {});
    }
    catch (error) {
...
function resolveTelegramMonitor() {
    return (getTelegramStatusRuntimeHelpers().monitorTelegramProvider ??
...
export const telegramPlugin = createChatChannelPlugin({
    base: {
        ...
        allowlist: buildDmGroupAccountAllowlistAdapter({
            ...
            normalize: ({ cfg, accountId, values }) => telegramConfigAdapter.formatAllowFrom({ cfg, accountId, allowFrom: values }),
        ...
        execApprovals: {
            getInitiatingSurfaceState: ({ cfg, accountId }) => isTelegramExecApprovalClientEnabled({ cfg, accountId })
                ? { kind: "enabled" }
                : { kind: "disabled" },
            hasConfiguredDmRoute: ({ cfg }) => hasTelegramExecApprovalDmRoute(cfg),
            shouldSuppressForwardingFallback: (params) => shouldSuppressTelegramExecApprovalForwardingFallback(params),
            buildPendingPayload: ({ request, nowMs }) => buildTelegramExecApprovalPendingPayload({ request, nowMs }),
            beforeDeliverPending: async ({ cfg, target, payload }) => {
                const hasExecApprovalData = payload.channelData &&
                ...
                const threadId = typeof target.threadId === "number"
                    ? target.threadId
    ...
    threading: {
        ...
        resolveAutoThreadId: ({ to, toolContext, replyToId }) => replyToId ? undefined : resolveTelegramAutoThreadId({ to, toolContext }),
    },
    outbound: {
        base: {
            deliveryMode: "direct",
