...
import { resolveCommandAuthorization, resolveCommandAuthorizedFromAuthorizers, resolveNativeCommandSessionTargets, } from "openclaw/plugin-sdk/command-auth";
import { buildCommandTextFromArgs, findCommandByNativeName, listNativeCommandSpecs, listNativeCommandSpecsForConfig, parseCommandArgs, resolveCommandArgMenu, } from "openclaw/plugin-sdk/command-auth";
import { getRuntimeConfigSnapshot } from "openclaw/plugin-sdk/config-runtime";
import { resolveMarkdownTableMode } from "openclaw/plugin-sdk/config-runtime";
import { normalizeTelegramCommandName, resolveTelegramCustomCommands, TELEGRAM_COMMAND_NAME_PATTERN, } from "openclaw/plugin-sdk/config-runtime";
...
import { buildTelegramRoutingTarget, buildTelegramThreadParams, buildSenderName, buildTelegramGroupFrom, extractTelegramForumFlag, resolveTelegramForumFlag, resolveTelegramGroupAllowFromContext, resolveTelegramThreadSpec, } from "./bot/helpers.js";
...
async function resolveTelegramCommandAuth(params) {
    ...
    const getChat = typeof bot.api.getChat === "function"
        ? bot.api.getChat.bind(bot.api)
        : undefined;
    ...
    const commandsAllowFromConfigured = commandsAllowFrom != null &&
...
export const registerTelegramNativeCommands = ({ bot, cfg, runtime, accountId, telegramCfg, allowFrom, groupAllowFrom, replyToMode, textLimit, useAccessGroups, nativeEnabled, nativeSkillsEnabled, nativeDisabledExplicit, resolveGroupPolicy, resolveTelegramGroupConfig, shouldSkipUpdate, telegramDeps = defaultTelegramBotDeps, opts, }) => {
    ...
    const skillCommands = nativeEnabled && nativeSkillsEnabled && boundRoute
    ...
    if (commandsToRegister.length > 0 || pluginCatalog.commands.length > 0) {
        for (const command of nativeCommands) {
            ...
            bot.command(normalizedCommandName, async (ctx) => {
                ...
                const disableBlockStreaming = typeof runtimeTelegramCfg.blockStreaming === "boolean"
