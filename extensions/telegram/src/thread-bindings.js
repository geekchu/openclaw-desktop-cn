...
async function persistBindingsToDisk(params) {
    ...
    const payload = {
        ...
        bindings: params.bindings ??
            [...getThreadBindingsState().bindingsByAccountConversation.values()].filter((entry) => entry.accountId === params.accountId),
    };
...
export function createTelegramThreadBindingManager(params = {}) {
    ...
    const sessionBindingAdapter = {
        ...
        bind: async (input) => {
            ...
            getThreadBindingsState().bindingsByAccountConversation.set(resolveBindingKey({ accountId, conversationId }), record);
