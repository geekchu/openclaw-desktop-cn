...
function getDraftStreamState() {
    if (!draftStreamState) {
        ...
        draftStreamState = globalStore[TELEGRAM_DRAFT_STREAM_STATE_KEY] ?? {
            nextDraftId: 0,
        };
...
export function createTelegramDraftStream(params) {
    ...
    const sendRenderedMessageWithThreadFallback = async (sendArgs) => {
        ...
        try {
            ...
            return {
                sent: await params.api.sendMessage(chatId, sendArgs.renderedText, Object.keys(threadlessParams).length > 0 ? threadlessParams : undefined),
        ...
        try {
            ...
            if (typeof sentId === "number" && Number.isFinite(sentId)) {
                ...
                if (resolvedDraftApi != null && streamDraftId != null) {
                    ...
                    const clearThreadParams = usedThreadParams && threadParams?.message_thread_id != null
                        ? { message_thread_id: threadParams.message_thread_id }
                        : undefined;
                    try {
