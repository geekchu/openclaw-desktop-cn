...
export function tagTelegramNetworkError(err, origin) {
    if (!err || typeof err !== "object") {
        return;
    }
...
export function getTelegramNetworkErrorOrigin(err) {
...
export function isTelegramPollingNetworkError(err) {
 ...
 */
export function isSafeToRetrySendError(err) {
    if (!err) {
        return false;
    }
    for (const candidate of collectTelegramErrorCandidates(err)) {
...
export function isTelegramServerError(err) {
...
export function isTelegramClientRejection(err) {
