...
#[cfg(not(target_os = "windows"))]
fn build_unix_openclaw_launcher() -> Result<(String, String), String> {
    ...
    extra_env.push(("OPENCLAW_GATEWAY_PORT".to_string(), "28789".to_string()));
...
#[cfg(target_os = "windows")]
fn build_windows_whatsapp_login_script() -> Result<String, String> {
...
pub async fn start_channel_login(channel_type: String) -> Result<String, String> {
    ...
    match channel_type.as_str() {
        "whatsapp" => {
            ...
            {
                let env_path = platform::get_env_file_path();
                let (env_lines, launcher) = build_unix_openclaw_launcher()?;
...
sleep 2
invoke_openclaw gateway start 2>/dev/null || invoke_openclaw gateway --port 28789 --bind lan &
sleep 3
echo "✅ Gateway 已重启"
echo ""
...
"#,
            ...
            #[cfg(target_os = "windows")]
            {
                ...
                let script_content = build_windows_whatsapp_login_script()?;
