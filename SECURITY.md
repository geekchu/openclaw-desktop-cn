...
- Reports that require write access to trusted local state (`~/.openclawcn`, workspace files like `MEMORY.md` / `memory/*.md`)
...
- Anyone who can modify `~/.openclawcn` state/config (including `openclaw.json`) is effectively a trusted operator.
