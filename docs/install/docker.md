...
<Steps>
  ...
  <Step title="Open the Control UI">
    Open `http://127.0.0.1:28789/` in your browser and paste the token into
    Settings.
...
docker compose run --rm --no-deps --entrypoint node openclaw-gateway \
  ...
  '["http://localhost:28789","http://127.0.0.1:28789"]' --strict-json
...
```bash
curl -fsS http://127.0.0.1:28789/healthz   # liveness
curl -fsS http://127.0.0.1:28789/readyz     # readiness
```
...
`http://127.0.0.1:28789` works with Docker port publishing.
...
`OPENCLAW_WORKSPACE_DIR` to `/home/node/.openclawcn/workspace`, so those paths
...
<AccordionGroup>
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    docker compose run --rm openclaw-cli devices list --url ws://127.0.0.1:28789
    ```
