...
<Steps>
  ...
  <Step title="Run onboarding inside the container">
    Run `./scripts/run-openclaw-podman.sh launch setup`, then open `http://127.0.0.1:28789/`.
  </Step>
...
- It creates `~/.openclawcn/openclaw.json` with `gateway.mode: "local"` if missing.
- It creates `~/.openclawcn/.env` with `OPENCLAW_GATEWAY_TOKEN` if missing.
- For manual launches, the helper reads only a small allowlist of Podman-related keys from `~/.openclawcn/.env` and passes explicit runtime env vars to the container; it does not hand the full env file to Podman.
...
Then open `http://127.0.0.1:28789/` and use the token from `~/.openclawcn/.env`.
...
- forwarded to `127.0.0.1:28789` inside the Podman VM
...
OPENCLAW_CONTAINER=openclaw openclaw config set gateway.controlUi.allowedOrigins \
  '["http://127.0.0.1:28789","http://localhost:28789","http://127.0.0.1:28889","http://localhost:28889"]' \
  --strict-json
...
- `28789` is usually already occupied on the Mac host by the Podman-published gateway port, so the tunnel uses `28889` as the local browser port.
...
openclaw devices list \
  ...
  --token "$(sed -n 's/^OPENCLAW_GATEWAY_TOKEN=//p' ~/.openclawcn/.env | head -n1)"
```
...
- **Config dir:** `~/.openclaw`
- **Workspace dir:** `~/.openclawcn/workspace`
- **Token file:** `~/.openclawcn/.env`
...
- `OPENCLAW_PODMAN_GATEWAY_HOST_PORT` -- host port mapped to container `28789`
...
The manual launcher reads `~/.openclawcn/.env` before finalizing container/image defaults, so you can persist these there.
...
- It still reads `~/.openclawcn/.env` for gateway runtime env such as `OPENCLAW_GATEWAY_TOKEN`, but it does not consume the manual launcher's Podman-specific override allowlist.
...
- **Gateway start blocked (missing `gateway.mode=local`):** Ensure `~/.openclawcn/openclaw.json` exists and sets `gateway.mode="local"`. `scripts/podman/setup.sh` creates this if missing.
