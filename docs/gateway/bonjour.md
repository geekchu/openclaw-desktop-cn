...
- serve your chosen domain (example: `openclaw.internal.`) from `~/.openclawcn/dns/<domain>.db`
...
The Gateway WS port (default `28789`) binds to loopback by default. For LAN/tailnet
access, bind explicitly and keep auth enabled.

For tailnet‑only setups:

- Set `gateway.bind: "tailnet"` in `~/.openclawcn/openclaw.json`.
