...
```json5
{
  agent: { workspace: "~/.openclawcn/workspace" },
  channels: { whatsapp: { allowFrom: ["+15555550123"] } },
}
```

Save to `~/.openclawcn/openclaw.json` and you can DM the bot from that number.
...
```json5
{
  ...
  session: {
    ...
    store: "~/.openclawcn/agents/default/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
  ...
  cron: {
    enabled: true,
    store: "~/.openclawcn/cron/cron.json",
    maxConcurrentRuns: 2,
  ...
  gateway: {
    ...
    remote: { url: "ws://gateway.tailnet:28789", token: "remote-token" },
