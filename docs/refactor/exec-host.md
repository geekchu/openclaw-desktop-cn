...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.openclawcn/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.openclawcn/node.json` (node id + pairing token).
...
Path: `~/.openclawcn/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.openclawcn/exec-approvals.sock",
...
- Unix socket at `~/.openclawcn/exec-approvals.sock` (0600).
