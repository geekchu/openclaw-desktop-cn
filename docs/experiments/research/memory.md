...
read_when:
  - Designing workspace memory (~/.openclawcn/workspace) beyond daily Markdown logs
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.openclawcn/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.openclawcn/workspace` as canonical human-readable memory.
...
```
~/.openclawcn/workspace/.memory/index.sqlite
```
...
Pragmatic take for `~/.openclawcn/workspace`:
