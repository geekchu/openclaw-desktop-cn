...
<Steps>
  <Step title="Existing config detection">
    - If `~/.openclawcn/openclaw.json` exists, choose Keep, Modify, or Reset.
...
- OAuth credentials: `~/.openclawcn/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.openclawcn/agents/<agentId>/agent/auth-profiles.json`
...
`~/.openclawcn/credentials/oauth.json` (or `$OPENCLAW_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
