...
3. Put a minimal config in `~/.openclawcn/openclaw.json`:
...
- Session files: `~/.openclawcn/agents/<agentId>/sessions/{{SessionId}}.jsonl`
- Session metadata (token usage, last route, etc): `~/.openclawcn/agents/<agentId>/sessions/sessions.json` (legacy: `~/.openclawcn/sessions/sessions.json`)
