...
- Secrets live in `~/.openclawcn/agents/<agentId>/agent/auth-profiles.json` (legacy: `~/.openclawcn/agent/auth-profiles.json`).
- Config `auth.profiles` / `auth.order` are **metadata + routing only** (no secrets).
- Legacy import-only OAuth file: `~/.openclawcn/credentials/oauth.json` (imported into `auth-profiles.json` on first use).
...
Profiles live in `~/.openclawcn/agents/<agentId>/agent/auth-profiles.json` under `profiles`.
