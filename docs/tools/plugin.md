...
- Memory (LanceDB) — bundled long-term memory plugin (auto-recall/capture; set `plugins.slots.memory = "memory-lancedb"`). On Intel Mac (`darwin-x64`), it automatically falls back to a local file backend because upstream LanceDB does not ship a macOS Intel binary.
