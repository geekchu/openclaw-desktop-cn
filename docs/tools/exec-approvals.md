...
- allowlist entries live in host-local `~/.openclawcn/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `openclaw approvals allowlist ...`).
...
`~/.openclawcn/exec-approvals.json` directly.
