...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your OpenClaw config directory (typically `~/.openclawcn/hooks/transforms`).
...
```bash
curl -X POST http://127.0.0.1:28789/hooks/wake \
...
```bash
curl -X POST http://127.0.0.1:28789/hooks/agent \
...
```bash
curl -X POST http://127.0.0.1:28789/hooks/gmail \
  -H 'Authorization: Bearer SECRET' \
  -H 'Content-Type: application/json' \
