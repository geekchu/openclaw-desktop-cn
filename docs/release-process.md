...
| jq | 任意 | 发布脚本需要，`winget install jq` 或 `brew install jq` |
...
> **如果需要重新生成：**
> ```bash
> cargo tauri signer generate -w ~/.tauri/openclaw.key
> ```
...
- 如果生成密钥时设置了密码，构建时还需设置 `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` 环境变量
...
```powershell
# 设置签名私钥环境变量（每次新开终端都需要）
$env:TAURI_SIGNING_PRIVATE_KEY = Get-Content ~/.tauri/openclaw.key -Raw
...
**构建脚本自动完成：** 环境检查 → `pnpm install` → 下载 Node.js 运行时 → `cargo tauri build` → 收集产物到 `dist/installers/`

> 如果忘记设置 `TAURI_SIGNING_PRIVATE_KEY`，构建脚本会输出警告但不会阻断构建。
> 没有签名的安装包**无法用于自动更新**（客户端验签会失败）。
...
| Windows | `src-tauri/target/release/bundle/msi/` | `*.msi`（仅安装用，不参与自动更新） |
| macOS | `src-tauri/target/release/bundle/macos/` | `*.app.tar.gz` + `.sig` |
| macOS | `src-tauri/target/release/bundle/dmg/` | `*.dmg`（仅安装用，不参与自动更新） |
| Linux | `src-tauri/target/release/bundle/appimage/` | `*.AppImage` + `.sig` |

所有产物会被自动复制到 `dist/installers/` 目录。

> **自动更新只使用带 `.sig` 的文件：** Windows 用 NSIS `.exe`，macOS 用 `.app.tar.gz`，Linux 用 `.AppImage`。
> MSI 和 DMG 不参与自动更新流程，仅供用户手动下载安装。
...
| 构建卡住不动 | 签名密钥有密码但未设置 `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` | 设置密码环境变量 |
...
| `jq: command not found` | 未安装 jq | `winget install jq` 或 `brew install jq` |
...
| `scripts/prepare-gateway-bundle.js` | beforeBuildCommand，打包 gateway 代码 |
