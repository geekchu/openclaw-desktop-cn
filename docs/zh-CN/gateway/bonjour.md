...
- 从 `~/.openclawcn/dns/<domain>.db` 提供你选择的域名服务（示例：`openclaw.internal.`）
...
Gateway 网关 WS 端口（默认 `28789`）默认绑定到 loopback。对于局域网/Tailnet
访问，请明确绑定并保持认证启用。

对于仅 Tailnet 的设置：

- 在 `~/.openclawcn/openclaw.json` 中设置 `gateway.bind: "tailnet"`。
