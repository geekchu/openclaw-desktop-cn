...
OpenClaw 会从 `~/.openclawcn/openclaw.json` 读取可选的 <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> 配置。
...
<Tabs>
  ...
  <Tab title="Control UI">
    打开 [http://127.0.0.1:28789](http://127.0.0.1:28789) 并使用 **Config** 标签页。
    Control UI 会根据配置 schema 渲染表单，并提供 **Raw JSON** 编辑器作为后备方式。
  </Tab>
  <Tab title="Direct edit">
    直接编辑 `~/.openclawcn/openclaw.json`。Gateway 网关会监视该文件并自动应用更改（参见[热重载](#config-hot-reload)）。
  </Tab>
</Tabs>
...
Gateway 网关会监视 `~/.openclawcn/openclaw.json` 并自动应用更改 —— 对于大多数设置，无需手动重启。
