...
- 磁盘上的凭证：`ls -l ~/.openclawcn/credentials/whatsapp/<accountId>/creds.json`（mtime 应该是最近的）。
- 会话存储：`ls -l ~/.openclawcn/agents/<agentId>/sessions/sessions.json`（路径可在配置中覆盖）。计数和最近收件人通过 `status` 显示。
...
- Gateway 网关不可达 → 启动它：`openclaw gateway --port 28789`（如果端口被占用则使用 `--force`）。
