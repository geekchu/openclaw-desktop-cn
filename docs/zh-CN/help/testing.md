...
- 配置文件存储：`~/.openclawcn/credentials/`（首选；测试中"配置文件密钥"的含义）
...
- `OPENCLAW_CONFIG_DIR=...`（默认：`~/.openclawcn`）挂载到 `/home/node/.openclawcn`
- `OPENCLAW_WORKSPACE_DIR=...`（默认：`~/.openclawcn/workspace`）挂载到 `/home/node/.openclawcn/workspace`
