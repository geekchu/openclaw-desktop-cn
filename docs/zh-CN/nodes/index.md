...
- **批准**：通过 `~/.openclawcn/exec-approvals.json` 在节点主机上执行。
...
```bash
# 终端 A（保持运行）：转发本地 18790 -> Gateway 网关 127.0.0.1:28789
...
- 令牌是 Gateway 网关配置中的 `gateway.auth.token`（Gateway 网关主机上的 `~/.openclawcn/openclaw.json`）。
...
- 在 `openclaw node run` / `openclaw node install` 上使用 `--display-name`（持久化在节点上的 `~/.openclawcn/node.json` 中）。
...
批准存储在节点主机的 `~/.openclawcn/exec-approvals.json` 中。
...
- 在无头节点主机上，`system.run` 受 exec 批准限制（`~/.openclawcn/exec-approvals.json`）。
...
- 节点主机将其节点 id、令牌、显示名称和 Gateway 网关连接信息存储在 `~/.openclawcn/node.json` 中。
- Exec 批准通过 `~/.openclawcn/exec-approvals.json` 在本地执行
