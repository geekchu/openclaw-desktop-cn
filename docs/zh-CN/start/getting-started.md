...
最快聊天：打开 Control UI（无需渠道设置）。运行 `openclaw dashboard` 并在浏览器中聊天，或在 Gateway 网关主机上打开 `http://127.0.0.1:28789/`。文档：[Dashboard](/web/dashboard) 和 [Control UI](/web/control-ui)。
...
- OAuth 凭证（旧版导入）：`~/.openclawcn/credentials/oauth.json`
- 认证配置文件（OAuth + API 密钥）：`~/.openclawcn/agents/<agentId>/agent/auth-profiles.json`
...
Dashboard（local loopback）：`http://127.0.0.1:28789/`
