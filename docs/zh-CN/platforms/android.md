...
Android 直接连接到 Gateway 网关 WebSocket（默认 `ws://<host>:28789`）并使用 Gateway 网关拥有的配对。
...
- 在 Gateway 网关主机的 `~/.openclawcn/openclaw.json` 中设置 `gateway.bind: "tailnet"`。
...
1. 在 Gateway 网关主机上创建 `~/.openclawcn/workspace/canvas/index.html`。
