...
打开：`http://<tailscale-ip>:28789`（需要 token）。
...
- `~/.openclawcn/` — 配置、凭证、会话数据
- `~/.openclawcn/workspace/` — 工作区（`SOUL.md`、memory 等）

这些内容在重启后仍会保留。请定期备份：

```bash
tar -czvf openclaw-backup.tar.gz ~/.openclaw ~/.openclawcn/workspace
```
