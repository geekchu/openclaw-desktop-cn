...
**推荐：** 运行 `openclaw configure --section web`。它会把密钥存储到 `~/.openclawcn/openclaw.json` 的 `plugins.entries.brave.config.webSearch.apiKey`。

**环境变量替代方案：** 在 Gateway 网关进程环境中设置 `BRAVE_API_KEY`。对于 Gateway 网关安装，将其放在 `~/.openclawcn/.env`（或你的服务环境）中。参见[环境变量](/help/faq#how-does-openclaw-load-environment-variables)。
...
**环境变量替代方案：** 在 Gateway 网关环境中设置 `OPENROUTER_API_KEY` 或 `PERPLEXITY_API_KEY`。对于 Gateway 网关安装，将其放在 `~/.openclawcn/.env` 中。
