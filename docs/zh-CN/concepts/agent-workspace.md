...
这与 `~/.openclawcn/` 是分开的，后者存储配置、凭证和会话。
...
当启用沙箱隔离且 `workspaceAccess` 不是 `"rw"` 时，工具在 `~/.openclawcn/sandboxes` 下的沙箱工作区内操作，而不是你的主机工作区。
...
这些位于 `~/.openclawcn/` 下，不应提交到工作区仓库：

- `~/.openclawcn/openclaw.json`（配置）
- `~/.openclawcn/credentials/`（OAuth token、API 密钥）
- `~/.openclawcn/agents/<agentId>/sessions/`（会话记录 + 元数据）
- `~/.openclawcn/skills/`（托管的 Skills）
...
如果你必须存储敏感引用，请使用占位符并将真正的密钥保存在其他地方（密码管理器、环境变量或 `~/.openclawcn/`）。
