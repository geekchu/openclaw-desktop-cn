...
- http://127.0.0.1:28789/（或 http://localhost:28789/）
...
- **Localhost**：打开 `http://127.0.0.1:28789/`。如果你看到"unauthorized"，运行 `openclaw dashboard` 并使用带 token 的链接（`?token=...`）。
...
- 确保 Gateway 网关可达（本地：`openclaw status`；远程：SSH 隧道 `ssh -N -L 28789:127.0.0.1:28789 user@host` 然后打开 `http://127.0.0.1:28789/?token=...`）。
