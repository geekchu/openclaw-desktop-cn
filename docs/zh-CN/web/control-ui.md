...
- 配置：查看/编辑 `~/.openclawcn/openclaw.json`（`config.get`、`config.set`）
...
- `http://<tailscale-ip>:28789/`（或你配置的 `gateway.controlUi.basePath`）
...
- `http://127.0.0.1:28789/`（在 Gateway 网关主机上）
...
然后将 UI 指向你的 Gateway 网关 WS URL（例如 `ws://127.0.0.1:28789`）。
