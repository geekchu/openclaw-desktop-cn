...
- **💾 session-memory**：当你发出 `/new` 时，将会话上下文保存到你的智能体工作区（默认是 `~/.openclawcn/workspace/memory/`）
- **📎 bootstrap-extra-files**：在 `agent:bootstrap` 期间，从已配置的 glob/路径模式中注入额外的工作区引导文件
- **📝 command-logger**：将所有命令事件记录到 `~/.openclawcn/logs/commands.log`
...
2. **托管 hooks**：`~/.openclawcn/hooks/`（用户安装，在各工作区之间共享）
...
Hook 包可以携带依赖；它们会安装到 `~/.openclawcn/hooks/<id>` 下。
...
- **托管 hooks**（`~/.openclawcn/hooks/`）：跨工作区共享
...
**输出**：`<workspace>/memory/YYYY-MM-DD-slug.md`（默认是 `~/.openclawcn/workspace`）
...
**输出**：`~/.openclawcn/logs/commands.log`
