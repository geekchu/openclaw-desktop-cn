...
<AccordionGroup>
  ...
  <Accordion title="Recommended way to install and set up OpenClaw">
    ...
    The wizard can also build UI assets automatically. After onboarding, you typically run the Gateway on port **28789**.
  ...
  <Accordion title="How do I authenticate the dashboard (token) on localhost vs remote?">
    ...
    **Not on localhost:**
    ...
    - **Tailnet bind**: run `openclaw gateway --bind tailnet --token "<token>"`, open `http://<tailscale-ip>:28789/`, paste token in dashboard settings.
    - **SSH tunnel**: `ssh -N -L 28789:127.0.0.1:28789 user@host` then open `http://127.0.0.1:28789/` and paste the token in Control UI settings.
  ...
  <Accordion title="Can I switch between npm and git installs later?">
    ...
    (`~/.openclaw`) and workspace (`~/.openclawcn/workspace`) stay untouched.
...
<AccordionGroup>
  ...
  <Accordion title="Can I load skills from a custom folder?">
    Yes. Add extra directories via `skills.load.extraDirs` in `~/.openclawcn/openclaw.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.openclawcn/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which OpenClaw treats as `<workspace>/skills` on the next session.
  </Accordion>
  ...
  <Accordion title="Do you have a Notion or HeyGen integration?">
    ...
    Native installs land in the active workspace `skills/` directory. For shared skills across agents, place them in `~/.openclawcn/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawHub](/tools/clawhub).
...
<AccordionGroup>
  ...
  <Accordion title="How do I set up Tailscale on a VPS and connect from my Mac?">
    ...
    4. **Use the tailnet hostname**
       ...
       - Gateway WS: `ws://your-vps.tailnet-xxxx.ts.net:28789`
...
<AccordionGroup>
  <Accordion title="What port does the Gateway use?">
    ...
    ```
    --port > OPENCLAW_GATEWAY_PORT > gateway.port > default 28789
    ```
  ...
  <Accordion title='What does "another gateway instance is already listening" mean?'>
    OpenClaw enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:28789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.
  ...
  <Accordion title='The Control UI says "unauthorized" (or keeps reconnecting). What now?'>
    ...
    - If remote, tunnel first: `ssh -N -L 28789:127.0.0.1:28789 user@host` then open `http://127.0.0.1:28789/`.
  ...
  <Accordion title='What does "invalid handshake" / code 1008 mean?'>
    ...
    1. Use the WS URL: `ws://<host>:28789` (or `wss://...` if HTTPS).
