...
function tauriBuild() {
  logStep(3, `Tauri 构建 (${buildProfile})`);

  const debugFlag = isDebug ? " --debug" : "";
  const cmd = `cargo tauri build${debugFlag}`;
...
function collectArtifacts() {
  ...
  const bundleDir = join(tauriDir, "target", buildProfile, "bundle");
