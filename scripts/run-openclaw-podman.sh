...
HOST_GATEWAY_PORT="${OPENCLAW_PODMAN_GATEWAY_HOST_PORT:-${OPENCLAW_GATEWAY_PORT:-28789}}"
...
podman run --pull="$PODMAN_PULL" -d --replace \
  ...
  -v "$CONFIG_DIR:/home/node/.openclaw:rw" \
  -v "$WORKSPACE_DIR:/home/node/.openclaw/workspace:rw" \
  -p "${HOST_GATEWAY_PORT}:28789" \
  -p "${HOST_BRIDGE_PORT}:18790" \
  "$OPENCLAW_IMAGE" \
  node dist/index.js gateway --bind "$GATEWAY_BIND" --port 28789
