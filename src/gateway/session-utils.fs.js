...
export function attachOpenClawTranscriptMeta(message, meta) {
    if (!message || typeof message !== "object" || Array.isArray(message)) {
        return message;
    }
    const record = message;
    const existing = record.__openclaw && typeof record.__openclaw === "object" && !Array.isArray(record.__openclaw)
        ? record.__openclaw
        : {};
    return {
        ...record,
...
function extractLatestUsageFromTranscriptChunk(chunk) {
    ...
    for (const line of lines) {
        try {
            ...
            const usageRaw = message.usage && typeof message.usage === "object" && !Array.isArray(message.usage)
                ? message.usage
            ...
            const modelProvider = typeof message.provider === "string"
            ...
            const hasMeaningfulUsage = hasNonzeroUsage(usage) ||
...
export function readLatestSessionUsageFromTranscript(sessionId, storePath, sessionFile, agentId) {
    const filePath = findExistingTranscriptPath(sessionId, storePath, sessionFile, agentId);
    if (!filePath) {
        return null;
    }
