import { createGatewayCredentialPlan, readGatewayPasswordEnv, readGatewayTokenEnv, trimCredentialToUndefined, trimToUndefined, } from "./credential-planner.js";
...
export class GatewaySecretRefUnavailableError extends Error {
    code = GATEWAY_SECRET_REF_UNAVAILABLE_ERROR_CODE;
    path;
    constructor(path) {
        super([
...
export function isGatewaySecretRefUnavailableError(error, expectedPath) {
...
export function resolveGatewayCredentialsFromValues(params) {
    ...
    const password = passwordPrecedence === "config-first" // pragma: allowlist secret
...
function resolveLocalGatewayCredentials(params) {
    ...
    const localPasswordCanWin = params.plan.authMode === "password" ||
        (params.plan.authMode !== "token" &&
            params.plan.authMode !== "none" &&
            params.plan.authMode !== "trusted-proxy" &&
            !localResolved.token);
    const localTokenCanWin = params.plan.authMode === "token" ||
        (params.plan.authMode !== "password" &&
            params.plan.authMode !== "none" &&
            params.plan.authMode !== "trusted-proxy" &&
            !localResolved.password);
    if (params.plan.localToken.refPath &&
    ...
    if (params.plan.localPassword.refPath &&
...
function resolveRemoteGatewayCredentials(params) {
    const token = params.remoteTokenFallback === "remote-only"
        ? params.plan.remoteToken.value
    ...
    const password = params.remotePasswordFallback === "remote-only" // pragma: allowlist secret
        ? params.plan.remotePassword.value
    ...
    const localPasswordFallback = params.remotePasswordFallback === "remote-only" ? undefined : params.plan.localPassword.value; // pragma: allowlist secret
    if (params.plan.remoteToken.refPath &&
        !token &&
        !params.plan.envToken &&
        !localTokenFallback &&
        !password) {
    ...
    if (params.plan.remotePassword.refPath &&
        !password &&
        !params.plan.envPassword &&
...
export function resolveGatewayCredentialsFromConfig(params) {
    ...
    const localTokenPrecedence = params.localTokenPrecedence ??
