...
describe("resolveGatewayRuntimeConfig", () => {
  describe("trusted-proxy auth mode", () => {
    ...
    it.each([
      ...
      const result = await resolveGatewayRuntimeConfig({ cfg, port: 28789 });
    ...
    it.each([
      ...
      await expect(resolveGatewayRuntimeConfig({ cfg, port: 28789 })).rejects.toThrow(
        expectedMessage,
      );
    });
  });
  ...
  describe("token/password auth modes", () => {
    ...
    it.each([
      ...
      await expect(resolveGatewayRuntimeConfig({ cfg, port: 28789, host })).rejects.toThrow(
        expectedMessage,
      );
    });
