...
const loadConfig = vi.fn(() => ({
  gateway: {
    mode: "remote",
    remote: { url: "ws://remote.example:28789", token: "rtok" },
    auth: { token: "ltok" },
  },
}));
const resolveGatewayPort = vi.fn(() => 28789);
  ...
  it("infers SSH target from gateway.remote.url and ssh config", async () => {
    ...
    try {
      ...
      loadConfig.mockReturnValueOnce({
        gateway: {
          mode: "remote",
          remote: { url: "ws://peters-mac-studio-1.sheep-coho.ts.net:28789", token: "rtok" },
        },
      });
  ...
  it("falls back to host-only when USER is missing and ssh config is unavailable", async () => {
    ...
    try {
      ...
      loadConfig.mockReturnValueOnce({
        gateway: {
          mode: "remote",
          remote: { url: "ws://studio.example:28789", token: "rtok" },
        },
      });
