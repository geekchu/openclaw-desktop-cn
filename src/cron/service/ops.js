...
export async function listPage(state, opts) {
    return await locked(state, async () => {
        await ensureLoadedForRead(state);
...
async function inspectManualRunPreflight(state, id, mode) {
    return await locked(state, async () => {
        ...
        return { ok: true, runnable: true, job, now };
    });
}
...
export async function enqueueRun(state, id, mode) {
    ...
    void enqueueCommandInLane(CommandLane.Cron, async () => {
        ...
        if (result.ok && "ran" in result && !result.ran) {
            state.deps.log.info({ jobId: id, runId, reason: result.reason }, "cron: queued manual run skipped before execution");
        }
        return result;
    }, {
        warnAfterMs: 5_000,
        onWait: (waitMs, queuedAhead) => {
            state.deps.log.warn({ jobId: id, runId, waitMs, queuedAhead }, "cron: queued manual run waiting for an execution slot");
        },
    }).catch((err) => {
