...
import { computeJobPreviousRunAtMs, computeJobNextRunAtMs, nextWakeAtMs, recomputeNextRunsForMaintenance, recordScheduleComputeError, resolveJobPayloadTextForMain, } from "./jobs.js";
 ...
 */
export function applyJobResult(state, job, result, opts) {
...
function isErrorBackoffPending(job, nowMs) {
    ...
    const consecutiveErrors = typeof consecutiveErrorsRaw === "number" && Number.isFinite(consecutiveErrorsRaw)
...
async function planStartupCatchup(state, opts) {
    ...
    return locked(state, async () => {
        ...
        if (startupCandidates.length > 0) {
            state.deps.log.info({ count: startupCandidates.length, jobIds: startupCandidates.map((j) => j.id) }, "cron: running missed jobs after restart");
        }
