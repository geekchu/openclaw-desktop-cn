...
export async function resolveDeliveryTarget(cfg, agentId, jobPayload) {
    ...
    if (!preliminary.channel) {
        if (preliminary.lastChannel) {
            ...
            try {
                ...
                channelResolutionError = new Error(`${detail} Set delivery.channel explicitly or use a main session with a previous channel.`);
            }
        }
    }
    ...
    const explicitAccountId = typeof jobPayload.accountId === "string" && jobPayload.accountId.trim()
