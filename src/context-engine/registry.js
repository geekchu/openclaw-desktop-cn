...
const LEGACY_COMPAT_PARAMS = ["sessionKey", "prompt"];
...
function issueRejectsLegacyCompatKeyStrictly(issue, key) {
    ...
    if (issueRecord.code === "unrecognized_keys" &&
 ...
 */
export function registerContextEngineForOwner(id, factory, owner, opts) {
 ...
 */
export function registerContextEngine(id, factory) {
 ...
 */
export function getContextEngineFactory(id) {
    return getContextEngineRegistryState().engines.get(id)?.factory;
}
/**
 * List all registered engine ids.
 */
export function listContextEngineIds() {
 ...
 */
export async function resolveContextEngine(config) {
