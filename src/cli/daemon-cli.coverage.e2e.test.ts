...
vi.mock("../infra/ports.js", () => ({
  ...
  formatPortDiagnostics: () => ["Port 28789 is already in use."],
}));
...
describe("daemon-cli coverage", () => {
  ...
  it("installs the daemon when requested", async () => {
    ...
    await program.parseAsync(["daemon", "install", "--port", "28789"], {
      from: "user",
    });
  ...
  it("installs the daemon with json output", async () => {
    ...
    await program.parseAsync(["daemon", "install", "--port", "28789", "--json"], {
      from: "user",
    });
