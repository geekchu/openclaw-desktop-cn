...
function isDirectRuntimeWebTargetPath(path) {
    return (path === "tools.web.fetch.firecrawl.apiKey" ||
...
async function resolveTargetSecretLocally(params) {
    ...
    try {
        ...
        if (!enforcesResolvedSecrets(params.mode)) {
            params.localResolutionDiagnostics.push(`${params.commandName}: failed to resolve ${params.target.path} locally (${describeUnknownError(error)}).`);
        }
    }
}
export async function resolveCommandSecretRefsViaGateway(params) {
    ...
    try {
        ...
        if (isUnsupportedSecretsResolveError(err)) {
            throw new Error(`${params.commandName}: active gateway does not support secrets.resolve (${describeUnknownError(err)}). Update the gateway or run without SecretRefs.`, { cause: err });
        }
