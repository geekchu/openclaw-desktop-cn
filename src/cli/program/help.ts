...
const EXAMPLES = [
  ...
  ["openclaw gateway --port 28789", "Run the WebSocket Gateway locally."],
...
export function configureProgramHelp(program: Command, ctx: ProgramContext) {
  program
    ...
    .option(
      "--dev",
      "Dev profile: isolate state under ~/.openclawcn-dev, default gateway port 19001, and shift derived ports (browser/canvas)",
    )
    .option(
      "--profile <name>",
      "Use a named profile (isolates OPENCLAW_STATE_DIR/OPENCLAW_CONFIG_PATH under ~/.openclawcn-<name>)",
    )
    .option(
      "--log-level <level>",
