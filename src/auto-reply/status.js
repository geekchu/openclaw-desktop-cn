...
const readUsageFromSessionLog = (sessionId, sessionEntry, agentId, sessionKey, storePath) => {
    // Transcripts are stored at the session file path (fallback: ~/.openclawcn/sessions/<SessionId>.jsonl)
    if (!sessionId) {
        return undefined;
    }
    let logPath;
...
export function buildStatusMessage(args) {
    ...
    if (runtimeModelRaw && !runtimeProviderRaw && runtimeModelRaw.includes("/")) {
        ...
        const fallbackMatchesRuntimeModel = initialFallbackState.active &&
        ...
        // slash ids.
        if ((fallbackMatchesRuntimeModel || runtimeMatchesSelectedModel) &&
    ...
    const persistedContextTokens = typeof entry?.contextTokens === "number" && entry.contextTokens > 0
        ? entry.contextTokens
        : undefined;
    const explicitRuntimeContextTokens = typeof args.runtimeContextTokens === "number" && args.runtimeContextTokens > 0
        ? args.runtimeContextTokens
        : undefined;
    const explicitConfiguredContextTokens = typeof args.explicitConfiguredContextTokens === "number" &&
        args.explicitConfiguredContextTokens > 0
        ? args.explicitConfiguredContextTokens
        : undefined;
    const cappedConfiguredContextTokens = typeof explicitConfiguredContextTokens === "number"
        ...
        ? (explicitRuntimeContextTokens ??
            (() => {
                if (persistedContextTokens !== undefined) {
                    const persistedLooksSelectedWindow = typeof selectedContextTokens === "number" &&
                        persistedContextTokens === selectedContextTokens;
                    const activeWindowDiffersFromSelected = typeof selectedContextTokens === "number" &&
                        typeof activeContextTokens === "number" &&
                        activeContextTokens !== selectedContextTokens;
                    const explicitConfiguredMatchesPersisted = typeof explicitConfiguredContextTokens === "number" &&
                        explicitConfiguredContextTokens === persistedContextTokens;
                    if (persistedLooksSelectedWindow &&
...
export function buildToolsMessage(result, options) {
