...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".openclawcn", "agents", "main", "sessions"), {
      recursive: true,
    });
    ...
    process.env.OPENCLAW_AGENT_DIR = path.join(home, ".openclawcn", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".openclawcn", "agent");
