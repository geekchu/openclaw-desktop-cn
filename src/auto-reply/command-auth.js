...
import { INTERNAL_MESSAGE_CHANNEL, isInternalMessageChannel, normalizeMessageChannel, } from "../utils/message-channel.js";
function resolveProviderFromContext(ctx, cfg) {
    ...
    const direct = normalizeAnyChannelId(explicitMessageChannel ?? undefined) ??
    ...
    for (const candidate of candidates) {
        ...
        const normalized = normalizeAnyChannelId(normalizedCandidateChannel ?? undefined) ??
    ...
    return {
        ...
        hadResolutionError: inferredProviders.droppedResolutionError ||
...
function resolveFallbackAllowFrom(params) {
    ...
    const accountCfg = resolveFallbackAccountConfig(channelCfg?.accounts, params.accountId) ??
...
function resolveFallbackDefaultAccountConfig(channelCfg) {
    ...
    const preferred = resolveFallbackAccountConfig(accounts, channelCfg?.defaultAccount) ??
...
export function resolveCommandAuthorization(params) {
    ...
    const allowAll = !resolvedAllowFrom.hadResolutionError &&
    ...
    const senderIsOwnerByScope = isInternalMessageChannel(ctx.Provider) &&
    ...
    if (commandsAllowFromList !== null || (providerResolutionError && commandsAllowFromConfigured)) {
        ...
        const commandsAllowAll = !providerResolutionError &&
