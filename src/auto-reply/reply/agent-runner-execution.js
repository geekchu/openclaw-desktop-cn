...
import { BILLING_ERROR_USER_MESSAGE, isCompactionFailureError, isContextOverflowError, isBillingErrorMessage, isLikelyContextOverflowError, isOverloadedErrorMessage, isRateLimitErrorMessage, isTransientHttpError, sanitizeUserFacingText, } from "../../agents/pi-embedded-helpers.js";
...
export async function runAgentTurnWithFallback(params) {
    ...
    if (runResult) {
        ...
        if (!hasNonErrorContent) {
            ...
            const rawErrorPayloadText = runResult.payloads?.find((p) => p.isError && p.text?.trim() && !p.text.startsWith("⚠️"))
                ?.text ?? "";
