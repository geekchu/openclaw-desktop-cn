...
function parseChoices(raw, maxItems) {
    return raw
        .split(",")
...
export function hasSlackDirectives(text) {
    SLACK_DIRECTIVE_RE.lastIndex = 0;
    return SLACK_DIRECTIVE_RE.test(text);
}
export function parseSlackDirectives(payload) {
    ...
    for (const match of text.matchAll(SLACK_DIRECTIVE_RE)) {
        ...
        const block = directiveType.toLowerCase() === "slack_buttons"
