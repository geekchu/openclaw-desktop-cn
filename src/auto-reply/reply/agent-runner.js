...
import { resolveAgentIdFromSessionKey, resolveSessionFilePath, resolveSessionFilePathOptions, resolveSessionTranscriptPath, updateSessionStore, updateSessionStoreEntry, } from "../../config/sessions.js";
...
import { appendUnscheduledReminderNote, hasSessionRelatedCronJobs, hasUnbackedReminderCommitment, } from "./agent-runner-reminder-guard.js";
...
export async function runReplyAgent(params) {
    ...
    const resetSession = async ({ failureLabel, buildLogMessage, cleanupTranscripts, }) => {
        ...
        if (cleanupTranscripts && prevSessionId) {
            ...
            const resolved = resolveSessionFilePath(prevSessionId, prevEntry, resolveSessionFilePathOptions({ agentId, storePath }));
            if (resolved) {
    ...
    try {
        ...
        const coveredByExistingCron = hasReminderCommitment && successfulCronAdds === 0
            ? await hasSessionRelatedCronJobs({
                cronStorePath: cfg.cron?.store,
                sessionKey,
            })
            : false;
        const guardedReplyPayloads = hasReminderCommitment && successfulCronAdds === 0 && !coveredByExistingCron
        ...
        if (autoCompactionCount > 0) {
            ...
            const refreshedSessionEntry = sessionKey && activeSessionStore ? activeSessionStore[sessionKey] : undefined;
