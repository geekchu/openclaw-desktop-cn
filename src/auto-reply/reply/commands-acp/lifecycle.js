...
async function bindSpawnedAcpSessionToCurrentConversation(params) {
    ...
    try {
        ...
        return {
            ok: false,
            error: message || `Failed to bind the current ${channel} conversation to the new ACP session.`,
        };
    }
}
...
export async function handleAcpSpawnAction(params, restTokens) {
    ...
    if (binding) {
        ...
        const placementLabel = resolveAcpBindingLabelNoun({
            conversationId: currentConversationId,
            placement: currentConversationId && boundConversationId === currentConversationId
                ? "current"
                : "child",
    ...
    const shouldPinBindingNotice = binding?.conversation.channel === "telegram" &&
