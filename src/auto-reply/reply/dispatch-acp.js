...
import { isSessionIdentityPending, resolveSessionIdentityFromMeta, } from "../../acp/runtime/session-identity.js";
...
import { normalizeAttachmentPath, normalizeAttachments, } from "../../media-understanding/attachments.normalize.js";
...
export function shouldBypassAcpDispatchForCommand(ctx, cfg) {
...
async function maybeUnbindStaleBoundConversations(params) {
    ...
    try {
        ...
        if (removed.length > 0) {
            logVerbose(`dispatch-acp: removed ${removed.length} stale bound conversation(s) for ${params.targetSessionKey} after ${params.error.code}: ${params.error.message}`);
        }
    }
    catch (error) {
...
async function finalizeAcpTurnOutput(params) {
    ...
    const shouldDeliverTextFallback = ttsMode !== "all" &&
...
export async function tryDispatchAcpReply(params) {
    ...
    const identityPendingBeforeTurn = isSessionIdentityPending(resolveSessionIdentityFromMeta(acpResolution.kind === "ready" ? acpResolution.meta : undefined));
    const shouldEmitResolvedIdentityNotice = identityPendingBeforeTurn &&
    ...
    const resolvedAcpAgent = acpResolution.kind === "ready"
        ? (acpResolution.meta.agent?.trim() ||
            params.cfg.acp?.defaultAgent?.trim() ||
            resolveAgentIdFromSessionKey(canonicalSessionKey)).trim()
