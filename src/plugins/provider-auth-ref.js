...
import { formatExecSecretRefIdValidationMessage, isValidExecSecretRefId, isValidFileSecretRefId, resolveDefaultSecretProviderAlias, } from "../secrets/ref-contract.js";
...
export function extractEnvVarFromSourceLabel(source) {
...
export function resolveRefFallbackInput(params) {
    ...
    if (!fallbackEnvVar) {
        throw new Error(`No default environment variable mapping found for provider "${params.provider}". Set a provider-specific env var, or re-run setup in an interactive terminal to configure a ref.`);
    }
    const env = params.env ?? process.env;
    const value = env[fallbackEnvVar]?.trim();
    if (!value) {
        throw new Error(`Environment variable "${fallbackEnvVar}" is required for --secret-input-mode ref in non-interactive setup.`);
    }
    return {
        ref: {
            source: "env",
...
async function promptEnvSecretRefForSetup(params) {
    ...
    if (!envVar) {
        throw new Error(`No valid environment variable name provided for provider "${params.provider}".`);
    }
    ...
    await params.prompter.note(params.copy?.envValidatedMessage?.(envVar) ??
        `Validated environment variable ${envVar}. OpenClaw will store a reference, not the key value.`, "Reference validated");
...
async function promptProviderSecretRefForSetup(params) {
    ...
    if (externalProviders.length === 0) {
        await params.prompter.note(params.copy?.noProvidersMessage ??
            "No file/exec secret providers are configured yet. Add one under secrets.providers, or select Environment variable.", "No providers configured");
    ...
    const selectedProvider = await params.prompter.select({
        ...
        initialValue: externalProviders.find(([providerName]) => providerName === defaultProvider)?.[0] ??
    ...
    try {
        ...
        await params.prompter.note(params.copy?.providerValidatedMessage?.(selectedProvider, id, providerEntry.source) ??
            `Validated ${providerEntry.source} reference ${selectedProvider}:${id}. OpenClaw will store a reference, not the key value.`, "Reference validated");
        return { ref, resolvedValue };
    }
    catch (error) {
...
export async function promptSecretRefForSetup(params) {
    ...
    let sourceChoice = "env"; // pragma: allowlist secret
    while (true) {
        ...
        const source = sourceRaw === "provider" ? "provider" : "env";
        sourceChoice = source;
        if (source === "env") {
