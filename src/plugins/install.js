...
import { resolveSafeInstallDir, safeDirName, safePathSegmentHashed, unscopedPackageName, } from "../infra/install-safe-path.js";
import { CONFIG_DIR, resolveUserPath } from "../utils.js";
import { resolvePackageExtensionEntries, } from "./manifest.js";
...
function matchesExpectedPluginId(params) {
    ...
    return (!params.manifestPluginId &&
...
async function detectNativePackageInstallSource(packageDir) {
    ...
    try {
        const manifest = await runtime.readJsonFile(manifestPath);
...
async function installPluginFromPackageDir(params) {
    ...
    const manifestPluginId = ocManifestResult.ok && ocManifestResult.manifest.id
    ...
    if (manifestPluginId && manifestPluginId !== npmPluginId) {
        logger.info?.(`Plugin manifest id "${manifestPluginId}" differs from npm package name "${npmPluginId}"; using manifest id as the config key.`);
    }
