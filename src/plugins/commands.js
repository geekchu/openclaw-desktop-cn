...
import { clearPluginCommands, clearPluginCommandsForPlugin, getPluginCommandSpecs, listPluginInvocationKeys, registerPluginCommand, validateCommandName, validatePluginCommandDefinition, } from "./command-registration.js";
import { pluginCommands, setPluginCommandRegistryLocked, } from "./command-registry-state.js";
...
export function matchPluginCommand(commandBody) {
    ...
    const command = pluginCommands.get(key) ??
        Array.from(pluginCommands.values()).find((candidate) => listPluginInvocationNames(candidate).includes(key));
    if (!command) {
        return null;
    }
...
function resolveBindingConversationFromCommand(params) {
    ...
    if (params.channel === "telegram") {
        ...
        const rawTarget = params.to && params.to.startsWith("slash:")
    ...
    if (params.channel === "discord") {
        const source = params.to?.startsWith("slash:") || !params.to?.trim()
            ? (params.from ?? params.to)
            : params.to;
        ...
        return {
            channel: "discord",
            accountId,
            conversationId: "conversationId" in target
