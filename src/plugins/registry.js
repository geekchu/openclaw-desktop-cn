  ...
  return (event, ctx) => {
    ...
    if (result && typeof result === "object" && "then" in result) {
      return Promise.resolve(result).then(
        (resolved) => stripPromptMutationFieldsFromLegacyHookResult(resolved)
      );
    }
    return stripPromptMutationFieldsFromLegacyHookResult(result);
  };
};
import { createEmptyPluginRegistry as createEmptyPluginRegistry2 } from "./registry-empty.js";
function createPluginRegistry(registryParams) {
  ...
  const registerHook = (record, events, handler, opts, config) => {
    ...
    const existingHook = registry.hooks.find((entry2) => entry2.entry.hook.name === name);
    if (existingHook) {
      pushDiagnostic({
        level: "error",
        pluginId: record.id,
  ...
  const registerCli = (record, registrar, opts) => {
    ...
    const existing = registry.cliRegistrars.find(
      (entry) => entry.commands.some((command) => commands.includes(command))
    );
    if (existing) {
...
export {
  createEmptyPluginRegistry2 as createEmptyPluginRegistry,
