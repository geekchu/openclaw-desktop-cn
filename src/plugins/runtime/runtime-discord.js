import { discordMessageActions, getThreadBindingManager, resolveThreadBindingIdleTimeoutMs, resolveThreadBindingInactivityExpiresAt, resolveThreadBindingMaxAgeExpiresAt, resolveThreadBindingMaxAgeMs, setThreadBindingIdleTimeoutBySessionKey, setThreadBindingMaxAgeBySessionKey, unbindThreadBindingsBySessionKey, } from "../../plugin-sdk/discord.js";
...
const loadRuntimeDiscordOps = createLazyRuntimeSurface(() => import("./runtime-discord-ops.runtime.js"), ({ runtimeDiscordOps }) => runtimeDiscordOps);
...
const listDirectoryGroupsLiveLazy = bindDiscordRuntimeMethod((runtimeDiscordOps) => runtimeDiscordOps.listDirectoryGroupsLive);
const listDirectoryPeersLiveLazy = bindDiscordRuntimeMethod((runtimeDiscordOps) => runtimeDiscordOps.listDirectoryPeersLive);
const probeDiscordLazy = bindDiscordRuntimeMethod((runtimeDiscordOps) => runtimeDiscordOps.probeDiscord);
const resolveChannelAllowlistLazy = bindDiscordRuntimeMethod((runtimeDiscordOps) => runtimeDiscordOps.resolveChannelAllowlist);
const resolveUserAllowlistLazy = bindDiscordRuntimeMethod((runtimeDiscordOps) => runtimeDiscordOps.resolveUserAllowlist);
...
export function createRuntimeDiscord() {
    return {
        ...
        typing: {
            ...
            start: async ({ channelId, accountId, cfg, intervalMs }) => await createDiscordTypingLease({
                ...
                cfg,
                intervalMs,
                pulse: async ({ channelId, accountId, cfg }) => void (await sendTypingDiscordLazy(channelId, { accountId, cfg })),
            }),
        },
        conversationActions: {
