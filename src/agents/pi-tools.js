...
import { assertRequiredParams, createHostWorkspaceEditTool, createHostWorkspaceWriteTool, createOpenClawReadTool, createSandboxedEditTool, createSandboxedReadTool, createSandboxedWriteTool, normalizeToolParams, patchToolSchemaForClaudeCompatibility, wrapToolMemoryFlushAppendOnlyWrite, wrapToolWorkspaceRootGuard, wrapExecToolPathGuard, wrapToolWorkspaceRootGuardWithOptions, wrapToolParamNormalization, } from "./pi-tools.read.js";
...
export function createOpenClawCodingTools(options) {
    ...
    // Self-modification protection: deny the running OpenClaw bundle directory
    // (only when it differs from the workspace — dev mode should not be blocked).
    const selfProtectDirs = [];
    const bundleDir = process.env.OPENCLAW_GATEWAY_BUNDLE_DIR?.trim();
    if (bundleDir) {
        const resolvedBundle = nodePath.resolve(bundleDir);
        const resolvedWorkspace = nodePath.resolve(workspaceRoot);
        // Only deny if the bundle dir is NOT the workspace (i.e. production mode)
        if (resolvedBundle.toLowerCase() !== resolvedWorkspace.toLowerCase()) {
            selfProtectDirs.push(resolvedBundle);
        }
    }
    ...
    const tools = [
        ...
        ...(applyPatchTool
            ? [
                workspaceOnly
                    ? wrapToolWorkspaceRootGuard(applyPatchTool, workspaceRoot, undefined, selfProtectDirs)
                    : applyPatchTool,
            ]
            : []),
        ...(execSecurity !== "deny"
            ? [
                workspaceOnly
                    ? wrapExecToolPathGuard(execTool, workspaceRoot, undefined, selfProtectDirs)
                    : execTool,
                processTool,
            ]
            : []),
    ...
    const toolsForMemoryFlush = isMemoryFlushRun && memoryFlushWritePath
        ...
        : tools;
    const toolsForMessageProvider = applyMessageProviderToolPolicy(toolsForMemoryFlush, options?.messageProvider);
