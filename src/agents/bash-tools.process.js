        ...
        execute: async (_toolCallId, args, _signal, _onUpdate) => {
            ...
            switch (params.action) {
                ...
                case "send-keys": {
                    ...
                    const cursorKeyMode = resolved.session.cursorKeyMode === "unknown"
                        ? undefined
                    ...
                    return runningSessionResult(resolved.session, `Sent ${data.length} bytes to session ${params.sessionId}.` +
                        (warnings.length ? `\nWarnings:\n- ${warnings.join("\n- ")}` : ""));
                }
                case "submit": {
                    const resolved = resolveBackgroundedWritableStdin();
