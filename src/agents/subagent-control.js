...
import { clearSubagentRunSteerRestart, countPendingDescendantRuns, getLatestSubagentRunByChildSessionKey, getSubagentSessionRuntimeMs, getSubagentSessionStartedAt, listSubagentRunsForController, markSubagentRunTerminated, markSubagentRunForSteerRestart, replaceSubagentRunAfterSteer, } from "./subagent-registry.js";
import { extractAssistantText, resolveInternalSessionKey, resolveMainSessionAlias, stripToolMessages, } from "./tools/sessions-helpers.js";
...
export function buildSubagentList(params) {
    ...
    const buildListEntry = (entry, runtimeMs) => {
        ...
        const childSessions = Array.from(new Set(listSubagentRunsForController(entry.childSessionKey)
            .map((run) => run.childSessionKey?.trim())
            .filter((childSessionKey) => Boolean(childSessionKey))
...
async function killSubagentRun(params) {
    ...
    if (resolved.entry) {
        try {
            ...
            logVerbose(`subagents control kill: failed to persist abortedLastRun for ${childSessionKey}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
...
export async function sendControlledSubagentMessage(params) {
    ...
    let runId = idempotencyKey;
    try {
        const historyBefore = await subagentControlDeps.callGateway({
            method: "chat.history",
            params: { sessionKey: targetSessionKey, limit: SUBAGENT_REPLY_HISTORY_LIMIT },
        });
        const baselineReply = resolveLatestAssistantReplySnapshot(stripToolMessages(Array.isArray(historyBefore?.messages) ? historyBefore.messages : []));
        const response = await subagentControlDeps.callGateway({
            method: "agent",
            params: {
                message: params.message,
        ...
        const waitMs = 30_000;
        const wait = await subagentControlDeps.callGateway({
            method: "agent.wait",
        ...
        if (wait?.status === "error") {
            ...
            return { status: "error", runId, error: waitError };
        }
        const history = await subagentControlDeps.callGateway({
            method: "chat.history",
            params: { sessionKey: targetSessionKey, limit: SUBAGENT_REPLY_HISTORY_LIMIT },
        });
        const latestReply = resolveLatestAssistantReplySnapshot(stripToolMessages(Array.isArray(history?.messages) ? history.messages : []));
        const replyText = latestReply.text && latestReply.fingerprint !== baselineReply.fingerprint
            ? latestReply.text
            : undefined;
        return { status: "ok", runId, replyText };
    }
    catch (err) {
        const error = err instanceof Error ? err.message : String(err);
