...
import { ANNOUNCE_COMPLETION_HARD_EXPIRY_MS, ANNOUNCE_EXPIRY_MS, capFrozenResultText, logAnnounceGiveUp, MAX_ANNOUNCE_RETRY_COUNT, MIN_ANNOUNCE_RETRY_DELAY_MS, persistSubagentSessionTiming, resolveAnnounceRetryDelayMs, safeRemoveAttachmentsDir, } from "./subagent-registry-helpers.js";
export function createSubagentRegistryLifecycleController(params) {
    ...
    const finalizeResumedAnnounceGiveUp = async (giveUpParams) => {
        ...
        const shouldDeleteAttachments = giveUpParams.entry.cleanup === "delete" || !giveUpParams.entry.retainAttachmentsOnKeep;
    ...
    const finalizeSubagentCleanup = async (runId, cleanup, didAnnounce) => {
        const entry = params.runs.get(runId);
        if (!entry) {
            return;
        }
    ...
    const completeSubagentRun = async (completeParams) => {
        ...
        let mutated = false;
        if (completeParams.reason === SUBAGENT_ENDED_REASON_COMPLETE &&
