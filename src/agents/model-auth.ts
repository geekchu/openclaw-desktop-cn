...
function resolveWebSearchPluginApiKey(config: OpenClawConfig | undefined, pluginId: string): unknown {
  const pluginConfig = resolveProviderWebSearchPluginConfig(config, pluginId);
  if (!pluginConfig || typeof pluginConfig !== "object" || Array.isArray(pluginConfig)) {
    return undefined;
  }
  return (pluginConfig as Record<string, unknown>).apiKey;
}
...
function resolveXaiConfigFallbackAuth(
  ...
  const xaiWebSearchPluginApiKey = resolveWebSearchPluginApiKey(config, "xai");

  const pluginApiKey = normalizeOptionalSecretInput(xaiWebSearchPluginApiKey);
  if (pluginApiKey) {
    return {
      apiKey: pluginApiKey,
  ...
  const pluginApiKeyRef = coerceSecretRef(xaiWebSearchPluginApiKey);
  if (pluginApiKeyRef) {
    return {
      apiKey:
  ...
  const grokSearchConfig = config?.tools?.web?.search?.grok as { apiKey?: unknown } | undefined;
  const grokApiKey = normalizeOptionalSecretInput(grokSearchConfig?.apiKey);
  if (grokApiKey) {
    return {
      apiKey: grokApiKey,
  ...
  const grokApiKeyRef = coerceSecretRef(grokSearchConfig?.apiKey);
  if (!grokApiKeyRef) {
    return undefined;
  }
  return {
