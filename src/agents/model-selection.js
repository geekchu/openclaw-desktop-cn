...
export function buildAllowedModelSet(params) {
    ...
    const resolveConfiguredSyntheticEntry = (provider, model) => {
        const configuredModels = params.cfg.models?.providers?.[provider]?.models;
        if (!Array.isArray(configuredModels)) {
            return null;
        }
        const configured = configuredModels.find((entry) => typeof entry?.id === "string" && entry.id.trim() === model);
        if (!configured) {
            return null;
        }
        return {
            provider,
            id: model,
            name: typeof configured.name === "string" && configured.name.trim().length > 0
                ? configured.name.trim()
                : model,
            contextWindow: typeof configured.contextWindow === "number" && configured.contextWindow > 0
