...
import { resolveAnthropicVertexClientRegion, resolveAnthropicVertexProjectId, } from "../plugin-sdk/anthropic-vertex.js";
function resolveAnthropicVertexMaxTokens(params) {
    const modelMax = typeof params.modelMaxTokens === "number" &&
        Number.isFinite(params.modelMaxTokens) &&
        params.modelMaxTokens > 0
        ? Math.floor(params.modelMaxTokens)
        : undefined;
    const requested = typeof params.requestedMaxTokens === "number" &&
...
export function createAnthropicVertexStreamFn(projectId, region, baseURL) {
    ...
    return (model, context, options) => {
        ...
        if (options?.reasoning) {
            const isAdaptive = model.id.includes("opus-4-6") ||
