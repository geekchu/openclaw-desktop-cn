...
export function resolveBootstrapWarningSignaturesSeen(report) {
    ...
    const single = typeof truncation?.promptWarningSignature === "string"
...
export function analyzeBootstrapBudget(params) {
    ...
    const nearLimitRatio = typeof params.nearLimitRatio === "number" &&
...
export function buildBootstrapTruncationSignature(analysis) {
...
export function formatBootstrapTruncationWarningLines(params) {
    ...
    const maxFiles = typeof params.maxFiles === "number" && Number.isFinite(params.maxFiles) && params.maxFiles > 0
    ...
    for (const file of topFiles) {
        const pct = file.rawChars > 0
            ? Math.round(((file.rawChars - file.injectedChars) / file.rawChars) * 100)
            : 0;
        const causeText = file.causes.length > 0
            ? file.causes.map((cause) => formatWarningCause(cause)).join(", ")
            : "";
        const nameLabel = (duplicateNameCounts.get(file.name) ?? 0) > 1 && file.path.trim().length > 0
            ? `${file.name} (${file.path})`
            : file.name;
...
export function buildBootstrapPromptWarning(params) {
    ...
    const warningSignaturesSeen = signature && params.mode !== "off"
...
export function appendBootstrapPromptWarning(prompt, warningLines, options) {
