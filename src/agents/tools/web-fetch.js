...
import { extractBasicHtmlContent, extractReadableContent, htmlToMarkdown, markdownToText, truncateText, } from "./web-fetch-utils.js";
...
function resolveFirecrawlApiKey(firecrawl) {
    const fromConfigRaw = firecrawl && "apiKey" in firecrawl
...
function resolveFirecrawlBaseUrl(firecrawl) {
    const fromConfig = firecrawl && "baseUrl" in firecrawl && typeof firecrawl.baseUrl === "string"
...
export async function fetchFirecrawlContent(params) {
    ...
    return await withTrustedWebToolsEndpoint({
        ...
        if (!response.ok || payload?.success === false) {
            ...
            throw new Error(`Firecrawl fetch failed (${response.status}): ${wrapWebContent(detail || response.statusText, "web_fetch")}`.trim());
        }
                ...
                if (readable?.text) {
                    ...
                    if (firecrawl) {
                        ...
                        if (basic?.text) {
                            ...
                            throw new Error("Web fetch extraction failed: Readability, Firecrawl, and basic HTML cleanup returned no content.");
                        }
                    }
                }
            }
...
export function createWebFetchTool(options) {
    ...
    const shouldResolveFirecrawlApiKey = runtimeFirecrawlActive === undefined ? firecrawl?.enabled !== false : runtimeFirecrawlActive;
