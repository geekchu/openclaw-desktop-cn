...
import { applyImageGenerationModelConfigDefaults, resolveMediaToolLocalRoots, } from "./media-tool-shared.js";
import { buildToolModelConfigFromCandidates, coerceToolModelConfig, hasToolModelConfig, resolveDefaultModelRef, } from "./model-config.helpers.js";
...
function resolveImageGenerationModelCandidates(cfg) {
...
function normalizeReferenceImages(args) {
    ...
    if (Array.isArray(args.images)) {
        imageCandidates.push(...args.images.filter((value) => typeof value === "string"));
    }
    const seen = new Set();
    const normalized = [];
    ...
    if (normalized.length > MAX_INPUT_IMAGES) {
        throw new ToolInputError(`Too many reference images: ${normalized.length} provided, maximum is ${MAX_INPUT_IMAGES}.`);
    }
    return normalized;
}
function resolveSelectedImageGenerationProvider(params) {
    const selectedRef = parseImageGenerationModelRef(params.modelOverride) ??
    ...
    return listRuntimeImageGenerationProviders({ config: params.config }).find((provider) => normalizeProviderId(provider.id) === selectedProvider ||
...
function validateImageGenerationCapabilities(params) {
    ...
    if (params.count > maxCount) {
        throw new ToolInputError(`${provider.id} ${isEdit ? "edit" : "generate"} supports at most ${maxCount} output image${maxCount === 1 ? "" : "s"}.`);
    }
    if (isEdit) {
        ...
        if (params.inputImageCount > maxInputImages) {
            throw new ToolInputError(`${provider.id} edit supports at most ${maxInputImages} reference image${maxInputImages === 1 ? "" : "s"}.`);
        }
    }
    if (params.size) {
        ...
        if ((geometry?.sizes?.length ?? 0) > 0 && !geometry?.sizes?.includes(params.size)) {
            throw new ToolInputError(`${provider.id} ${isEdit ? "edit" : "generate"} size must be one of ${geometry?.sizes?.join(", ")}.`);
        }
    }
    if (params.aspectRatio) {
        ...
        if ((geometry?.aspectRatios?.length ?? 0) > 0 &&
            !geometry?.aspectRatios?.includes(params.aspectRatio)) {
            throw new ToolInputError(`${provider.id} ${isEdit ? "edit" : "generate"} aspectRatio must be one of ${geometry?.aspectRatios?.join(", ")}.`);
        }
    }
    if (params.resolution) {
        ...
        if ((geometry?.resolutions?.length ?? 0) > 0 &&
            !geometry?.resolutions?.includes(params.resolution)) {
            throw new ToolInputError(`${provider.id} ${isEdit ? "edit" : "generate"} resolution must be one of ${geometry?.resolutions?.join("/")}.`);
        }
    }
}
async function loadReferenceImages(params) {
    ...
    for (const imageRawInput of params.imageInputs) {
        ...
        if (hasScheme && !looksLikeWindowsDrivePath && !isFileUrl && !isHttpUrl && !isDataUrl) {
            throw new ToolInputError(`Unsupported image reference: ${imageRawInput}. Use a file path, a file:// URL, a data: URL, or an http(s) URL.`);
        }
...
export function createImageGenerateTool(options) {
    ...
    return {
        ...
        execute: async (_toolCallId, args) => {
            ...
            if (action === "list") {
                const providers = listRuntimeImageGenerationProviders({ config: effectiveCfg }).map((provider) => ({
                    id: provider.id,
                ...
                const lines = providers.flatMap((provider) => {
                    ...
                    const modelLine = provider.models.length > 0
            ...
            const savedImages = await Promise.all(result.images.map((image) => saveMediaBuffer(image.buffer, image.mimeType, "tool-image-generation", undefined, filename || image.fileName)));
