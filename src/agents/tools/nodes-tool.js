...
import { cameraTempPath, parseCameraClipPayload, parseCameraSnapPayload, writeCameraClipPayloadToFile, writeCameraPayloadToFile, } from "../../cli/nodes-camera.js";
...
function resolveApproveScopes(commands) {
    const normalized = Array.isArray(commands)
        ? commands.filter((value) => typeof value === "string")
        : [];
...
async function resolveNodePairApproveScopes(gatewayOpts, requestId) {
    const pairing = await callGatewayTool("node.pair.list", gatewayOpts, {}, { scopes: ["operator.pairing", "operator.write"] });
                ...
                switch (action) {
                    ...
                    case "photos_latest": {
                        ...
                        for (const [index, photoRaw] of photos.entries()) {
                            ...
                            const createdAt = photoRaw && typeof photoRaw === "object" && !Array.isArray(photoRaw)
                                ? photoRaw.createdAt
                                : undefined;
                            details.push({
                                index,
