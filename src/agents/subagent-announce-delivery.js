...
import { deliveryContextFromSession, mergeDeliveryContext, normalizeDeliveryContext, resolveConversationDeliveryTarget, } from "../utils/delivery-context.js";
...
const DIRECT_ANNOUNCE_TRANSIENT_RETRY_DELAYS_MS = FAST_TEST_MODE
    ? [8, 16, 32]
    : [5_000, 10_000, 20_000];
export function resolveSubagentAnnounceTimeoutMs(cfg) {
...
export function isInternalAnnounceRequesterSession(sessionKey) {
...
export async function runAnnounceDeliveryWithRetry(params) {
    let retryIndex = 0;
    for (;;) {
...
export async function resolveSubagentCompletionOrigin(params) {
    ...
    const conversationId = threadId ||
        ...
        "";
    const requesterConversation = channel && conversationId ? { channel, accountId, conversationId } : undefined;
...
export function loadRequesterSessionEntry(requesterSessionKey) {
    const cfg = loadConfig();
...
async function sendSubagentAnnounceDirectly(params) {
    ...
    try {
        ...
        const effectiveDirectOrigin = params.expectsCompletionMessage && completionDirectOrigin
            ? completionDirectOrigin
            : directOrigin;
        const directChannel = typeof effectiveDirectOrigin?.channel === "string"
            ? effectiveDirectOrigin.channel.trim()
            : "";
        const directTo = typeof effectiveDirectOrigin?.to === "string" ? effectiveDirectOrigin.to.trim() : "";
        const shouldDeliverExternally = !params.requesterIsSubagent &&
            Boolean(directChannel) &&
            Boolean(directTo) &&
            !isInternalMessageChannel(directChannel);
        const threadId = effectiveDirectOrigin?.threadId != null && effectiveDirectOrigin.threadId !== ""
