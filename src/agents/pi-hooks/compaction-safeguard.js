...
export default function compactionSafeguardExtension(api) {
    api.on("session_before_compact", async (event, ctx) => {
        ...
        try {
            ...
            setCompactionSafeguardCancelReason(ctx.sessionManager, `Compaction safeguard could not resolve request credentials for ${model.provider}/${model.id}: ${error}`);
            return { cancel: true };
        }
        if (!requestAuth.ok) {
            log.warn(`Compaction safeguard: request credential resolution failed for ${model.provider}/${model.id}: ${requestAuth.error}`);
            setCompactionSafeguardCancelReason(ctx.sessionManager, `Compaction safeguard could not resolve request credentials for ${model.provider}/${model.id}: ${requestAuth.error}`);
            return { cancel: true };
        }
        ...
        if (!apiKey && !headers) {
            ...
            setCompactionSafeguardCancelReason(ctx.sessionManager, `Compaction safeguard could not resolve request credentials for ${model.provider}/${model.id}.`);
            return { cancel: true };
        }
        try {
            ...
            const reservedSuffix = appendSummarySection(appendSummarySection(appendSummarySection(appendSummarySection("", lastSplitTurnSection), preservedTurnsSection), toolFailureSection), fileOpsSummary);
            ...
            const normalizedSuffix = fullReservedSuffix && !/^\s/.test(fullReservedSuffix)
                ? `\n\n${fullReservedSuffix}`
                : fullReservedSuffix;
