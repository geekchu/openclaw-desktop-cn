...
import { deliveryContextFromSession, formatConversationTarget, normalizeDeliveryContext, resolveConversationDeliveryTarget, } from "../utils/delivery-context.js";
import { resolveAcpSpawnStreamLogPath, startAcpSpawnParentStreamRelay, } from "./acp-spawn-parent-stream.js";
import { resolveAgentConfig, resolveDefaultAgentId } from "./agent-scope.js";
import { resolveSandboxRuntimeStatus } from "./sandbox/runtime-status.js";
import { resolveInternalSessionKey, resolveMainSessionAlias } from "./tools/sessions-helpers.js";
const log = createSubsystemLogger("agents/acp-spawn");
export const ACP_SPAWN_MODES = ["run", "session"];
export const ACP_SPAWN_SANDBOX_MODES = ["inherit", "require"];
export const ACP_SPAWN_STREAM_TARGETS = ["parent"];
...
function isHeartbeatEnabledForSessionAgent(params) {
    ...
    const enabledByPolicy = hasExplicitHeartbeatAgents
        ? agentEntries.some((entry) => Boolean(entry?.heartbeat) && normalizeAgentId(entry?.id) === requesterAgentId)
        : requesterAgentId === resolveDefaultAgentId(params.cfg);
    if (!enabledByPolicy) {
        return false;
    }
    const heartbeatEvery = resolveAgentConfig(params.cfg, requesterAgentId)?.heartbeat?.every ??
...
function resolveAcpSpawnRequesterState(params) {
    ...
    const hasActiveSubagentBinding = isSubagentSession && params.parentSessionKey
        ? bindingService
            .listBySession(params.parentSessionKey)
            .some((record) => record.targetKind === "subagent" && record.status !== "ended")
        : false;
    const hasThreadContext = typeof params.ctx.agentThreadId === "string"
    ...
    return {
        ...
        heartbeatRelayRouteUsable: params.parentSessionKey && requesterAgentId
...
function resolveAcpSpawnStreamPlan(params) {
    ...
    const implicitStreamToParent = !params.streamToParentRequested &&
...
async function bindPreparedAcpThread(params) {
    ...
    if (!binding.conversation.conversationId) {
        throw new Error(`Failed to create and bind a ${params.preparedBinding.channel} thread for this ACP session.`);
    }
...
function resolveAcpSpawnBootstrapDeliveryPlan(params) {
    ...
    const inferredDeliveryTo = boundDeliveryTarget.to ??
    ...
    const useInlineDelivery = hasDeliveryTarget &&
...
export async function spawnAcpDirect(params, ctx) {
    ...
    const streamLogPath = effectiveStreamToParent && parentSessionKey
