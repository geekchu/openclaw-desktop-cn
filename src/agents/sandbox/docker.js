...
async function ensureDockerDaemon() {
    ...
    if (!(await isDockerInstalled())) {
        throw new Error(`${DOCKER_NOT_INSTALLED_MARKER} Docker 未安装。目录访问限制功能需要 Docker 来创建隔离的沙盒环境。请先安装 Docker Desktop。`);
    }
    ...
    if (!launched) {
        ...
        throw new Error(`${DOCKER_NOT_RUNNING_MARKER} Docker 已安装但未运行。请手动启动 Docker Desktop 后重试。`);
    }
    ...
    dockerAutoStartAttempted = false;
    throw new Error(`${DOCKER_NOT_RUNNING_MARKER} Docker Desktop 已安装但未能在 30 秒内启动。请手动启动 Docker Desktop 后重试。`);
}
