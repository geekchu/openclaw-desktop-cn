...
function parseClaudeCliJsonlResult(params) {
    ...
    if (typeof params.parsed.type === "string" &&
...
export function parseCliJsonl(raw, backend, providerId) {
    const lines = raw
        .split(/\r?\n/g)
        .map((line) => line.trim())
        .filter(Boolean);
...
export function parseCliOutput(params) {
    ...
    if (outputMode === "jsonl") {
        return (parseCliJsonl(params.raw, params.backend, params.providerId) ?? {
