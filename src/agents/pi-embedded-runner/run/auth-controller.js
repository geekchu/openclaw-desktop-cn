...
export function createEmbeddedRunAuthController(params) {
    ...
    const refreshRuntimeAuth = async (reason) => {
        ...
        runtimeAuthState.refreshInFlight = (async () => {
            ...
            .catch((err) => {
            const runtimeModel = params.getRuntimeModel();
            params.log.warn(`Runtime auth refresh failed for ${runtimeModel.provider}: ${describeUnknownError(err)}`);
            throw err;
        })
            .finally(() => {
    ...
    const applyApiKeyInfo = async (candidate) => {
        ...
        if (shouldTraceProviderAuth(params.getRuntimeModel().provider)) {
            params.log.info(`[xai-auth] auth-controller resolved api key: source=${apiKeyInfo.source} mode=${apiKeyInfo.mode} profile=${apiKeyInfo.profileId ?? candidate ?? "none"} key=${summarizeProviderAuthKey(apiKeyInfo.apiKey)}`);
        }
        ...
        if (shouldTraceProviderAuth(runtimeModel.provider)) {
            params.log.info(`[xai-auth] auth-controller prepared runtime auth: returnedKey=${summarizeProviderAuthKey(preparedAuth?.apiKey)} baseUrl=${preparedAuth?.baseUrl ?? runtimeModel.baseUrl ?? "default"} expiresAt=${preparedAuth?.expiresAt ?? "none"}`);
        }
        if (preparedAuth?.baseUrl) {
            params.setRuntimeModel({ ...runtimeModel, baseUrl: preparedAuth.baseUrl });
        ...
        if (preparedAuth?.apiKey) {
            params.authStorage.setRuntimeApiKey(runtimeModel.provider, preparedAuth.apiKey);
            if (shouldTraceProviderAuth(runtimeModel.provider)) {
                params.log.info(`[xai-auth] auth-controller set runtime api key from prepared auth: key=${summarizeProviderAuthKey(preparedAuth.apiKey)}`);
            }
            params.setRuntimeAuthState({
        ...
        if (!runtimeAuthHandled) {
            ...
            if (shouldTraceProviderAuth(runtimeModel.provider)) {
                params.log.info(`[xai-auth] auth-controller set runtime api key directly: key=${summarizeProviderAuthKey(apiKeyInfo.apiKey)}`);
            }
