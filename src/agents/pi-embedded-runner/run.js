...
import { coerceToFailoverError, describeFailoverError, FailoverError, resolveFailoverStatus, } from "../failover-error.js";
import { hasDifferentLiveSessionModelSelection, LiveSessionModelSwitchError, resolveLiveSessionModelSelection, consumeLiveSessionModelSwitch, } from "../live-model-switch.js";
import { applyLocalNoAuthHeaderOverride, ensureAuthProfileStore, resolveAuthProfileOrder, } from "../model-auth.js";
import { normalizeProviderId } from "../model-selection.js";
import { ensureOpenClawModelsJson } from "../models-config.js";
import { classifyFailoverReason, extractObservedOverflowTokenCount, formatAssistantErrorText, formatBillingErrorMessage, isAuthAssistantError, isBillingAssistantError, isCompactionFailureError, isFailoverAssistantError, isFailoverErrorMessage, isLikelyContextOverflowError, isRateLimitAssistantError, isTimeoutErrorMessage, parseImageDimensionError, parseImageSizeError, pickFallbackThinkingLevel, } from "../pi-embedded-helpers.js";
...
import { buildErrorAgentMeta, buildUsageAgentMetaFields, createCompactionDiagId, OVERLOAD_FAILOVER_BACKOFF_POLICY, resolveActiveErrorContext, resolveMaxRunRetryIterations, scrubAnthropicRefusalMagic, } from "./run/helpers.js";
import { buildEmbeddedRunPayloads } from "./run/payloads.js";
import { resolveEffectiveRuntimeModel, resolveHookModelSelection } from "./run/setup.js";
import { sessionLikelyHasOversizedToolResults, truncateOversizedToolResultsInSession, } from "./tool-result-truncation.js";
...
export async function runEmbeddedPiAgent(params) {
    ...
    return enqueueSession(() => enqueueGlobal(async () => {
        ...
        let bootstrapPromptWarningSignaturesSeen = params.bootstrapPromptWarningSignaturesSeen ??
        ...
        try {
            ...
            const runOwnsCompactionBeforeHook = async (reason) => {
                ...
                try {
                    await hookRunner.runBeforeCompaction({ messageCount: -1, sessionFile: params.sessionFile }, hookCtx);
                }
                catch (hookErr) {
            ...
            while (true) {
                ...
                if (hasDifferentLiveSessionModelSelection(resolveCurrentLiveSelection(), nextSelection)) {
                    log.info(`live session model switch detected before attempt for ${params.sessionId}: ${provider}/${modelId} -> ${nextSelection.provider}/${nextSelection.model}`);
                ...
                const assistantErrorText = lastAssistant?.stopReason === "error"
                    ...
                    : undefined;
                const canRestartForLiveSwitch = !attempt.didSendViaMessagingTool &&
                ...
                const failedOrAbortedAttempt = aborted || Boolean(promptError) || Boolean(assistantErrorText) || timedOut;
                ...
                if (failedOrAbortedAttempt &&
                    ...
                    log.info(`live session model switch detected after failed attempt for ${params.sessionId}: ${provider}/${modelId} -> ${persistedSelection.provider}/${persistedSelection.model}`);
                ...
                if (timedOut && !timedOutDuringCompaction) {
                    ...
                    const tokenUsedRatio = lastTurnPromptTokens != null && ctxInfo.tokens > 0
                        ? lastTurnPromptTokens / ctxInfo.tokens
                        : 0;
                    if (timeoutCompactionAttempts >= MAX_TIMEOUT_COMPACTION_ATTEMPTS) {
                        log.warn(`[timeout-compaction] already attempted timeout compaction ${timeoutCompactionAttempts} time(s); falling through to failover rotation`);
                    }
                    else if (tokenUsedRatio > 0.65) {
                        const timeoutDiagId = createCompactionDiagId();
                        timeoutCompactionAttempts++;
                        log.warn(`[timeout-compaction] LLM timed out with high prompt token usage (${Math.round(tokenUsedRatio * 100)}%); ` +
                ...
                if (promptError && !aborted) {
                    ...
                    if (fallbackConfigured && promptFailoverFailure) {
                        ...
                        await maybeBackoffBeforeOverloadFailover(promptFailoverReason);
                        throw (normalizedPromptFailover ??
                ...
                if (authFailure &&
                    (await maybeRefreshRuntimeAuthForAuthError(lastAssistant?.errorMessage ?? "", runtimeAuthRetry))) {
                    authRetryPending = true;
                    continue;
                }
                ...
                if (payloads.length === 0 &&
                    ...
                    if (incompleteStopReason === "toolUse" || incompleteStopReason === "error") {
                        log.warn(`incomplete turn detected: runId=${params.runId} sessionId=${params.sessionId} ` +
                        ...
                        const hadMutatingTools = attempt.toolMetas.some((t) => isLikelyMutatingToolName(t.toolName));
