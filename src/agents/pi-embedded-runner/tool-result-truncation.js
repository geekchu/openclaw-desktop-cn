...
function hasImportantTail(text) {
    ...
    return (/\b(error|exception|failed|fatal|traceback|panic|stack trace|errno|exit code)\b/.test(tail) ||
...
export async function truncateOversizedToolResultsInSession(params) {
    ...
    try {
        ...
        log.info(`[tool-result-truncation] Truncated ${rewriteResult.rewrittenEntries} tool result(s) in session ` +
