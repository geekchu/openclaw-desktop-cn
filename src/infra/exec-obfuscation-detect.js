...
function extractHttpUrls(command) {
...
function pathMatchesSafePrefix(pathname, pathPrefix) {
    return pathname === pathPrefix || pathname.startsWith(`${pathPrefix}/`);
}
function shouldSuppressCurlPipeShell(command) {
    ...
    return SAFE_CURL_PIPE_URLS.some((candidate) => url.hostname === candidate.host &&
...
export function detectCommandObfuscation(command) {
    ...
    for (const pattern of OBFUSCATION_PATTERNS) {
        ...
        const suppressed = pattern.id === "curl-pipe-shell" && urlCount <= 1 && shouldSuppressCurlPipeShell(command);
        if (suppressed) {
            continue;
        }
