...
import { describeNativeExecApprovalClientSetup, listNativeExecApprovalClientLabels, supportsNativeExecApprovalClient, } from "./exec-approval-surface.js";
import { resolveExecApprovalAllowedDecisions, } from "./exec-approvals.js";
...
function resolveNativeExecApprovalClientList(params) {
    return formatHumanList(listNativeExecApprovalClientLabels({
...
export function buildApprovalInteractiveReplyFromActionDescriptors(actions) {
...
export function parseExecApprovalCommandText(raw) {
    const trimmed = raw.trim();
    const match = trimmed.match(/^\/?approve(?:@[^\s]+)?\s+([A-Za-z0-9][A-Za-z0-9._:-]*)\s+(allow-once|allow-always|always|deny)\b/i);
    if (!match) {
        return null;
    }
...
export function formatExecApprovalExpiresIn(expiresAtMs, nowMs) {
...
export function buildExecApprovalPendingReplyPayload(params) {
    ...
    if (typeof params.expiresAtMs === "number" && Number.isFinite(params.expiresAtMs)) {
        info.push(`Expires in: ${formatExecApprovalExpiresIn(params.expiresAtMs, params.nowMs ?? Date.now())}`);
    }
...
export function buildExecApprovalUnavailableReplyPayload(params) {
    ...
    if (params.reason === "initiating-platform-disabled") {
        ...
        const setupText = channel && params.channelLabel && supportsNativeExecApprovalClient(channel)
