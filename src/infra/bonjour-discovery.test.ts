  ...
  it("discovers beacons on darwin across local + wide-area domains", async () => {
    ...
    const run = vi.fn(async (argv: string[], options: { timeoutMs: number }) => {
      ...
      if (argv[0] === "dns-sd" && argv[1] === "-L") {
        ...
        return {
          stdout: [
            `${instance}._openclaw-gw._tcp. can be reached at ${host}:28789`,
            txtParts.join(" "),
            "",
          ].join("\n"),
          stderr: "",
  ...
  it("decodes dns-sd octal escapes in TXT displayName", async () => {
    const run = vi.fn(async (argv: string[], options: { timeoutMs: number }) => {
      ...
      if (argv[0] === "dns-sd" && argv[1] === "-L") {
        return {
          stdout: [
            "Studio Gateway._openclaw-gw._tcp. can be reached at studio.local:28789",
            "txtvers=1 displayName=Peter\\226\\128\\153s\\032Mac\\032Studio lanHost=studio.local gatewayPort=28789 sshPort=22",
            "",
          ].join("\n"),
          stderr: "",
          code: 0,
