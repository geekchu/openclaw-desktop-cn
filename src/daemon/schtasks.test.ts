...
describe("readScheduledTaskCommand", () => {
  it("parses basic command script", async () => {
    ...
    try {
      ...
      await fs.writeFile(
        scriptPath,
        ["@echo off", "node gateway.js --port 28789"].join("\r\n"),
        "utf8",
      );
      ...
      expect(result).toEqual({
        programArguments: ["node", "gateway.js", "--port", "28789"],
      });
    } finally {
  ...
  it("parses script with environment variables", async () => {
    ...
    try {
      ...
      await fs.writeFile(
        scriptPath,
        ["@echo off", "set NODE_ENV=production", "set PORT=28789", "node gateway.js"].join("\r\n"),
        "utf8",
      );
  ...
  it("parses command with Windows backslash paths", async () => {
    ...
    try {
      ...
      await fs.writeFile(
        ...
        [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\openclaw\\dist\\index.js gateway --port 28789',
        ].join("\r\n"),
        "utf8",
      );
  ...
  it("preserves UNC paths in command arguments", async () => {
    ...
    try {
      ...
      await fs.writeFile(
        ...
        [
          "@echo off",
          '"\\\\fileserver\\OpenClaw Share\\node.exe" "\\\\fileserver\\OpenClaw Share\\dist\\index.js" gateway --port 28789',
        ].join("\r\n"),
        "utf8",
      );
