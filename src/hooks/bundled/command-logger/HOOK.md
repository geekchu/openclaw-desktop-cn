...
2. **Appends to log file** - Writes a JSON line to `~/.openclawcn/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.openclawcn/logs/commands.log ~/.openclawcn/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/openclaw`:
   ```
   /home/username/.openclawcn/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
