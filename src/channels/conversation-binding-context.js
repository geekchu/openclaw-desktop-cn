...
export function resolveConversationBindingContext(params) {
    ...
    if (resolvedByProvider?.conversationId) {
        const resolvedParentConversationId = channel === "telegram" && !threadId && !resolvedByProvider.parentConversationId
    ...
    const normalizedParentConversationId = channel === "telegram" && !threadId && !parentConversationId
        ? conversationId
