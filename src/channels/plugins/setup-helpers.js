...
export function resolveSingleAccountKeysToMove(params) {
    const hasNamedAccounts = Object.keys(params.channel.accounts ?? {}).filter(Boolean).length >
        0;
...
export function resolveSingleAccountPromotionTarget(params) {
    ...
    const normalizedDefaultAccount = typeof params.channel.defaultAccount === "string" && params.channel.defaultAccount.trim()
    ...
    if (normalizedDefaultAccount) {
        if (normalizedDefaultAccount !== DEFAULT_ACCOUNT_ID) {
            const matchedAccountId = Object.entries(accounts).find(([accountId, value]) => accountId &&
                value &&
                typeof value === "object" &&
                normalizeAccountId(accountId) === normalizedDefaultAccount)?.[0];
            if (matchedAccountId) {
