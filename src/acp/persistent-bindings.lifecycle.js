...
export async function resetAcpSessionInPlace(params) {
    ...
    const configuredBinding = !meta || !normalizeText(meta.agent)
    ...
    const acpManager = getAcpSessionManager();
    const agent = normalizeText(meta.agent) ??
    ...
    try {
        ...
        const runtimeOptionsPatch = Object.fromEntries(Object.entries(runtimeOptions).filter(([, value]) => value !== undefined));
